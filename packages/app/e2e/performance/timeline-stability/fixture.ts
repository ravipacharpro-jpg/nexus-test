...
import { SessionStatusEvent } from "@nexus-ai/schema/session-status-event"
import { SessionV1 } from "@nexus-ai/schema/session-v1"
import type {
  AssistantMessage,
  GlobalEvent,
  Message,
...
export const directory = "C:/NEXUS/TimelineStability"
...
export const model = { providerID: "nexus", modelID: "claude-opus-4-6", variant: "max" }
...
export async function setupTimeline(
  ...
  if (input.locale) {
    await page.addInitScript((locale) => {
      localStorage.setItem("nexus.global.dat:language", JSON.stringify({ locale }))
    }, input.locale)
  }
