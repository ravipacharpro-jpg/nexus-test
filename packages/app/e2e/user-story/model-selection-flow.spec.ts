...
const directory = "C:/NEXUS/NewProject"

test("creates a session in a new project, connects NEXUS Go, and selects its model", async ({ page }) => {
  ...
  await mockNEXUSServer(page, {
    ...
    provider: () => ({
      ...
      connected: connectedGo ? ["nexus", "nexus-go"] : ["nexus"],
      default: { providerID: "nexus", modelID: "free-model" },
    }),
    integrationMethods: { "nexus-go": [{ type: "api", label: "API key" }] },
    onConnectKey: (input) => {
      connections.push(input)
      if (input.integrationID === "nexus-go") pendingGo = true
    },
    onInstanceDispose: () => {
  ...
  await page.addInitScript(() => {
    ...
    localStorage.setItem("nexus.global.dat:server", JSON.stringify({ projects: { local: [] } }))
  })
  ...
  await expect(page.locator('[data-section="free-models"]')).toContainText("Free models provided by NEXUS")

  await page.locator('[data-provider-id="nexus-go"]').click()
  ...
  expect(connections).toEqual([{ integrationID: "nexus-go", body: { type: "api", key: "mock-go-api-key" } }])

  await expect(modelControl).toHaveAttribute("data-control-type", "popover")
  await modelControl.click()
  const goModel = page.locator('[data-option-key="nexus-go:go-model-1"]')
