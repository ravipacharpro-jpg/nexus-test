...
import { NormalizedProviderListResponse } from "@nexus-ai/session-ui/context"
...
export const loadGlobalConfigQuery = (scope: ServerScope, sdk: NexusClient, protocol?: Promise<ServerProtocol>) =>
  queryOptions({
    ...
    queryFn: () =>
      retry(() =>
        api.list().then((projects) => {
          return projects
            .filter((p) => !!p?.id)
            .filter((p) => !!p.worktree && !p.worktree.includes("nexus-test"))
