...
import type { retry } from "@nexus-ai/core/util/retry"
import type { NEXUSEvent, SessionApi } from "@nexus-ai/client/promise"
import type { Message, NexusClient, Part, Session } from "@nexus-ai/sdk/v2/client"
...
describe("server session", () => {
  test("projects V2 session events into current and legacy message state", () => {
    ...
    const apply = (input: object) => ctx.store.applyV2(input as NEXUSEvent)
  ...
  test("extends a current page to include the user for split assistant turns", async () => {
    ...
    const store = createServerSession({} as NexusClient, {} as SessionApi, messageApi)
    store.remember(session("root"))
