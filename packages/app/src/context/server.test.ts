  ...
  test("moves an existing canonical web bucket into local scope", () => {
    expect(
      migrateCanonicalLocalServerState(
        {
          list: [],
          projects: { "https://nexus.example.com": [{ worktree: "/remote", expanded: true }] },
          lastProject: { "https://nexus.example.com": "/remote" },
        },
        ServerConnection.Key.make("https://nexus.example.com"),
      ),
    ).toEqual({
      list: [],
      ...
      migrateCanonicalLocalServerState(
        {
          ...
          lastProject: { local: "/local", "https://nexus.example.com": "/remote" },
        },
        ServerConnection.Key.make("https://nexus.example.com"),
      ),
    ).toEqual({
