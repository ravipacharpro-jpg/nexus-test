...
import { type Session } from "@nexus-ai/sdk/v2/client"
import {
  childSessionOnPath,
  closeHomeProject,
  compareSessionTime,
...
describe("layout deep links", () => {
  test("parses open-project deep links", () => {
    expect(parseDeepLink("nexus://open-project?directory=/tmp/demo")).toBe("/tmp/demo")
  })

  test("ignores non-project deep links", () => {
    expect(parseDeepLink("nexus://other?directory=/tmp/demo")).toBeUndefined()
    expect(parseDeepLink("https://example.com")).toBeUndefined()
  })

  test("ignores malformed deep links safely", () => {
    expect(() => parseDeepLink("nexus://open-project/%E0%A4%A%")).not.toThrow()
    expect(parseDeepLink("nexus://open-project/%E0%A4%A%")).toBeUndefined()
  })
  ...
  test("ignores open-project deep links without directory", () => {
    expect(parseDeepLink("nexus://open-project")).toBeUndefined()
    expect(parseDeepLink("nexus://open-project?directory=")).toBeUndefined()
  })

  test("collects only valid open-project directories", () => {
    const result = collectOpenProjectDeepLinks([
      "nexus://open-project?directory=/a",
      "nexus://other?directory=/b",
      "nexus://open-project?directory=/c",
    ])
    expect(result).toEqual(["/a", "/c"])
  })

  test("parses new-session deep links with optional prompt", () => {
    expect(parseNewSessionDeepLink("nexus://new-session?directory=/tmp/demo")).toEqual({ directory: "/tmp/demo" })
    expect(parseNewSessionDeepLink("nexus://new-session?directory=/tmp/demo&prompt=hello%20world")).toEqual({
      directory: "/tmp/demo",
      prompt: "hello world",
    })
  })
  ...
  test("ignores new-session deep links without directory", () => {
    expect(parseNewSessionDeepLink("nexus://new-session")).toBeUndefined()
    expect(parseNewSessionDeepLink("nexus://new-session?directory=")).toBeUndefined()
  })

  test("collects only valid new-session deep links", () => {
    const result = collectNewSessionDeepLinks([
      "nexus://new-session?directory=/a",
      "nexus://open-project?directory=/b",
      "nexus://new-session?directory=/c&prompt=ship%20it",
    ])
  ...
  test("drains global deep links once", () => {
    const target = {
      __NEXUS__: {
        deepLinks: ["nexus://open-project?directory=/a"],
      },
    } as unknown as Window & { __NEXUS__?: { deepLinks?: string[] } }

    expect(drainPendingDeepLinks(target)).toEqual(["nexus://open-project?directory=/a"])
