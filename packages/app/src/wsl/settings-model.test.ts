...
describe("WSL server settings presentation", () => {
  ...
  test("offers install and update only when NEXUS needs attention", () => {
    expect(wslNexusAction(undefined)).toBeUndefined()
    expect(
      wslNexusAction({
        distro: "Debian",
        resolvedPath: null,
    ...
    ).toBe("wsl.onboarding.installNexus")
    expect(
      wslNexusAction({
        distro: "Debian",
    ...
    ).toBe("wsl.onboarding.updateNexus")
    expect(
      wslNexusAction({
        distro: "Debian",
        resolvedPath: "/usr/local/bin/nexus",
  ...
  test("does not report ready when NEXUS is present but cannot run", () => {
    const model = addServerViewModel({
      state: {
        ...readyWslState,
        installed: [{ name: "Debian", version: 2, isDefault: true }],
        ...
        nexusChecks: {
          Debian: {
            distro: "Debian",
            resolvedPath: "/home/me/.nexus/bin/nexus",
            version: null,
            expectedVersion: "1.2.3",
            matchesDesktop: null,
            error: "nexus is installed but could not run",
          },
        },
      },
      view: "main",
    ...
    expect(model.distroStatuses.Debian).toEqual({
      label: { key: "wsl.onboarding.installNexus" },
      tone: "warning",
    })
    expect(model.primaryButton.action).toBe("install-nexus")
  })
