...
export function wslNexusAction(check?: WslNexusCheck) {
  if (!check) return
  if (!check.resolvedPath) return "wsl.onboarding.installNexus"
  if (check.matchesDesktop === false) return "wsl.onboarding.updateNexus"
}
...
export function addServerViewModel(input: {
  ...
  const nexusCheck = selectedDistro ? (state?.nexusChecks[selectedDistro] ?? null) : null
...
function addServerDistroStatus(input: {
  ...
  const check = input.state?.nexusChecks[input.name]
  if (!check) {
    if (input.probingAddable || (job?.kind === "probe-addable" && job.distros.includes(input.name))) {
      return checkingStatus()
    }
  ...
  if (check.matchesDesktop === false) return { label: { key: "wsl.onboarding.updateNexus" }, tone: "warning" }
  if (!check.resolvedPath) return { label: { key: "wsl.onboarding.distroStatus.nexusMissing" }, tone: "warning" }
  if (check.error) return { label: { key: "wsl.onboarding.installNexus" }, tone: "warning" }
...
function addServerPrimaryButton(input: {
  ...
  const installingNexus =
    input.state?.job?.kind === "install-nexus" && input.state.job.distro === input.selectedDistro
  if (!ready || probingNexus) {
    return {
      variant: "contrast",
  ...
  if (!addServerNexusReady(input.nexusCheck)) {
    const update = !!input.nexusCheck?.resolvedPath && input.nexusCheck.matchesDesktop === false
    return {
      variant: "neutral",
      label: installingNexus
        ? { key: "wsl.onboarding.updatingNexus" }
        : update
          ? { key: "wsl.onboarding.updateNexus" }
          : { key: "wsl.onboarding.installNexus" },
      disabled: !!input.state?.job || input.adding,
      action: "install-nexus",
      loading: installingNexus,
...
function addServerNexusReady(check: WslNexusCheck | null) {
...
function addServerSelectedDistroSettled(state: WslServersState | undefined, selectedDistro: string | null) {
  ...
  return !!state.nexusChecks[selectedDistro]
}
...
export function addableProbePlan(input: {
  ...
  const pending = ordered.flatMap((item) => {
    ...
    if (wslDistroReady(state, item.name) && !state.nexusChecks[item.name]) return [`nexus:${item.name}`]
    return []
  })
