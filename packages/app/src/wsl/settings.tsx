...
import { Tag } from "@nexus-ai/ui/v2/badge-v2"
...
import { wslNexusAction, wslRuntimeRetryable } from "./settings-model"
    ...
    <Show when={api}>
      <For each={props.servers()}>
        {(item) => {
          ...
          const check = () => wsl.data?.nexusChecks[item.config.distro]
          const nexusAction = () => wslNexusAction(check())
          const busy = () => wsl.data?.job?.kind === "install-nexus" && wsl.data.job.distro === item.config.distro
          return (
            <div class="settings-v2-servers-row">
              ...
              <div class="settings-v2-servers-actions">
                ...
                <Show when={nexusAction()}>
                  {(label) => (
                    <ButtonV2
                      size="small"
                      disabled={busy() || request.isPending}
                      onClick={() => api && request.mutate(() => api.installNexus(item.config.distro))}
                    >
