...
This is the living design and status document for `@nexus-ai/codemode` and its existing V2 NEXUS adapter.
...
`packages/nexus/src/tool/code-mode.ts`, where nested MCP calls run the `tool.execute.before` and
