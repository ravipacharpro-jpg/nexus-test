...
import { and, Database, eq, isNull, lt, or, sql } from "@nexus-ai/console-core/drizzle/index.js"
import { KeyTable } from "@nexus-ai/console-core/schema/key.sql.js"
import { BillingTable, LiteTable, SubscriptionTable, UsageTable } from "@nexus-ai/console-core/schema/billing.sql.js"
import { centsToMicroCents } from "@nexus-ai/console-core/util/price.js"
import { getMonthlyBounds, getWeekBounds } from "@nexus-ai/console-core/util/date.js"
import { Identifier } from "@nexus-ai/console-core/identifier.js"
import { Billing } from "@nexus-ai/console-core/billing.js"
import { Actor } from "@nexus-ai/console-core/actor.js"
import { WorkspaceTable } from "@nexus-ai/console-core/schema/workspace.sql.js"
import { ZenData } from "@nexus-ai/console-core/model.js"
import { Subscription } from "@nexus-ai/console-core/subscription.js"
import { BlackData } from "@nexus-ai/console-core/black.js"
import { UserTable } from "@nexus-ai/console-core/schema/user.sql.js"
import { ModelTable } from "@nexus-ai/console-core/schema/model.sql.js"
import { ProviderTable } from "@nexus-ai/console-core/schema/provider.sql.js"
...
import { LiteData } from "@nexus-ai/console-core/lite.js"
import { Resource } from "@nexus-ai/console-resource"
...
import { Workspace } from "@nexus-ai/console-core/workspace.js"
...
export async function handler(
  ...
  try {
    ...
    const sessionId = input.request.headers.get("x-nexus-session") ?? ""
    const requestId = input.request.headers.get("x-nexus-request") ?? ""
    const ocClient = input.request.headers.get("x-nexus-client") ?? ""
    const projectId = input.request.headers.get("x-nexus-project") ?? ""
    ...
    if (
      ...
      throw new DataPolicyError(
        t("zen.api.error.trainingNotAllowed", {
          consoleGoUrl: `https://nexus.ai/workspace/${authInfo.workspaceID}/go`,
        }),
      )
    ...
    const retriableRequest = async (retry: RetryOptions = { excludeProviders: [], retryCount: 0 }) => {
      ...
      const res = await fetchWithRetryableStatus(
        reqUrl,
        {
          ...
          headers: (() => {
            ...
            headers.delete("x-nexus-request")
            if (!isNewInference) headers.delete("x-nexus-session")
            headers.delete("x-nexus-project")
            headers.delete("x-nexus-client")
            return headers
          })(),
          body: reqBody,
      ...
      if (isNewInference) {
        const resEndpointId = res.headers.get("x-nexus-endpoint-id")
        const resEndpointModelId = res.headers.get("x-nexus-upstream-model-id")
  ...
  function validateBilling(authInfo: AuthInfo, modelInfo: ModelInfo): BillingSource {
    ...
    if (opts.modelList === "lite" && authInfo.billing.lite && authInfo.lite) {
      try {
        const consoleGoUrl = `https://nexus.ai/workspace/${authInfo.workspaceID}/go`
    ...
    const billingUrl = `https://nexus.ai/workspace/${authInfo.workspaceID}/billing`
    const membersUrl = `https://nexus.ai/workspace/${authInfo.workspaceID}/members`
