...
import type { IntegrationOAuthMethodRegistration } from "@nexus-ai/plugin/v2/effect/integration"
import { define } from "@nexus-ai/plugin/v2/effect/plugin"
...
const defaultServer = "https://nexus.ai/console"
const clientID = "nexus-cli"
...
export const NexusPlugin = define<HttpClient.HttpClient | EventV2.Service | Scope.Scope>({
  id: "nexus",
  effect: Effect.fn(function* (ctx) {
    ...
    const load = Effect.fn("NexusPlugin.load")(function* () {
      const connection = yield* ctx.integration.connection.active("nexus")
      ...
      providers = credential
        ? yield* fetchProviders(http, credential).pipe(
            Effect.catch((cause) =>
              Effect.logWarning("failed to load NEXUS provider config", { cause }).pipe(Effect.as(undefined)),
            ),
          )
        : undefined
    })
    ...
    yield* ctx.integration.transform((draft) => {
      draft.update("nexus", (integration) => {
        integration.name = "NEXUS"
      })
      draft.method.update(oauth(http))
      draft.method.update({ integrationID: "nexus", method: { type: "key", label: "API key (service account)" } })
    })

    connected = (yield* ctx.integration.connection.active("nexus")) !== undefined
    yield* ctx.catalog.transform((catalog) => {
      for (const [providerID, item] of Object.entries(providers ?? {})) {
        catalog.provider.update(providerID, (provider) => {
          provider.integrationID = Integration.ID.make("nexus")
      ...
      const item = catalog.provider.get(ProviderV2.ID.nexus)
      if (!item) return
      const hasKey = Boolean(process.env.NEXUS_API_KEY || connected || item.provider.request.body.apiKey)
    ...
    yield* events.subscribe(Integration.Event.ConnectionUpdated).pipe(
      Stream.filter((event) => event.data.integrationID === Integration.ID.make("nexus")),
