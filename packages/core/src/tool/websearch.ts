...
export class ConfigService extends Context.Service<ConfigService, Config>()("@nexus/v2/WebSearchConfig") {}

/** Isolates the retained product environment contract from the generic tool implementation. */
export const defaultConfigLayer = Layer.sync(ConfigService, () =>
  ConfigService.of({
    provider:
      process.env.NEXUS_WEBSEARCH_PROVIDER === "exa" || process.env.NEXUS_WEBSEARCH_PROVIDER === "parallel"
        ? process.env.NEXUS_WEBSEARCH_PROVIDER
        : undefined,
    enableExa: truthy("NEXUS_EXPERIMENTAL") || truthy("NEXUS_ENABLE_EXA") || truthy("NEXUS_EXPERIMENTAL_EXA"),
    enableParallel: truthy("NEXUS_ENABLE_PARALLEL") || truthy("NEXUS_EXPERIMENTAL_PARALLEL"),
              ...
              const text =
                provider === "exa"
                  ...
                  : yield* callMcp(
                      ...
                      {
                        "User-Agent": `nexus/${InstallationVersion}`,
