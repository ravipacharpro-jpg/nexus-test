...
import { Config } from "@nexus-ai/core/config"
import { ConfigProvider } from "@nexus-ai/core/config/provider"
import { AppNodeBuilder } from "@nexus-ai/core/effect/app-node-builder"
import { LayerNode } from "@nexus-ai/core/effect/layer-node"
import { ConfigMigrateV1 } from "@nexus-ai/core/v1/config/migrate"
...
import { Policy } from "@nexus-ai/core/policy"
import { Project } from "@nexus-ai/core/project"
...
describe("Config", () => {
  ...
  it.live("loads nexus JSON and JSONC files from lowest to highest priority", () =>
    Effect.acquireRelease(
      Effect.promise(() => tmpdir()),
      (tmp) => Effect.promise(() => tmp[Symbol.asyncDispose]()),
    ).pipe(
      Effect.flatMap((tmp) =>
        Effect.gen(function* () {
          yield* Effect.promise(() =>
            Promise.all([
              fs.writeFile(
                path.join(tmp.path, "nexus.json"),
                JSON.stringify({ $schema: "base", providers: { base: provider } }),
              ),
              fs.writeFile(
                path.join(tmp.path, "nexus.jsonc"),
                `{
          ...
          return yield* Effect.gen(function* () {
            ...
            expect(documents[0]?.path).toBe(path.join(tmp.path, "nexus.json"))
            expect(documents[1]?.info.providers?.last).toBeInstanceOf(ConfigProvider.Info)

            yield* Effect.promise(() =>
              fs.writeFile(path.join(tmp.path, "nexus.jsonc"), JSON.stringify({ $schema: "changed" })),
            )
            expect(
    ...
    Effect.acquireRelease(
      ...
      Effect.flatMap((tmp) =>
        Effect.gen(function* () {
          yield* Effect.promise(() =>
            Promise.all([
              fs.writeFile(path.join(tmp.path, "nexus.json"), JSON.stringify({ $schema: "base" })),
              fs.writeFile(path.join(tmp.path, "nexus.jsonc"), "{ invalid"),
            ]),
          )
          return yield* Effect.gen(function* () {
            const config = yield* Config.Service
  ...
  it.live("loads global, ancestor, and .nexus configuration up to the project boundary", () =>
    Effect.acquireRelease(
      Effect.promise(() => tmpdir()),
      (tmp) => Effect.promise(() => tmp[Symbol.asyncDispose]()),
    ).pipe(
      Effect.flatMap((tmp) => {
        ...
        return Effect.gen(function* () {
          yield* Effect.promise(async () => {
            ...
            await fs.mkdir(path.join(directory, ".nexus"), { recursive: true })
            await Promise.all([
              fs.writeFile(path.join(tmp.path, "nexus.json"), JSON.stringify({ $schema: "outside" })),
              fs.writeFile(path.join(global, "nexus.json"), JSON.stringify({ $schema: "global" })),
              fs.writeFile(path.join(root, "nexus.json"), JSON.stringify({ $schema: "root" })),
              fs.writeFile(path.join(parent, "nexus.jsonc"), JSON.stringify({ $schema: "parent" })),
              fs.writeFile(path.join(directory, "nexus.json"), JSON.stringify({ $schema: "directory" })),
              fs.writeFile(path.join(root, ".nexus", "nexus.json"), JSON.stringify({ $schema: "root-dot" })),
              fs.writeFile(
                path.join(directory, ".nexus", "nexus.jsonc"),
          ...
          return yield* Effect.gen(function* () {
            ...
            expect(entries.filter((entry) => entry.type === "directory").map((entry) => entry.path)).toEqual([
              ...
              AbsolutePath.make(path.join(root, ".nexus")),
              AbsolutePath.make(path.join(directory, ".nexus")),
            ])
