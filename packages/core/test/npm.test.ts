...
describe("Npm.sanitize", () => {
  test("keeps normal scoped package specs unchanged", () => {
    expect(Npm.sanitize("@nexus/acme")).toBe("@nexus/acme")
    expect(Npm.sanitize("@nexus/acme@1.0.0")).toBe("@nexus/acme@1.0.0")
    expect(Npm.sanitize("prettier")).toBe("prettier")
  })

  test("handles git https specs", () => {
    const spec = "acme@git+https://github.com/nexus/acme.git"
    const expected = win ? "acme@git+https_//github.com/nexus/acme.git" : spec
