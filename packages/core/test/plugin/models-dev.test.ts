...
import { ModelsDevPlugin } from "@nexus-ai/core/plugin/models-dev"
...
describe("ModelsDevPlugin", () => {
  ...
  it.effect("registers key methods for providers with environment variables", () =>
    Effect.acquireUseRelease(
      Effect.sync(() => {
        const previous = {
          path: Flag.NEXUS_MODELS_PATH,
          disabled: Flag.NEXUS_DISABLE_MODELS_FETCH,
        }
        Flag.NEXUS_MODELS_PATH = path.join(import.meta.dir, "fixtures", "models-dev.json")
      ...
      (previous) =>
        Effect.sync(() => {
          Flag.NEXUS_MODELS_PATH = previous.path
          Flag.NEXUS_DISABLE_MODELS_FETCH = previous.disabled
        }),
    ),
  )
})
