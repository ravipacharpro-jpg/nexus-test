...
import { GoogleVertexPlugin } from "@nexus-ai/core/plugin/provider/google-vertex"
      ...
      yield* catalog.transform((catalog) =>
        catalog.provider.update(ProviderV2.ID.nexus, (provider) => {
          provider.api = {
            ...
            url: "https://nexus.ai/zen/v1",
          }
        }),
      )
      yield* addPlugin()

      const provider = required(yield* catalog.provider.get(ProviderV2.ID.nexus))
