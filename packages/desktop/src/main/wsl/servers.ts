...
import { expectNexusVersion, pendingRestartAfterWslInstall, wslServerIdsToStartOnInitialize } from "./startup"
...
type WslServersControllerOptions = {
  ...
  resolveNexus?: typeof resolveWslNexus
...
export function createWslServersController(
  ...
  const setNexusCheck = (distro: string, check: WslNexusCheck) => {
    setState({
      nexusChecks: {
        ...state.nexusChecks,
        [distro]: check,
  ...
  const checkNexus = async (distro: string, opts?: { signal?: AbortSignal }) => {
    const resolved = await (options?.resolveNexus ?? resolveWslNexus)(distro, opts)
    const version = resolved
      ? await (options?.readCommandVersion ?? readWslCommandVersion)(resolved, distro, opts)
      : null
    return nexusCheck(distro, resolved, version, appVersion)
  }

  const refreshNexusCheck = async (distro: string, opts?: { signal?: AbortSignal }) => {
    setNexusCheck(distro, await checkNexus(distro, opts))
  }
  ...
  const probeAddableDistros = async (distros: string[], opts?: { signal?: AbortSignal }) => {
    ...
    const nexusChecks = await Promise.all(
      unique
        .filter((distro) => distroProbeReady(state.distroProbes[distro]))
        .filter((distro) => !state.nexusChecks[distro])
        .map(async (distro) => [distro, await checkNexus(distro, opts)] as const),
    )
    if (nexusChecks.length) {
      setState({ nexusChecks: { ...state.nexusChecks, ...Object.fromEntries(nexusChecks) } })
    }
  }
  ...
  const refreshNexusCheckBackground = (id: string, distro: string) => {
    void checkNexus(distro)
      .then((check) => {
        if (!hasServer(id, distro)) return
        setNexusCheck(distro, check)
      })
      .catch((error) => {
        const message = error instanceof Error ? error.message : String(error)
        logger?.error("wsl nexus check failed", { id, distro, message })
      })
  }

  const refreshNexusChecks = async () => {
    await Promise.all(
      state.servers.map((item) =>
        checkNexus(item.config.distro)
          .then((check) => {
            if (!hasServer(item.config.id, item.config.distro)) return
            setNexusCheck(item.config.distro, check)
          })
          .catch((error) => {
            const message = error instanceof Error ? error.message : String(error)
            logger?.error("wsl nexus check failed", {
              id: item.config.id,
  ...
  const startServer = async (id: string) => {
    ...
    try {
      ...
      refreshNexusCheckBackground(id, item.config.distro)
  ...
  return {
    ...
    async installNexus(name: string) {
      await runJob({ kind: "install-nexus", distro: name, startedAt: Date.now() }, async (abort) => {
        const result = await installWslNexus(appVersion, name, { signal: abort.signal })
        if (result.code !== 0) {
          throw new Error(summarize(result.stderr || result.stdout) || nativeT("desktop.wsl.error.installNexus"))
        }
        await refreshNexusCheck(name, { signal: abort.signal })
        expectNexusVersion(state.nexusChecks[name]?.version ?? null, appVersion, name)
    ...
    async removeServer(id: string) {
      ...
      setState({
        ...
        ...(distro ? clearWslDistroState(state.distroProbes, state.nexusChecks, distro) : {}),
      })
    },
...
function nexusCheck(
  ...
  if (!resolvedPath) {
    return {
      ...
      expectedVersion,
      matchesDesktop: null,
      error: nativeT("desktop.wsl.error.nexusMissing"),
    }
  }
  if (!version) {
    return {
      ...
      expectedVersion,
      matchesDesktop: null,
      error: nativeT("desktop.wsl.error.nexusCannotRun"),
    }
  }
  return {
    distro,
