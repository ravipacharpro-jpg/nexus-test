...
import type { UpdaterState } from "@nexus-ai/app/updater"
...
import "./styles.css"
import { Splash } from "@nexus-ai/ui/logo"
import { useTheme } from "@nexus-ai/ui/theme/context"
    ...
    integrations: (integrations) => {
      return integrations.filter(
        (i) =>
          i.name !== "Breadcrumbs" &&
          !(
            import.meta.env.NEXUS_CHANNEL === "prod" &&
...
const deepLinkEvent = "nexus:deep-link"
...
const emitDeepLinks = (urls: string[]) => {
  ...
  window.__NEXUS__ ??= {}
  const pending = window.__NEXUS__.deepLinks ?? []
  window.__NEXUS__.deepLinks = [...pending, ...urls]
...
function windowLastActiveUrlKey(windowID: string) {
  return `nexus.desktop.window.${windowID}.last-active-url`
}
  ...
  return {
    ...
    notify: async (title, description, onClick) => {
      ...
      const notification = new Notification(title, {
        body: description ?? "",
        icon: "https://nexus.ai/favicon-96x96-v3.png",
      })
...
function DesktopRoot(props: { windowState: DesktopWindowState }) {
  ...
  const loadLocale = async () => {
    const current = await platform.storage?.("nexus.global.dat").getItem("language")
