...
import { lazy } from "@nexus-ai/core/util/lazy"
...
export namespace Storage {
  ...
  function s3(): Adapter {
    const bucket = process.env.NEXUS_STORAGE_BUCKET!
    const region = process.env.NEXUS_STORAGE_REGION || "us-east-1"
    const client = new AwsClient({
      region,
      accessKeyId: process.env.NEXUS_STORAGE_ACCESS_KEY_ID!,
      secretAccessKey: process.env.NEXUS_STORAGE_SECRET_ACCESS_KEY!,
    })
    return createAdapter(client, `https://s3.${region}.amazonaws.com`, bucket)
  }

  function r2() {
    const accountId = process.env.NEXUS_STORAGE_ACCOUNT_ID!
    const client = new AwsClient({
      accessKeyId: process.env.NEXUS_STORAGE_ACCESS_KEY_ID!,
      secretAccessKey: process.env.NEXUS_STORAGE_SECRET_ACCESS_KEY!,
    })
    return createAdapter(client, `https://${accountId}.r2.cloudflarestorage.com`, process.env.NEXUS_STORAGE_BUCKET!)
  }

  const adapter = lazy(() => {
    const type = process.env.NEXUS_STORAGE_ADAPTER
