...
> current private `@nexus-ai/llm` API. Names and exact TypeScript signatures
> are illustrative until implementation, but the domain boundaries and defaults
> are deliberate.

## Status

- Proposed package: `@nexus-ai/ai`
...
- Secondary audience: NEXUS and other durable agent runtimes
...
import { LLM } from "@nexus-ai/ai"
import { OpenAI } from "@nexus-ai/ai/providers/openai"
...
```ts
import { Message } from "@nexus-ai/ai"
...
import { LLM, Tool } from "@nexus-ai/ai"
...
NEXUS and other durable runtimes need to own persistence, tool settlement,
...
```ts
import { LLM } from "@nexus-ai/ai/promise"
import { OpenAI } from "@nexus-ai/ai/providers/openai"
...
```ts
import { LLMSchema } from "@nexus-ai/ai/schema"
...
```ts
import { Provider, Protocol } from "@nexus-ai/ai/provider"
...
@nexus-ai/ai/provider
  experimental Provider and Protocol authoring APIs

@nexus-ai/ai/providers/openai
@nexus-ai/ai/providers/anthropic
@nexus-ai/ai/providers/google
...
```
...
NEXUS should migrate to `generateTurn` / `streamTurn`, preserving its durable
