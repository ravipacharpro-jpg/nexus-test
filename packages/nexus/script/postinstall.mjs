...
const base = `nexus-${platform}-${arch}`
const sourceBinary = platform === "windows" ? "nexus.exe" : "nexus"
const targetBinary = path.join(__dirname, "bin", "nexus.exe")
...
function installPackage(name) {
  ...
  const temp = fs.mkdtempSync(path.join(os.tmpdir(), "nexus-install-"))
...
function verifyBinary() {
  if (os.platform() === "android") {
    ...
    // matching the official NEXUS behavior on Termux.
    return true
  }
...
function main() {
  ...
  if (os.platform() === "android") {
    console.warn("nexus-ai installed on Android/Termux. Native binary may require glibc-runner or PRoot to execute.");
    return
  }

  throw new Error(
    `It seems your package manager failed to install the right nexus CLI package. Try manually installing ${packageNames()
