...
Practical reference for a future `packages/server` split after the nexus
server moved to the Effect HttpApi backend.

## Current State

- The server still lives in `packages/nexus`.
...
Until enough shared service code lives outside `packages/nexus`, a future
`packages/server` should either:

- own pure HttpApi contracts only, or
- accept host-provided services/layers/callbacks from `packages/nexus`

It should not import `packages/nexus` services while `packages/nexus`
...
2. Move stable domain schemas into shared packages only when they no longer
   depend on nexus-local runtime modules.
3. Extract pure HttpApi contract modules into `packages/server` once the contract
   can compile without importing `packages/nexus` implementation details.
