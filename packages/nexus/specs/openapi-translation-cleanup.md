...
Trim `packages/nexus/src/server/routes/instance/httpapi/public.ts` until OpenAPI generation is mostly a direct projection of the `HttpApi` route declarations, without breaking the generated SDK surface.
...
- `[x]` Add or extend `packages/nexus/test/server/httpapi-query-schema-drift.test.ts`.
...
- `[x]` `bun test --timeout 5000 test/server/httpapi-query-schema-drift.test.ts` from `packages/nexus`.
- `[x]` `bun typecheck` from `packages/nexus`.

### PR 2: Delete Spec-Only Workspace Query Injection

- `[x]` Edit `packages/nexus/src/server/routes/instance/httpapi/public.ts`.
...
- `[x]` `bun test --timeout 5000 test/server/httpapi-query-schema-drift.test.ts` from `packages/nexus`.
- `[x]` `bun dev generate > /tmp/nexus-openapi.json` from `packages/nexus`.
...
- Edit `packages/nexus/src/server/routes/instance/httpapi/public.ts`.
...
- `bun dev generate > /tmp/nexus-openapi.json` from `packages/nexus`.
...
- Check source schemas in files like `packages/nexus/src/session/schema.ts`, `packages/nexus/src/permission/schema.ts`, and pty schema definitions.
...
- Edit route group files under `packages/nexus/src/server/routes/instance/httpapi/groups/`.
- Replace SDK-visible `HttpApiError.BadRequest` / `HttpApiError.NotFound` with explicit error schemas from `packages/nexus/src/server/routes/instance/httpapi/errors.ts` or add new ones there.
