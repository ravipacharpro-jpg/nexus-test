...
- Author package entrypoint is `@nexus-ai/plugin/tui`.
...
```json
{
  "$schema": "https://nexus.ai/tui.json",
  "theme": "smoke-theme",
  "leader_timeout": 2000,
  "keybinds": {
    "leader": "ctrl+x",
  ...
  "plugin": ["@acme/nexus-plugin@1.2.3", ["./plugins/demo.tsx", { "label": "demo" }]],
  "plugin_enabled": {
    "acme.demo": false
  },
  "attention": {
...
- Import types from `@nexus-ai/plugin/tui`.
- `@nexus-ai/plugin` exports `./tui` and declares optional peer deps on `@opentui/core` and `@opentui/solid`.

Minimal module shape:

```tsx
/** @jsxImportSource @opentui/solid */
import type { TuiPlugin, TuiPluginModule } from "@nexus-ai/plugin/tui"
...
- The value is a semver range checked against the running NEXUS version.
- If the range is not satisfied, the plugin is skipped with a warning and a session error.
- If `engines.nexus` is absent, no check is performed (backward compatible).
...
- `nexus plugin <module>` and TUI install both run install → manifest read → config patch.
- Alias: `nexus plug <module>`.
- `-g` / `--global` writes into the global config dir.
- Local installs resolve target dir inside `patchPluginConfig`.
- For local scope, path is `<worktree>/.nexus` only when VCS is git and `worktree !== "/"`; otherwise `<directory>/.nexus`.
- Root-worktree fallback (`worktree === "/"` uses `<directory>/.nexus`) is covered by regression tests.
...
When `plugin` entries exist in a writable `.nexus` dir or `NEXUS_CONFIG_DIR`, NEXUS installs `@nexus-ai/plugin` into that dir and writes:
...
That is what makes local config-scoped plugins able to import `@nexus-ai/plugin/tui`.
...
- The host already installs the default OpenTUI bundle (`default keys`, metadata fields, and enabled fields) plus NEXUS's comma bindings, leader token, base layout fallback, pending-sequence helpers, and managed textarea layer.
...
NEXUS registers a `mode` layer field on the host keymap. Plugins can use it to keep bindings active only in the relevant UI state.
...
- For generic config-to-bindings helpers, import `createBindingLookup` from `@nexus-ai/plugin/tui`.
...
- `message` is required; `title` defaults to `"nexus"`; `notification` defaults to enabled with `when: "blurred"`; `sound` defaults to enabled with `when: "always"`.
...
- Config `attention.sounds` overrides active-pack sounds by slot. Failed loads fall back to the active pack and then `nexus.default`.
...
- Local plugins persist installed themes under the local `.nexus/themes` area near the plugin config source.
...
- `--pure` / `NEXUS_PURE` skips external TUI plugins only.
...
- Local smoke plugin: `.nexus/plugins/tui-smoke.tsx`
- Local vim plugin: `.nexus/plugins/tui-vim.tsx`
- Local smoke config: `.nexus/tui.json`
- Local smoke theme: `.nexus/plugins/smoke-theme.json`
