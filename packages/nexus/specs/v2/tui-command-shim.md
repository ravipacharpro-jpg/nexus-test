...
Delete `packages/nexus/src/cli/cmd/tui/plugin/command-shim.ts`.

In `packages/nexus/src/cli/cmd/tui/plugin/api.tsx`, remove:

- the `createCommandShim` import
- the `command: createCommandShim(...)` field from `createTuiApi(...)`

In `packages/nexus/src/cli/cmd/tui/plugin/runtime.ts`, remove:
...
- `bun typecheck` in `packages/nexus`
- TUI plugin loader tests in `packages/nexus` if runtime plugin API wiring changed
