...
export const McpAuthCommand = effectCmd({
  ...
  handler: Effect.fn("Cli.mcp.auth")(function* (args) {
    ...
    if (servers.length === 0) {
      ...
      prompts.log.info("Remote MCP servers support OAuth by default. Add a remote server in nexus.json:")
      prompts.log.info(`
  "mcp": {
    "my-server": {
      "type": "remote",
...
async function resolveConfigPath(baseDir: string, global = false) {
  // Check for existing config files (prefer .jsonc over .json, check .nexus/ subdirectory too)
  const candidates = [path.join(baseDir, "nexus.json"), path.join(baseDir, "nexus.jsonc")]

  if (!global) {
    candidates.push(path.join(baseDir, ".nexus", "nexus.json"), path.join(baseDir, ".nexus", "nexus.jsonc"))
  }
  ...
  // Default to nexus.json if none exist
  return candidates[0]
}
        ...
        const response = await fetch(serverConfig.url, {
          ...
          body: JSON.stringify({
            ...
            params: {
              ...
              capabilities: {},
              clientInfo: { name: "nexus-debug", version: InstallationVersion },
            },
            id: 1,
          }),
        })
