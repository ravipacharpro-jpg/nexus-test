...
export const UninstallCommand = {
  command: "uninstall",
  describe: "uninstall NEXUS and remove all related files",
  builder: (yargs: Argv) =>
    yargs
      .option("keep-config", {
        alias: "c",
...
async function showRemovalSummary(targets: RemovalTargets, method: Installation.Method) {
  ...
  if (method !== "curl" && method !== "unknown") {
    const cmds: Record<string, string> = {
      npm: "npm uninstall -g nexus-ai",
      pnpm: "pnpm uninstall -g nexus-ai",
      bun: "bun remove -g nexus-ai",
      yarn: "yarn global remove nexus-ai",
      brew: "brew uninstall nexus",
      choco: "choco uninstall nexus",
      scoop: "scoop uninstall nexus",
    }
...
async function executeUninstall(method: Installation.Method, targets: RemovalTargets) {
  ...
  if (method !== "curl" && method !== "unknown") {
    ...
    if (cmd) {
      ...
      const result = await Process.run(method === "choco" ? ["choco", "uninstall", "nexus", "-y", "-r"] : cmd, {
        nothrow: true,
      })
      if (result.code !== 0) {
  ...
  UI.empty()
  prompts.log.success("Thank you for using NEXUS!")
}
...
async function getShellConfigFile(): Promise<string | null> {
  ...
  for (const file of candidates) {
    ...
    if (content.includes("# nexus") || content.includes(".nexus/bin")) {
      return file
    }
  }
...
async function cleanShellConfig(file: string) {
  ...
  for (const line of lines) {
    ...
    if (skip) {
      skip = false
      if (trimmed.includes(".nexus/bin") || trimmed.includes("fish_add_path")) {
        continue
      }
    }

    if (
      (trimmed.startsWith("export PATH=") && trimmed.includes(".nexus/bin")) ||
      (trimmed.startsWith("fish_add_path") && trimmed.includes(".nexus"))
    ) {
      continue
    }
