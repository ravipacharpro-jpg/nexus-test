...
const MANAGED_PLIST_DOMAIN = "ai.nexus.managed"

// Keys injected by macOS/MDM into the managed plist that are not NEXUS config
...
function systemManagedConfigDir(): string {
  switch (process.platform) {
    ...
    case "win32":
      return path.join(process.env.ProgramData || "C:\\ProgramData", "nexus")
    default:
      return "/etc/nexus"
  }
}
...
export function managedConfigDir() {
  return process.env.NEXUS_TEST_MANAGED_CONFIG_DIR || systemManagedConfigDir()
}
