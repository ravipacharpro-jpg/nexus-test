...
const TUI_SCHEMA_URL = "https://nexus.ai/tui.json"
...
/**
 * Migrates tui-specific keys (theme, keybinds, tui) from nexus.json files
 * into dedicated tui.json files. Migration is performed per-directory and
 * skips only locations where a tui.json already exists.
 */
export async function migrateTuiConfig(input: MigrateInput) {
  const nexus = await nexusFiles(input)
  for (const file of nexus) {
...
async function nexusFiles(input: { directories: string[]; cwd: string }) {
  const files = [
    ...ConfigPaths.fileInDirectory(Global.Path.config, "nexus"),
    ...(await Filesystem.findUp(["nexus.json", "nexus.jsonc"], input.cwd, undefined, { rootFirst: true })),
  ]
  for (const dir of unique(input.directories)) {
    files.push(...ConfigPaths.fileInDirectory(dir, "nexus"))
  }
  if (Flag.NEXUS_CONFIG) files.push(Flag.NEXUS_CONFIG)
