...
import { IdeEvent } from "@nexus-ai/schema/ide-event"
...
export function alreadyInstalled() {
  return process.env["NEXUS_CALLER"] === "vscode" || process.env["NEXUS_CALLER"] === "vscode-insiders"
}
...
export async function install(ide: (typeof SUPPORTED_IDES)[number]["name"]) {
  ...
  const p = await Process.run([cmd, "--install-extension", "sst-dev.nexus"], {
    nothrow: true,
  })
