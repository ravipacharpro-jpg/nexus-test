...
export const DEPRECATED_PLUGIN_PACKAGES = ["nexus-openai-codex-auth", "nexus-copilot-auth"]
...
export async function checkPluginCompatibility(target: string, nexusVersion: string, pkg?: PluginPackage) {
  if (!semver.valid(nexusVersion) || semver.major(nexusVersion) === 0) return
  const hit = pkg ?? (await readPluginPackage(target).catch(() => undefined))
  if (!hit) return
  const engines = hit.json.engines
  if (!isRecord(engines)) return
  const range = engines.nexus
  if (typeof range !== "string") return
  if (!semver.satisfies(nexusVersion, range)) {
    throw new Error(`Plugin requires NEXUS ${range} but running ${nexusVersion}`)
  }
}
