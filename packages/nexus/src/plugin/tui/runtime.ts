...
import {
  ...
  type TuiSlotPlugin,
  type TuiTheme,
} from "@nexus-ai/plugin/tui"
import path from "path"
import { fileURLToPath } from "url"
import { TuiConfig } from "@/config/tui"
import { AppNodeBuilder } from "@nexus-ai/core/effect/app-node-builder"
import { errorData, errorMessage } from "@nexus-ai/tui/util/error"
...
import { hasTheme, upsertTheme } from "@nexus-ai/tui/context/theme"
import { Global } from "@nexus-ai/core/global"
import { Filesystem } from "@/util/filesystem"
import { Process } from "@/util/process"
import { Flock } from "@nexus-ai/core/util/flock"
import { Flag } from "@nexus-ai/core/flag/flag"
import { internalTuiPlugins, type InternalTuiPlugin } from "./internal"
import type { HostPluginApi, HostSlots } from "@nexus-ai/tui/plugin/slots"
import { ConfigPlugin } from "@/config/plugin"
import { ConfigPluginV1 } from "@nexus-ai/core/v1/config/plugin"
import { createCommandShim } from "@nexus-ai/tui/plugin/command-shim"
import { RuntimeFlags } from "@/effect/runtime-flags"
import { Effect } from "effect"
import { createPluginRuntime, type PluginRuntime, type TuiPluginHost } from "@nexus-ai/tui/plugin/runtime"
...
function createThemeInstaller(
  ...
  return async (file) => {
    ...
    const local_dir =
      path.basename(source_dir) === ".nexus"
        ? path.join(source_dir, "themes")
        : path.join(source_dir, ".nexus", "themes")
...
function defaultPluginOrigin(state: RuntimeState, spec: string): ConfigPlugin.Origin {
  return {
    spec,
    scope: "local",
    source: state.api.state.path.config || path.join(state.directory, ".nexus", "tui.json"),
  }
}
...
async function load(input: {
  ...
  try {
    ...
    const records = Flag.NEXUS_PURE ? [] : pluginOrigins
    if (Flag.NEXUS_PURE && pluginOrigins.length) {
    }
