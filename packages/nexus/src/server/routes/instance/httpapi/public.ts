...
const LegacyComponentDescriptions: Record<string, string> = {
  LogLevel: "Log level",
  ServerConfig: "Server configuration for nexus serve and web commands",
...
function fixSelfReferencingComponents(spec: OpenApiSpec) {
  ...
  const raw: OpenApiSpec = OpenApi.fromApi(NEXUSHttpApi)
...
export const PublicApi = NEXUSHttpApi.annotateMerge(
  OpenApi.annotations({
    title: "NEXUS",
    version: "1.0.0",
