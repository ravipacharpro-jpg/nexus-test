...
import type { LLMClientShape } from "@nexus-ai/llm/route"
...
function statusWithFetch(
  ...
  if (providerID !== "openai" && providerID !== "anthropic" && !providerID.startsWith("nexus"))
    return { type: "unsupported", reason: "provider is not openai, nexus, or anthropic" }
...
export function stream(input: StreamInput): StreamResult {
  ...
  // Integration point with @nexus-ai/llm: native-request lowers session data
...
export function nativeTools(tools: Record<string, Tool>, input: Pick<StreamInput, "messages" | "abort">) {
  return Object.fromEntries(
    Object.entries(tools).map(([name, item]) => [
      name,
      // Tool execution remains nexus-owned. The native runtime only adapts
      // the @nexus-ai/llm tool call back into the AI SDK Tool.execute shape.
      NativeTool.make({
