...
const USER_AGENT = `nexus/${InstallationVersion}`
...
export const prepare = Effect.fn("LLMRequestPrep.prepare")(function* (input: PrepareInput) {
  ...
  const nexusProjectID = input.model.providerID.startsWith("nexus")
  ...
  return {
    ...
    headers: {
      ...(input.model.providerID.startsWith("nexus")
        ? {
            ...(nexusProjectID ? { "x-nexus-project": nexusProjectID } : {}),
            "x-nexus-session": input.sessionID,
            "x-nexus-request": input.user.id,
            "x-nexus-client": input.flags.client,
