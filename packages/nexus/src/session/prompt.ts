...
function formatMasterTaskResult(task: MasterTask) {
  ...
  // NEXUS Master Agent silent mode (Termux / capability-gap friendly):
  // blocked steps are normal when the user's device lacks a specific adapter
  // (e.g. web/android). Surface the completed count prominently; collapse
  // blocked steps into a single low-emphasis line so the user is not
  // distracted by every capability gap.
  const blockedStepTitles = task.steps
    .filter((step) => step.status === "blocked")
    .map((step) => step.kind)
  const lines = [
    `Master: ${completed}/${task.steps.length} done${failed ? `, ${failed} failed` : ""}.`,
    ...(task.activeStepID ? [`Active: ${task.activeStepID}`] : []),
    ...(task.status === "awaiting_approval"
      ? ["Approval required before the next sensitive or irreversible action."]
      : []),
    ...(task.queuedInstructions.length ? [`${task.queuedInstructions.length} queued`] : []),
    ...(blocked
      ? [`${blocked} step(s) skipped (no adapter on this device: ${blockedStepTitles.join(", ")}).`]
      : []),
  ]
  for (const step of task.steps) {
    if (step.status === "blocked") {
      // Silent mode: blocked steps are not surfaced in the per-step list.
      // The summary line above already tells the user what was skipped.
      continue
    }
  ...
  if (task.error && task.status !== "blocked") lines.push(`Note: ${truncateMasterStatus(task.error)}`)
  return lines.join("\n")
}

/**
 * Manus-style silent reply: a single short sentence the user actually wants
 * to read, instead of a multi-step orchestration log. Only the summary
 * numbers stay; every per-step line is dropped. The full task JSON is
 * still persisted to disk for debugging — it just never reaches the chat.
 */
function formatMasterTaskResultSilent(task: MasterTask): string {
  const completed = task.steps.filter((s) => s.status === "completed").length
  const blocked = task.steps.filter((s) => s.status === "blocked").length
  const failed = task.steps.filter((s) => s.status === "failed").length
  const total = task.steps.length
  // Queue is fully removed — a cancelled task means the user submitted a
  // new instruction. Surface the hand-off explicitly so the running
  // session shell can pick it up and start a fresh session.
  if (task.status === "cancelled" && task.error?.startsWith("User submitted a new instruction")) {
    return "Handing off to a fresh session so you are not held up."
  }
  if (task.status === "completed" && failed === 0) {
    return `Done — ${completed} of ${total} steps completed cleanly.`
  }
  if (failed > 0) {
    return `Partially done — ${completed} of ${total} steps completed, ${failed} failed.`
  }
  if (blocked > 0 && completed === 0) {
    return `Skipped — this device cannot run ${blocked} of the ${total} steps (no adapter). Try the action from a desktop browser.`
  }
  return `Done — ${completed} of ${total} steps completed${blocked ? `, ${blocked} skipped` : ""}${failed ? `, ${failed} failed` : ""}.`
}
  ...
  Effect.gen(function* () {
    ...
    const dispatchMasterTask = Effect.fn("SessionPrompt.dispatchMasterTask")(function* (input: {
      ...
      if (Exit.isSuccess(exit)) {
        ...
        // NEXUS Master Agent silent mode (default ON, Manus-style):
        // The user does not want to see the multi-step orchestration log
        // (queued instructions, blocked workers, etc.) for every routine
        // task. They want a single short reply that looks like the
        // assistant itself answered. Set NEXUS_SILENT_MASTER=0 to restore
        // the verbose orchestration log.
        const silent = process.env.NEXUS_SILENT_MASTER !== "0"
        const text = silent
          ? formatMasterTaskResultSilent(task)
          : formatMasterTaskResult(task)
    ...
    const prompt: (input: PromptInput) => Effect.Effect<SessionV1.WithParts, Image.Error> = Effect.fn(
      ...
      if (input.agent === "master" && input.noReply !== true && isAutonomousMasterObjective(objective)) {
        // NEXUS_NO_MASTER=1 (default) — skip the multi-worker orchestration
        // and fall through to the normal session loop. This keeps the chat
        // a direct, hand-to-hand conversation without 'web: blocked' or
        // 'queued: N step(s) blocked' banners. The MasterAgent.run() stub
        // still runs (so the chat pipeline stays unchanged), but since we
        // don't dispatch from here, the stub is never invoked.
