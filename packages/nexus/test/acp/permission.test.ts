...
import type { Event, NexusClient } from "@nexus-ai/sdk/v2"
...
type PermissionReplyParams = Parameters<NexusClient["permission"]["reply"]>[0]
...
async function tempFile(name: string, content: string) {
  const dir = await mkdtemp(path.join(tmpdir(), "nexus-acp-permission-"))
