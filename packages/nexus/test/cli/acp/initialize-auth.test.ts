...
describe("nexus acp initialize/auth subprocess", () => {
  cliIt.live(
    "initialize responds with capabilities",
    ({ nexus }) =>
      Effect.gen(function* () {
        const initialized = yield* initialize(yield* createAcpClient({ nexus }))
        ...
        expect(initialized.agentInfo?.name).toBe("NEXUS")
      }),
    60_000,
  )
  ...
  cliIt.live(
    ...
    ({ nexus }) =>
      Effect.gen(function* () {
        const acp = yield* createAcpClient({ nexus })
        const initialized = yield* initialize(acp)

        expect(initialized.authMethods?.[0]?.id).toBe("nexus-login")
        expect(initialized.authMethods?.[0]?._meta?.["terminal-auth"]).toBeDefined()
        expect(yield* acp.request<AuthenticateResponse>("authenticate", { methodId: "nexus-login" })).toMatchObject({
          result: {},
        })
        ...
        expect(JSON.stringify(rejected.error)).not.toContain(process.env.NEXUS_AUTH_CONTENT ?? "not-present")
      }),
    60_000,
  )
  ...
  cliIt.live(
    ...
    ({ nexus }) =>
      Effect.gen(function* () {
        ...
        expect(initialized.result?.authMethods?.[0]?.id).toBe("nexus-login")
