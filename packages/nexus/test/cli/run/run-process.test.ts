// Subprocess integration tests for `nexus run` (non-interactive mode).
// These exercise the real CLI binary against a TestLLMServer running in the
// same process. See `test/lib/cli-process.ts` for the harness — each test uses
// `nexus.run(message, opts?)` to spawn `bun src/index.ts run ...` with
// `NEXUS_CONFIG_CONTENT` providing the test provider config inline.
import { describe, expect } from "bun:test"
import { Effect } from "effect"
import { reply } from "../../lib/llm-server"
import { cliIt } from "../../lib/cli-process"

describe("nexus run (non-interactive subprocess)", () => {
  ...
  cliIt.concurrent(
    ...
    ({ llm, nexus }) =>
      Effect.gen(function* () {
        ...
        const result = yield* nexus.run("say hi")
        nexus.expectExit(result, 0)
  ...
  cliIt.concurrent(
    ...
    ({ llm, nexus }) =>
      Effect.gen(function* () {
        ...
        const result = yield* nexus.run("use a tool", {
  ...
  cliIt.concurrent(
    ...
    ({ llm, nexus }) =>
      Effect.gen(function* () {
        ...
        const thinking = yield* nexus.run("think", { extraArgs: ["--thinking"] })
        nexus.expectExit(thinking, 0)
        expect(thinking.stdout).toBe("Thinking: considering\nanswer\n")

        yield* llm.reason("hidden", { text: "visible" })
        const plain = yield* nexus.run("think again")
        nexus.expectExit(plain, 0)
  ...
  cliIt.concurrent(
    ...
    ({ llm, nexus }) =>
      Effect.gen(function* () {
        ...
        const result = yield* nexus.run("trigger midstream error", { timeoutMs: 30_000 })
  ...
  cliIt.concurrent(
    ...
    ({ llm, nexus }) =>
      Effect.gen(function* () {
        ...
        const result = yield* nexus.run("say hi", { format: "json" })
        nexus.expectExit(result, 0)

        const events = nexus.parseJsonEvents(result.stdout)
  ...
  cliIt.concurrent(
    ...
    ({ nexus }) =>
      Effect.gen(function* () {
        const result = yield* nexus.run("use an unknown model", {
          model: "test/nonexistent-model",
          format: "json",
        })
  ...
  cliIt.concurrent(
    ...
    ({ llm, nexus }) =>
      Effect.gen(function* () {
        ...
        const result = yield* nexus.run("exercise json records", {
          format: "json",
  ...
  cliIt.concurrent(
    ...
    ({ llm, nexus }) =>
      Effect.gen(function* () {
        ...
        const result = yield* nexus.run("fail after output", { format: "json" })
  ...
  cliIt.concurrent(
    ...
    ({ home, llm, nexus }) =>
      Effect.gen(function* () {
        ...
        const denied = yield* nexus.run("request permission", { permission: { bash: "ask" } })
        nexus.expectExit(denied, 0)
        ...
        const allowed = yield* nexus.run("request permission", {
          permission: { bash: "ask" },
          extraArgs: ["--dangerously-skip-permissions"],
        })
        nexus.expectExit(allowed, 0)
        ...
        const explicitlyDenied = yield* nexus.run("request denied permission", {
          permission: { bash: "deny" },
          extraArgs: ["--dangerously-skip-permissions"],
        })
        nexus.expectExit(explicitlyDenied, 0)
  ...
  cliIt.concurrent(
    ...
    ({ llm, nexus }) =>
      Effect.gen(function* () {
        ...
        const result = yield* nexus.run("delegate a command", {
          permission: { bash: "ask" },
          extraArgs: ["--dangerously-skip-permissions"],
          timeoutMs: 10_000,
        })
        ...
        const rejected = yield* nexus.run("delegate a rejected command", {
          permission: { bash: "ask" },
          timeoutMs: 10_000,
        })

        nexus.expectExit(rejected, 0)
  ...
  cliIt.live(
    ...
    ({ home, llm, nexus }) =>
      Effect.gen(function* () {
        ...
        const server = yield* nexus.serve()

        const result = yield* nexus.run("read the attachment", {
  ...
  cliIt.concurrent(
    ...
    ({ home, nexus }) =>
      Effect.gen(function* () {
        const result = yield* nexus.run("read the directory", {
  ...
  cliIt.live(
    ...
    ({ llm, nexus }) =>
      Effect.gen(function* () {
        yield* llm.hang
        const run = yield* nexus.startRun("wait forever")
        yield* llm.wait(1)
        run.interrupt()
