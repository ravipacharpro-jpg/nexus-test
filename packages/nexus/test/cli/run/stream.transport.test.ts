...
import { NexusClient, type GlobalEvent } from "@nexus-ai/sdk/v2"
import { createSessionTransport } from "@/cli/cmd/run/stream.transport"
import type { FooterApi, FooterEvent, LocalReplayRow, RunFilePart, StreamCommit } from "@/cli/cmd/run/types"

type EventStream = Awaited<ReturnType<NexusClient["event"]["subscribe"]>>["stream"]
type GlobalEventStream = Awaited<ReturnType<NexusClient["global"]["event"]>>["stream"]
type SdkEvent = EventStream extends AsyncGenerator<infer T, unknown, unknown> ? T : never
type SessionMessage = NonNullable<Awaited<ReturnType<NexusClient["session"]["messages"]>>["data"]>[number]
type SessionChild = NonNullable<Awaited<ReturnType<NexusClient["session"]["children"]>>["data"]>[number]
type SessionToolPart = Extract<SessionMessage["parts"][number], { type: "tool" }>
type SessionStatusMap = NonNullable<Awaited<ReturnType<NexusClient["session"]["status"]>>["data"]>
...
function ok<T>(data: T) {
  return Promise.resolve({
    data,
    error: undefined,
    request: new Request("https://nexus.test"),
...
function sdk(
  input: {
    ...
    permissions?: NexusClient["permission"]["list"]
    questions?: NexusClient["question"]["list"]
  } = {},
) {
  const client = new NexusClient()

  const subscribe: NexusClient["event"]["subscribe"] = input.subscribe ?? (() => sse(input.stream ?? emptyStream()))
  ...
  const permissions: NexusClient["permission"]["list"] = input.permissions ?? (() => ok([]))
  const questions: NexusClient["question"]["list"] = input.questions ?? (() => ok([]))
