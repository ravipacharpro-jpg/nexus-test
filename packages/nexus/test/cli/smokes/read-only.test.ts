...
describe("nexus read-only commands (smoke)", () => {
  // `mcp list` reads MCP server config and pings each one. With the empty
  // NEXUS_CONFIG_CONTENT={} we provide, no servers should be configured
  // and the command should report that cleanly.
  cliIt.live(
    "mcp list: exits 0",
    ({ nexus }) =>
      Effect.gen(function* () {
        const r = yield* nexus.spawn(["mcp", "list"])
        nexus.expectExit(r, 0, "mcp list")
      }),
    60_000,
  )
  ...
  cliIt.live(
    ...
    ({ nexus }) =>
      Effect.gen(function* () {
        const r = yield* nexus.spawn(["providers", "list"])
        nexus.expectExit(r, 0, "providers list")
  ...
  cliIt.live(
    ...
    ({ nexus }) =>
      Effect.gen(function* () {
        const r = yield* nexus.spawn(["models"])
        nexus.expectExit(r, 0, "models")
  ...
  cliIt.live(
    ...
    ({ nexus }) =>
      Effect.gen(function* () {
        const r = yield* nexus.spawn(["agent", "list"])
        nexus.expectExit(r, 0, "agent list")
      }),
    60_000,
  )

  // `session list` reads the session DB. Fresh NEXUS_TEST_HOME means
  // empty DB. Exit 0 with no sessions.
  cliIt.live(
    "session list: exits 0",
    ({ nexus }) =>
      Effect.gen(function* () {
        const r = yield* nexus.spawn(["session", "list"])
        nexus.expectExit(r, 0, "session list")
      }),
    60_000,
  )
  ...
  cliIt.live(
    ...
    ({ nexus }) =>
      Effect.gen(function* () {
        const r = yield* nexus.spawn(["stats"])
        nexus.expectExit(r, 0, "stats")
      }),
    60_000,
  )
  ...
  // in production it'd be a path under NEXUS_TEST_HOME / XDG_DATA_HOME.
  // Accept either form — both prove the resolver ran without crashing.
  cliIt.live(
    "db path: exits 0 and prints a path or :memory:",
    ({ nexus }) =>
      Effect.gen(function* () {
        const r = yield* nexus.spawn(["db", "path"])
        nexus.expectExit(r, 0, "db path")
