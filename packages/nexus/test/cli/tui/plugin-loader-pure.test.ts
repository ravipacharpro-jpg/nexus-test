...
`,
  ...
  const pure = process.env.NEXUS_PURE
  const meta = process.env.NEXUS_PLUGIN_META_FILE
  process.env.NEXUS_PURE = "1"
  process.env.NEXUS_PLUGIN_META_FILE = tmp.extra.meta
  ...
  try {
    ...
    wait.mockRestore()
    if (pure === undefined) {
      delete process.env.NEXUS_PURE
    } else {
      process.env.NEXUS_PURE = pure
    }
    if (meta === undefined) {
      delete process.env.NEXUS_PLUGIN_META_FILE
    } else {
      process.env.NEXUS_PLUGIN_META_FILE = meta
    }
  }
})
