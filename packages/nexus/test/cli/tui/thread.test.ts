...
describe("tui thread", () => {
  ...
  cliIt.live("rejects mini-only options without --mini", ({ nexus }) =>
    Effect.gen(function* () {
      const result = yield* nexus.spawn(["--replay-limit", "10"])

      nexus.expectExit(result, 1)
      expect(result.stderr).toContain("--replay-limit requires --mini")
    }),
  )

  cliIt.live("routes attached sessions to mini mode", ({ nexus }) =>
    Effect.gen(function* () {
      const result = yield* nexus.spawn(["attach", "http://127.0.0.1:1", "--mini"])
  ...
  cliIt.live("rejects network options in mini mode", ({ nexus }) =>
    Effect.gen(function* () {
      const result = yield* nexus.spawn(["--mini", "--port", "4096"])
