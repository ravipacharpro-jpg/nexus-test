// Subprocess test harness for the nexus CLI. Spawns the real binary against
...
//
// Configuration flows through nexus's built-in test affordances:
//   - NEXUS_CONFIG_CONTENT      : provider config inline, no files to find
//   - NEXUS_TEST_HOME           : pins os.homedir() → tmpdir
//   - NEXUS_DISABLE_PROJECT_CONFIG : skip walking up for nexus.json
//   - NEXUS_PURE                : skip external plugin discovery + install
//   - NEXUS_DISABLE_AUTOUPDATE / AUTOCOMPACT / MODELS_FETCH : no background work
// Plus HOME / XDG_* pointing at the tmpdir for belt-and-suspenders isolation.
//
// Today only `nexus.run` is fully wired. The shape supports adding more
// builders (`nexus.serve(opts)`, `nexus.acp(opts)`, `nexus.auth(...)`)
// without changing the fixture. Long-lived commands like `serve` will need a
// different return shape — see the TODO at the bottom of NexusCli.
...
const nexusRoot = path.resolve(import.meta.dir, "../../")
const cliEntry = path.join(nexusRoot, "src/index.ts")
...
function isolatedEnv(home: string, configJson: string): Record<string, string> {
  return {
    NEXUS_TEST_HOME: home,
    HOME: home,
    XDG_CONFIG_HOME: path.join(home, ".config"),
    XDG_DATA_HOME: path.join(home, ".local/share"),
    XDG_STATE_HOME: path.join(home, ".local/state"),
    XDG_CACHE_HOME: path.join(home, ".cache"),
    NEXUS_CONFIG_CONTENT: configJson,
    NEXUS_DISABLE_PROJECT_CONFIG: "1",
    NEXUS_PURE: "1",
    NEXUS_DISABLE_AUTOUPDATE: "1",
    NEXUS_DISABLE_AUTOCOMPACT: "1",
    NEXUS_DISABLE_MODELS_FETCH: "1",
    NEXUS_AUTH_CONTENT: "{}",
  }
}
...
// Typed equivalent of constructing argv for `nexus run`. New flags should
...
// `nexus serve` is a long-lived process — it never exits on its own.
...
// `nexus acp` speaks newline-delimited JSON-RPC over stdin/stdout. It is
...
export type NexusCli = {
  ...
  // Spawn `nexus serve` and wait until it's listening. Long-lived: the
  ...
  // Spawn `nexus acp` and return a duplex JSON-RPC handle. Long-lived:
...
export function withCliFixture<A, E>(
  ...
  return Effect.gen(function* () {
    ...
    const spawn = Effect.fn("nexus.spawn")(function* (args: string[], opts?: SpawnOpts) {
    ...
    const runOpts = (opts?: RunOpts): SpawnOpts | undefined => {
      ...
      return {
        ...
        env: {
          ...opts.env,
          NEXUS_CONFIG_CONTENT: JSON.stringify({
    ...
    const startRun = Effect.fn("nexus.startRun")(function* (message: string, opts?: RunOpts) {
      const start = Date.now()
    ...
    const serve = Effect.fn("nexus.serve")(function* (opts?: ServeOpts) {
      const argv = ["serve"]
      ...
      //   "nexus server listening on http://<host>:<port>"
      ...
      const match = yield* Deferred.await(readyDeferred).pipe(
        Effect.timeoutOrElse({
          ...
          orElse: () =>
            Effect.fail(
              new Error(
                `nexus serve did not become ready within ${readyTimeoutMs}ms\n` +
    ...
    const acp = Effect.fn("nexus.acp")(function* (opts?: AcpOpts) {
      const argv = ["acp"]
    ...
    const nexus: NexusCli = { run, startRun, serve, acp, spawn, expectExit, parseJsonEvents }

    return yield* fn({ llm, home, nexus })
    // FetchHttpClient is provided so test bodies can `yield* HttpClient.HttpClient`
    // and hit endpoints on `nexus.serve()` without rolling their own fetch.
  }).pipe(
    Effect.provide(
      Layer.mergeAll(
        TestLLMServer.layer,
...
function expectExit(result: RunResult, expected: number, label = "nexus") {
...
// every call site. Use this for any test that needs the nexus CLI fixture.
//
...
// resources (e.g. `nexus.serve`) without an extra `Effect.scoped` wrapper —
