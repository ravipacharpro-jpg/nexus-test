...
function requestServer(path: string, directory: string, init: RequestInit = {}) {
  ...
  headers.set("x-nexus-directory", directory)
      ...
      try {
        const response = yield* requestDefault(url.toString(), dir, {
          ...
          headers: {
            "accept-encoding": "br",
            "content-type": "application/json",
            "x-nexus-workspace": "internal",
          },
          body: JSON.stringify({ $schema: "https://nexus.ai/config.json" }),
        })
        ...
        expect(forwarded[0]?.headers).not.toHaveProperty("x-nexus-directory")
        expect(forwarded[0]?.headers).not.toHaveProperty("x-nexus-workspace")
