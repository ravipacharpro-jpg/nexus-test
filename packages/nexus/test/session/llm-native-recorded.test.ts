...
import { HttpRecorder } from "@nexus-ai/http-recorder"
import { HttpRecorderInternal } from "@nexus-ai/http-recorder/internal"
...
import { LLMEvent, LLMResponse } from "@nexus-ai/llm"
import { RequestExecutor } from "@nexus-ai/llm/route"
...
const zenURL = (connection: string) => `https://console.nexus.ai/proxy/connections/${connection}/v1`
...
function decodeRecordOpenAIOAuth() {
  const value = process.env.NEXUS_RECORD_OPENAI_AUTH
...
const RECORDED_SCENARIOS = [
  {
    ...
    tags: ["nexus", "native", "tool-loop"],
    canRecord: () => Boolean(envValue("NEXUS_RECORD_OPENAI_API_KEY", "OPENAI_API_KEY")),
    config: (model) =>
      providerConfig({
        providerID: ProviderV2.ID.openai,
        name: "OpenAI",
        ...
        model,
        options: {
          apiKey: envValue("NEXUS_RECORD_OPENAI_API_KEY", "OPENAI_API_KEY") ?? "fixture-openai-key",
  ...
  },
  {
    ...
    protocol: "openai-responses",
    tags: ["nexus", "native", "oauth", "tool-loop"],
  ...
  },
  {
    id: "nexus-proxy",
    name: "NEXUS proxy",
    providerID: ProviderV2.ID.nexus,
    modelID: "gpt-5.2-codex",
    cassette: "session/native-zen-tool-loop",
    protocol: "openai-responses",
    tags: ["nexus", "zen", "native", "tool-loop"],
    canRecord: () => Boolean(process.env.NEXUS_RECORD_CONSOLE_TOKEN && process.env.NEXUS_RECORD_ZEN_ORG_ID),
    config: (model) =>
      providerConfig({
        providerID: ProviderV2.ID.nexus,
        name: "NEXUS Zen",
        env: ["NEXUS_CONSOLE_TOKEN"],
        npm: "@ai-sdk/openai-compatible",
        api: zenURL(process.env.NEXUS_RECORD_ZEN_CONNECTION ?? "fixture"),
        model,
        options: {
          apiKey: process.env.NEXUS_RECORD_CONSOLE_TOKEN ?? "fixture-console-token",
          headers: { "x-org-id": process.env.NEXUS_RECORD_ZEN_ORG_ID ?? "fixture-org" },
        },
      }),
  },
  {
    ...
    tags: ["nexus", "native", "tool-loop"],
    canRecord: () => Boolean(envValue("NEXUS_RECORD_ANTHROPIC_API_KEY", "ANTHROPIC_API_KEY")),
    config: (model) =>
      providerConfig({
        providerID: ProviderV2.ID.anthropic,
        name: "Anthropic",
        ...
        model,
        options: {
          apiKey: envValue("NEXUS_RECORD_ANTHROPIC_API_KEY", "ANTHROPIC_API_KEY") ?? "fixture-anthropic-key",
...
const selectedScenarios = new Set(
  (envValue("NEXUS_RECORDED_SCENARIO", "RECORDED_PROVIDER") ?? "")
    .split(",")
...
const recordError = (scenario: RecordedScenario) =>
  scenario.id === "openai-oauth"
    ? "Set NEXUS_RECORD_OPENAI_AUTH to an OAuth auth JSON object in the recording environment."
...
function recordedNativeLLMLayer(scenario: RecordedScenario) {
  ...
  // Only the HTTP client is recorded; RequestExecutor and the nexus LLM stack remain real.
  const metadata = {
...
const writeConfig = (directory: string, scenario: RecordedScenario, model: ModelsDev.Provider["models"][string]) =>
  Effect.promise(() =>
    Bun.write(
      path.join(directory, "nexus.json"),
      JSON.stringify({ $schema: "https://nexus.ai/config.json", ...scenario.config(model) }),
    ),
  )
