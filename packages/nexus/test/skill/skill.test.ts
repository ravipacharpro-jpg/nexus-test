...
const withHome = <A, E, R>(home: string, self: Effect.Effect<A, E, R>) =>
  Effect.acquireUseRelease(
    Effect.sync(() => {
      const prev = process.env.NEXUS_TEST_HOME
    ...
    (prev) =>
      Effect.sync(() => {
        process.env.NEXUS_TEST_HOME = prev
      }),
  )
    ...
    provideTmpdirInstance(
      (dir) =>
        Effect.gen(function* () {
          yield* Effect.promise(() =>
            Bun.write(
              path.join(dir, ".nexus", "skill", "manual-skill", "SKILL.md"),
              `---
name: manual-skill
---
...
`,
              ...
              Bun.write(
                path.join(dir, ".nexus", "skill", "nexus-skill", "SKILL.md"),
                `---
name: nexus-skill
...
`,
          ...
          const skill = yield* Skill.Service
          const list = (yield* skill.all()).filter((s) => s.location !== "<built-in>")
          expect(list.map((s) => s.name)).toEqual(["nexus-skill"])
        }),
      { git: true },
    ),
  )
