...
describe("filesystem", () => {
  ...
  describe("findUp()", () => {
    ...
    test("rootFirst preserves json then jsonc order per directory", async () => {
      ...
      await fs.writeFile(path.join(tmp.path, "nexus.json"), "{}", "utf-8")
      await fs.writeFile(path.join(tmp.path, "nexus.jsonc"), "{}", "utf-8")
      await fs.writeFile(path.join(project, "nexus.json"), "{}", "utf-8")
      await fs.writeFile(path.join(project, "nexus.jsonc"), "{}", "utf-8")

      const result = await Filesystem.findUp(["nexus.json", "nexus.jsonc"], nested, tmp.path, {
        rootFirst: true,
      })
      ...
      expect(result).toEqual([
        ...
        path.join(project, "nexus.json"),
        path.join(project, "nexus.jsonc"),
      ])
    })
  })
