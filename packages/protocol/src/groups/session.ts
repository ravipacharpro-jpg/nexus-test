...
import { PromptInput } from "@nexus-ai/schema/prompt-input"
import { Session } from "@nexus-ai/schema/session"
import { Project } from "@nexus-ai/schema/project"
import { AbsolutePath, NonNegativeInt, PositiveInt, RelativePath, statics } from "@nexus-ai/schema/schema"
...
import { SessionEvent } from "@nexus-ai/schema/session-event"
    ...
    .add(
      HttpApiEndpoint.get("session.active", "/api/session/active", {
        ...
        OpenApi.annotations({
          ...
          description:
            "Retrieve foreground Session drains currently owned by this NEXUS process. Sessions absent from the result are inactive.",
        }),
      ),
    )
    .add(
      ...
      HttpApiEndpoint.post("session.interrupt", "/api/session/:sessionID/interrupt", {
        ...
        .annotateMerge(
          OpenApi.annotations({
            ...
            description: "Interrupt active execution owned by this NEXUS process. Idle interruption is a no-op.",
          }),
        ),
    )
    .add(
