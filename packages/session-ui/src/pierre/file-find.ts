...
function clearHighlightFind() {
  ...
  if (!api) return
  api.delete("nexus-find")
  api.delete("nexus-find-current")
}
...
export function createFileFind(opts: CreateFileFindOptions) {
  ...
  const setHighlights = (ranges: Range[], currentIndex: number) => {
    ...
    if (active) api.set("nexus-find-current", new Highlight(active))

    const rest = ranges.filter((_, i) => i !== currentIndex)
    if (rest.length > 0) api.set("nexus-find", new Highlight(...rest))
    return true
  }
