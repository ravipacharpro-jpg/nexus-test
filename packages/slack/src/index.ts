...
import { createNexus, type ToolPart } from "@nexus-ai/sdk"
...
console.log("✅ Nexus server ready")

const sessions = new Map<string, { client: any; server: any; sessionId: string; channel: string; thread: string }>()
void (async () => {
  const events = await nexus.client.event.subscribe()
...
app.message(async ({ message, say }) => {
  ...
  if (!session) {
    console.log("🆕 Creating new nexus session...")
    const { client, server } = nexus
    ...
    console.log("✅ Created nexus session:", createResult.data.id)
  ...
  console.log("📝 Sending to nexus:", message.text)
  const result = await session.client.session.prompt({
    path: { id: session.sessionId },
    body: { parts: [{ type: "text", text: message.text }] },
  })

  console.log("📤 Nexus response:", JSON.stringify(result, null, 2))
