...
describe("inference stat normalization", () => {
  ...
  test("uses provider.model to resolve nexus route providers", () => {
    ...
    expect(statProvider("big-pickle", "claude-sonnet-4-5", "nexus")).toBe("anthropic")
    expect(statProvider("big-pickle", "gpt-5", "nexus")).toBe("openai")
    expect(statProvider("big-pickle", "", "nexus")).toBe("unknown")
  ...
  test("model aggregates prefer provider.model and use normalized model", () => {
    ...
    expect(
      toModelAggregate({ ...aggregate("big-pickle", "nexus"), provider_model: "claude-sonnet-4-5" }),
    ).toMatchObject([
      {
        provider: "anthropic",
  ...
  test("provider aggregates never keep nexus as the provider", () => {
    expect(toProviderAggregate({ ...aggregate("big-pickle", "nexus"), provider_model: "gpt-5" })).toMatchObject([
      { provider: "openai" },
    ])
    expect(toProviderAggregate(aggregate("big-pickle", "nexus"))).toMatchObject([{ provider: "unknown" }])
    expect(toProviderAggregate(aggregate("muse-spark-1.2-contributor", "unknown"))).toMatchObject([
      { provider: "meta" },
    ])
  })

  test("geo aggregates never keep nexus or big-pickle dimensions", () => {
    expect(toGeoAggregate({ ...aggregate("big-pickle", "nexus"), country: "US" })).toMatchObject([
