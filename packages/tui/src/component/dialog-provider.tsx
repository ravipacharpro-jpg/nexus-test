...
import type { ProviderAuthAuthorization, ProviderAuthMethod } from "@nexus-ai/sdk/v2"
    ...
    const value = await DialogPrompt.show(dialog, "Other", {
      ...
      description: () => (
        <text fg={theme.textMuted}>
          This only stores a credential. Configure the provider in nexus.json to use it.
        </text>
      ),
    })
    ...
    <DialogPrompt
        ...
        ({
          nexus: (
            <box gap={1}>
              ...
              <text fg={theme.text}>
                Go to <span style={{ fg: theme.primary }}>https://nexus.ai/zen</span> to get a key
              </text>
            </box>
          ),
          "nexus-go": (
            <box gap={1}>
              ...
              <text fg={theme.text}>
                Go to <span style={{ fg: theme.primary }}>https://nexus.ai/go</span> and enable NEXUS Go
              </text>
            </box>
          ),
      ...
      onConfirm={async (value) => {
        ...
        if (props.custom && !sync.data.provider_next.all.some((provider) => provider.id === props.providerID)) {
          toast.show({
            variant: "info",
            message: `Saved credential for ${props.providerID}. Configure it in nexus.json to use it.`,
          })
          dialog.clear()
          return
        }
