...
export function Prompt(props: PromptProps) {
  ...
  // NEXUS_INPUT_ALWAYS_ACTIVE (default ON): when this is set, the input
  // field is never disabled by the prompt's own "disabled" prop. The user
  // can type and submit a new instruction even while a previous turn is
  // still resolving — the new instruction is delivered to a fresh session
  // so they never see a "pending" or "queued" label again. Set
  // NEXUS_INPUT_ALWAYS_ACTIVE=0 to restore the legacy "disabled while
  // busy" behavior.
  const effectiveDisabled = createMemo(() => {
    if (process.env.NEXUS_INPUT_ALWAYS_ACTIVE === "0") return !!effectiveDisabled()
    return false
  })
  ...
  createEffect(() => {
    if (!input || input.isDestroyed) return
    if (effectiveDisabled()) input.cursorColor = theme.backgroundElement
    if (!effectiveDisabled()) input.cursorColor = theme.text
  ...
  useBindings(() => {
    return {
      target: inputTarget,
      enabled: inputTarget() !== undefined && !effectiveDisabled(),
  ...
  useBindings(() => {
    return {
      target: inputTarget,
      enabled: inputTarget() !== undefined && !effectiveDisabled() && store.prompt.input !== "",
  ...
  useBindings(() => {
    return {
      ...
      enabled: (() => {
        cursorVersion()
        return inputTarget() !== undefined && !effectiveDisabled() && !auto()?.visible && input !== undefined
      })(),
      commands: [
        {
  ...
  async function submitInner() {
    ...
    if (effectiveDisabled()) return false
        ...
        >
          ...
          >
            <textarea
              ...
              cursorColor={effectiveDisabled() ? theme.backgroundElement : theme.text}
