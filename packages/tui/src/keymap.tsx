...
export const NEXUS_BASE_MODE = "base"
export const COMMAND_PALETTE_COMMAND = "command.palette.show"

const NEXUS_MODE_KEY = "nexus.mode"

export const NexusKeymapProvider = KeymapProvider
export const useNexusKeymap = useKeymap

export { useBindings, useKeymapSelector }

export type OpenTuiKeymap = ReturnType<typeof useKeymap>
type NexusModeStack = ReturnType<typeof createNexusModeStack>
...
const modeStacks = new WeakMap<OpenTuiKeymap, NexusModeStack>()

function isVisiblePaletteCommand(command: Command) {
  return command.hidden !== true && command.name !== COMMAND_PALETTE_COMMAND
}

export function createNexusModeStack(keymap: OpenTuiKeymap) {
  keymap.setData(NEXUS_MODE_KEY, NEXUS_BASE_MODE)

  const offFields = keymap.registerLayerFields({
    mode(value, ctx) {
      ctx.require(NEXUS_MODE_KEY, value)
    },
  })
  ...
  const update = () => {
    keymap.setData(NEXUS_MODE_KEY, stack.at(-1)?.mode ?? NEXUS_BASE_MODE)
  }

  const stackApi = {
    current() {
      return stack.at(-1)?.mode ?? NEXUS_BASE_MODE
    },
    push(mode: string) {
      if (disposed) return () => {}
    ...
    dispose() {
      ...
      stack.length = 0
      offFields()
      keymap.setData(NEXUS_MODE_KEY, undefined)
...
export function useNexusModeStack() {
  return getNexusModeStack(useNexusKeymap())
}

export function getNexusModeStack(keymap: OpenTuiKeymap) {
  const value = modeStacks.get(keymap)
  if (!value) throw new Error("Nexus mode stack is not registered for this keymap")
  return value
}
...
export function registerNexusKeymap(keymap: OpenTuiKeymap, renderer: CliRenderer, config: ResolvedKeymapConfig) {
  const modeStack = createNexusModeStack(keymap)
