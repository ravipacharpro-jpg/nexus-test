import type { TuiDialogSelectOption, TuiPluginApi, TuiSlotProps } from "@nexus-ai/plugin/tui"
...
import type { useNexusKeymap } from "../keymap"
...
type Input = {
  ...
  keymap: ReturnType<typeof useNexusKeymap>
...
export function createTuiApiAdapters(input: Input): Omit<TuiPluginApi, "lifecycle"> {
  return {
    ...
    mode: {
      current() {
        return Keymap.getNexusModeStack(input.keymap).current()
      },
      push(mode) {
        return Keymap.getNexusModeStack(input.keymap).push(mode)
      },
    },
    route: {
      register(list) {
