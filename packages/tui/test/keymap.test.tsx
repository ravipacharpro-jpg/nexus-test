...
import { getNexusModeStack, NEXUS_BASE_MODE, NexusKeymapProvider, registerNexusKeymap } from "../src/keymap"
...
test("legacy page key aliases compile as page keys", async () => {
  ...
  function Harness() {
    ...
    const offKeymap = registerNexusKeymap(keymap, renderer, config)
...
test("mode-less bindings stay active when nexus mode changes", async () => {
  ...
  function Harness() {
    ...
    const popQuestion = getNexusModeStack(keymap).push("question")
    counts.question = activeCounts()
    popQuestion()
    const popAutocomplete = getNexusModeStack(keymap).push("autocomplete")
