...
أنشئ الملف `.nexus/commands/test.md`:
...
يمكنك إضافة أوامر مخصصة عبر إعدادات NEXUS أو بإنشاء ملفات Markdown داخل الدليل `commands/`.

---

### JSON

استخدم خيار `command` في [config](/docs/config) الخاص بـ NEXUS:
...
- على مستوى النظام: `~/.config/nexus/commands/`
- لكل مشروع: `.nexus/commands/`
...
يتضمن nexus عدة أوامر مضمنة مثل `/init` و`/undo` و`/redo` و`/share` و`/help`؛ [اعرف المزيد](/docs/tui#commands).
