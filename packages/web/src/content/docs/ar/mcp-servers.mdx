...
يمكنك إضافة أدوات خارجية إلى NEXUS باستخدام _Model Context Protocol_، أو MCP. يدعم NEXUS خوادم محلية وبعيدة.
...
يمكنك تعريف خوادم MCP في [إعدادات NEXUS](https://nexus.ai/docs/config/) ضمن `mcp`. أضف كل خادم MCP باسم فريد. ويمكنك الإشارة إلى ذلك الخادم بالاسم عند توجيه الطلب إلى الـ LLM.
...
يمكن للمنظمات توفير خوادم MCP الافتراضية عبر نقطة النهاية `.well-known/nexus` الخاصة بها. قد تكون هذه الخوادم معطّلة افتراضيا، مما يتيح للمستخدمين تفعيل ما يحتاجونه فقط.
...
يتولى NEXUS تلقائيا معالجة مصادقة OAuth لخوادم MCP البعيدة. عندما يتطلب خادم ما المصادقة، سيقوم NEXUS بما يلي:
...
إذا كان الخادم يتطلب المصادقة، فسيطلب منك NEXUS المصادقة عند أول محاولة لاستخدامه. وإذا لم يحدث ذلك، يمكنك [تشغيل التدفق يدويا](#authenticating) باستخدام `nexus mcp auth <server-name>`.
...
سيفتح أمر `mcp auth` متصفحك للتفويض. بعد إكمال التفويض، سيخزّن NEXUS الرموز بشكل آمن في `~/.local/share/nexus/mcp-auth.json`.
...
تتوفر خوادم MCP لديك كأدوات داخل NEXUS إلى جانب الأدوات المضمنة. لذا يمكنك إدارتها عبر إعدادات NEXUS مثل أي أداة أخرى.
...
سيؤدي ذلك إلى فتح نافذة متصفح لإكمال تدفق OAuth وربط NEXUS بحساب Sentry الخاص بك.
