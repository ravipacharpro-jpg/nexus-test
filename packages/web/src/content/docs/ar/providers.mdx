---
title: المزوّدون
description: استخدام أي موفّر LLM في NEXUS.
---

import config from "../../../../config.mjs"
export const console = config.console

يستخدم NEXUS ‏[AI SDK](https://ai-sdk.dev/) و[Models.dev](https://models.dev) لدعم **أكثر من 75 موفّر LLM**، كما يدعم تشغيل النماذج المحلية.

لإضافة مزوّد تحتاج إلى:

1. أضف مفاتيح API الخاصة بالمزوّد باستخدام الأمر `/connect`.
2. اضبط المزوّد في ملف إعدادات NEXUS.
...
في `~/.local/share/nexus/auth.json`.

---

### الإعدادات

يمكنك تخصيص المزوّدات عبر قسم `provider` في ملف إعدادات NEXUS.
...
NEXUS Zen هي قائمة نماذج يوفّرها فريق NEXUS وقد تم
اختبارها والتحقق من أنها تعمل بشكل جيد مع NEXUS. [اعرف المزيد](/docs/zen).

:::tip
إذا كنت جديدا، فننصح بالبدء مع NEXUS Zen.
:::

1. شغّل الأمر `/connect` في TUI، واختر nexus، ثم انتقل إلى [nexus.ai/auth](https://nexus.ai/auth).
...
يعمل مثل أي مزوّد آخر في NEXUS واستخدامه اختياري بالكامل.

---

## NEXUS Go

NEXUS Go هي خطة اشتراك منخفضة التكلفة توفّر وصولا موثوقا إلى نماذج البرمجة المفتوحة الشهيرة المقدّمة من فريق NEXUS، والتي تم اختبارها والتحقق من أنها تعمل بشكل جيد مع NEXUS.

1. شغّل الأمر `/connect` في TUI، واختر `NEXUS Go`، ثم انتقل إلى [nexus.ai/auth](https://nexus.ai/zen).
...
لاستخدام Amazon Bedrock مع NEXUS:
...
2. **اضبط المصادقة** باستخدام إحدى الطرق التالية:
   ...
   عيّن أحد متغيرات البيئة التالية أثناء تشغيل nexus:
   ...
   للإعداد الخاص بالمشروع أو الإعداد الدائم، استخدم `nexus.json`:
...
:::info
استخدام اشتراك Claude Pro/Max داخل NEXUS غير مدعوم رسميا من [Anthropic](https://anthropic.com).
:::
...
يمكنك تكوين nexus لاستخدام النماذج المحلية عبر [Atomic Chat](https://atomic.chat)، وهو تطبيق سطح مكتب يقوم بتشغيل LLMs المحلية خلف خادم API متوافق مع OpenAI (نقطة النهاية الافتراضية `http://127.0.0.1:1337/v1`).
...
2. اذهب إلى [Azure AI Foundry](https://ai.azure.com/) وانشر نموذجا.
   ...
   :::note
   يجب أن يطابق اسم النشر اسم النموذج كي يعمل nexus بشكل صحيح.
   :::
...
5. شغّل الأمر `/models` لاختيار نموذج.
   ...
   يمكنك أيضا إضافة نماذج عبر إعدادات nexus.
...
تخزين الرمز في مساحة تخزين مصادقة nexus.
:::

##### GitLab مستضاف ذاتيا

:::note[ملاحظة امتثال]
يستخدم NEXUS نموذجا صغيرا لبعض مهام الذكاء الاصطناعي مثل إنشاء عنوان الجلسة.
تم ضبطه لاستخدام gpt-5-nano افتراضيا، مستضافا عبر Zen. لحصر NEXUS
على استخدام مثيل GitLab المستضاف لديك فقط، أضف ما يلي إلى ملف
`nexus.json` لديك. يوصى أيضا بتعطيل مشاركة الجلسات.
...
مزيد من التوثيق على صفحة [nexus-gitlab-auth](https://www.npmjs.com/package/nexus-gitlab-auth).
...
لاستخدام اشتراك GitHub Copilot مع nexus:
...
لاستخدام Google Vertex AI مع NEXUS:
...
إذا رأيت ميزة أو نموذجا في Helicone لم يتم ضبطه تلقائيا عبر nexus، فيمكنك دائما ضبطه بنفسك.
...
تتيح ميزة [Sessions](https://docs.helicone.ai/features/sessions) في Helicone تجميع طلبات LLM المرتبطة معا. استخدم ملحق [nexus-helicone-session](https://github.com/H2Shami/nexus-helicone-session) لتسجيل كل محادثة NEXUS تلقائيا كجلسة في Helicone.
...
يقوم الملحق بحقن ترويسات `Helicone-Session-Id` و`Helicone-Session-Name` ضمن طلباتك. في صفحة Sessions داخل Helicone، سترى كل محادثة NEXUS مدرجة كجلسة مستقلة.
...
يمكنك ضبط nexus لاستخدام نماذج محلية عبر أداة llama-server ضمن [llama.cpp](https://github.com/ggml-org/llama.cpp)
...
يمكنك ضبط nexus لاستخدام نماذج محلية عبر LM Studio.
...
يمكنك ضبط nexus لاستخدام نماذج محلية عبر Ollama.

:::tip
يمكن لـ Ollama ضبط نفسه تلقائيا للعمل مع NEXUS. راجع [توثيق تكامل Ollama](https://docs.ollama.com/integrations/nexus) للتفاصيل.
:::
...
لاستخدام Ollama Cloud مع NEXUS:
...
3. انسخ مفتاح API لاستخدامه في NEXUS.
...
6. **مهم**: قبل استخدام نماذج السحابة في NEXUS، يجب سحب معلومات النموذج محليا:
...
NEXUS Zen هي قائمة من النماذج التي تم اختبارها والتحقق منها مقدّمة من فريق NEXUS. [اعرف المزيد](/docs/zen).

1. سجّل الدخول إلى **<a href={console}>NEXUS Zen</a>** وانقر **Create API Key**.

2. شغّل الأمر `/connect` وابحث عن **NEXUS Zen**.

   ```txt
   /connect
   ```

3. أدخل مفتاح API الخاص بـ NEXUS.
...
4. يتم تحميل العديد من نماذج OpenRouter مسبقا بشكل افتراضي؛ شغّل الأمر `/models` لاختيار ما تريده.
   ...
   يمكنك أيضا إضافة نماذج إضافية عبر إعدادات nexus.
...
5. يمكنك أيضا تخصيصها عبر إعدادات nexus. إليك مثالا على تحديد مزوّد
...
لاستخدام [Scaleway Generative APIs](https://www.scaleway.com/en/docs/generative-apis/) مع Nexus:
...
يمكنك أيضا تخصيص النماذج عبر إعدادات nexus. إليك مثالا على تحديد ترتيب توجيه المزوّدات.
...
:::tip
يمكنك استخدام أي مزوّد OpenAI-compatible مع nexus. معظم مزوّدي الذكاء الاصطناعي الحديثة يقدّمون APIs متوافقة مع OpenAI.
:::
...
4. أنشئ أو حدّث ملف `nexus.json` في دليل مشروعك:
...
تسمح حقول `limit` لـ NEXUS بفهم مقدار السياق المتبقي لديك. تسحب المزوّدات القياسية هذه القيم تلقائيا من models.dev.
...
1. **تحقق من إعداد auth**: شغّل `nexus auth list` لمعرفة ما إذا كانت بيانات الاعتماد
   الخاصة بالمزوّد قد أضيفت إلى إعداداتك.

   لا ينطبق هذا على مزوّدات مثل Amazon Bedrock التي تعتمد على متغيرات البيئة لـ auth.

2. للمزوّدات المخصّصة، راجع إعدادات nexus وتأكد من:
   - أن provider ID المستخدم في الأمر `/connect` يطابق الـ ID في إعدادات nexus.
