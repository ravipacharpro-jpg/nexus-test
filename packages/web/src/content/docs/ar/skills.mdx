...
تتيح مهارات الوكيل لـ NEXUS اكتشاف تعليمات قابلة لإعادة الاستخدام من مستودعك أو من دليل المنزل.
...
يبحث NEXUS في هذه المواقع:

- إعدادات المشروع: `.nexus/skills/<name>/SKILL.md`
- إعدادات عامة: `~/.config/nexus/skills/<name>/SKILL.md`
...
بالنسبة لمسارات المشروع المحلية، يتدرج NEXUS صعوداً من دليل العمل الحالي حتى يصل إلى `git worktree`.
ويحمّل أي ملفات مطابقة لـ `skills/*/SKILL.md` داخل `.nexus/` وأي ملفات مطابقة لـ `.claude/skills/*/SKILL.md` أو `.agents/skills/*/SKILL.md` على طول الطريق.

كما تُحمَّل التعريفات العامة أيضاً من `~/.config/nexus/skills/*/SKILL.md` و `~/.claude/skills/*/SKILL.md` و `~/.agents/skills/*/SKILL.md`.
...
أنشئ `.nexus/skills/git-release/SKILL.md` بهذا الشكل:
...
يعرض NEXUS المهارات المتاحة في وصف أداة `skill`.
...
تحكم في المهارات التي يمكن للوكلاء الوصول إليها باستخدام صلاحيات مبنية على أنماط داخل `nexus.json`:
...
**للوكلاء المدمجين** (في `nexus.json`):
