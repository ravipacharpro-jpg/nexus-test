...
مع NEXUS يمكنك الاختيار من بين عدة سمات مدمجة، أو استخدام سمة تتكيّف مع سمة terminal لديك، أو تعريف سمة مخصصة خاصة بك.

افتراضيًا، يستخدم NEXUS سمتنا `nexus`.
...
- يريدون أن يطابق NEXUS مظهر terminal لديهم
...
يدعم NEXUS نظام سمات مرنًا قائمًا على JSON يتيح للمستخدمين إنشاء السمات وتخصيصها بسهولة.
...
2. **مجلد ضبط المستخدم** - معرّف في `~/.config/nexus/themes/*.json` أو `$XDG_CONFIG_HOME/nexus/themes/*.json`
3. **مجلد جذر المشروع** - معرّف في `<project-root>/.nexus/themes/*.json`
4. **مجلد العمل الحالي** - معرّف في `./.nexus/themes/*.json`
