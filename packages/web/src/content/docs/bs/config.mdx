---
title: Konfiguracija
description: Korištenje NEXUS JSON konfiguracije.
---

Možete konfigurirati NEXUS koristeći JSON konfiguracijski fajl.

---

## Format

NEXUS podržava i **JSON** i **JSONC** (JSON sa komentarima) formate.
...
1. **Udaljena konfiguracija** (od `.well-known/nexus`) - organizacijske postavke
2. **Globalna konfiguracija** (`~/.config/nexus/nexus.json`) - korisničke preferencije
3. **Prilagođena konfiguracija** (`NEXUS_CONFIG` env var) - prilagođena preinačenja
4. **Konfiguracija projekta** (`nexus.json` u projektu) - postavke specifične za projekat
5. **`.nexus` direktoriji** - agenti, komande, dodaci
6. **Inline konfiguracija** (`NEXUS_CONFIG_CONTENT` env var) - runtime preinačenja
...
:::note
Direktoriji `.nexus` i `~/.config/nexus` koriste **imena u množini** za poddirektorije: `agents/`, `commands/`, `modes/`, `plugins/`, `skills/`, `tools/` i `themes/`. Pojedinačna imena (npr. `agent/`) su također podržana za kompatibilnost unatrag.
:::
...
Organizacije mogu pružiti zadanu konfiguraciju preko `.well-known/nexus` krajnje tačke. Ovo se automatski preuzima kada se autentifikujete kod provajdera koji to podržava.
...
Postavite svoju globalnu NEXUS konfiguraciju u `~/.config/nexus/nexus.json`. Koristite globalnu konfiguraciju za korisničke preferencije kao što su provajderi, modeli i dozvole.

Za postavke specifične za TUI, koristite `~/.config/nexus/tui.json`.
...
Dodajte `nexus.json` u korijen projekta. Konfiguracija projekta ima najveći prioritet među standardnim konfiguracijskim datotekama - ona nadjačava globalne i udaljene konfiguracije.
...
Kada se NEXUS pokrene, traži konfiguracijsku datoteku u trenutnom direktoriju ili prelazi do najbližeg Git direktorija.
...
Navedite prilagođenu putanju konfiguracijske datoteke koristeći varijablu okruženja `NEXUS_CONFIG`.
...
Navedite prilagođeni konfiguracijski direktorij koristeći `NEXUS_CONFIG_DIR` varijablu okruženja. U ovom direktoriju će se tražiti agenti, komande, modovi i dodaci baš kao standardni `.nexus` direktorij, i trebali bi pratiti istu strukturu.
...
Prilagođeni direktorij se učitava nakon direktorija globalne konfiguracije i `.nexus`, tako da **može nadjačati** njihove postavke.

---

## Šema

Konfiguracijski fajl ima šemu koja je definirana u [**`nexus.ai/config.json`**](https://nexus.ai/config.json).

TUI konfiguracija koristi [**`nexus.ai/tui.json`**](https://nexus.ai/tui.json).
...
Koristite `NEXUS_TUI_CONFIG` da pokažete na prilagođeni TUI konfiguracijski fajl.

Stari `theme`, `keybinds`, i `tui` ključevi u `nexus.json` su zastarjeli i automatski će se migrirati kada je to moguće.
...
Možete konfigurirati postavke servera za naredbe `nexus serve` i `nexus web` putem opcije `server`.
...
- `mdns` - Omogući mDNS otkrivanje servisa. Ovo omogućava drugim uređajima na mreži da otkriju vaš NEXUS server.
- `mdnsDomain` - Prilagođeno ime domene za mDNS servis. Zadano je `nexus.local`. Korisno za pokretanje više instanci na istoj mreži.
...
Možete konfigurirati dobavljače i modele koje želite koristiti u svojoj NEXUS konfiguraciji kroz opcije `provider`, `model` i `small_model`.
...
Opcija `small_model` konfigurira poseban model za lagane zadatke poput generiranja naslova. Podrazumevano, NEXUS pokušava da koristi jeftiniji model ako je dostupan od vašeg provajdera, inače se vraća na vaš glavni model.
...
Također možete definirati agente koristeći markdown datoteke u `~/.config/nexus/agents/` ili `.nexus/agents/`. [Saznajte više ovdje](/docs/agents).
...
Zadani agent mora biti primarni agent (ne podagent). Ovo može biti ugrađeni agent kao što je `"build"` ili `"plan"`, ili [prilagođeni agent](/docs/agents) koji ste definirali. Ako navedeni agent ne postoji ili je podagent, NEXUS će se vratiti na `"build"` s upozorenjem.

Ova postavka se primjenjuje na sva sučelja: TUI, CLI (`nexus run`), desktop aplikaciju i GitHub Action.
...
Također možete definirati naredbe koristeći markdown fajlove u `~/.config/nexus/commands/` ili `.nexus/commands/`. [Saznajte više ovdje](/docs/commands).
...
NEXUS će automatski preuzeti sva nova ažuriranja kada se pokrene. Ovo možete onemogućiti opcijom `autoupdate`.
...
Prema zadanim postavkama, NEXUS **dopušta sve operacije** bez potrebe za eksplicitnim dopuštenjem. Ovo možete promijeniti koristeći opciju `permission`.
...
[Plugins](/docs/plugins) proširuju NEXUS sa prilagođenim alatima, kukicama i integracijama.

Postavite datoteke dodataka u `.nexus/plugins/` ili `~/.config/nexus/plugins/`. Također možete učitati dodatke iz npm-a preko opcije `plugin`.
...
Ovo je korisno kada želite da ograničite NEXUS da koristi samo određene provajdere umjesto da ih onemogućavate jednog po jednog.
