...
Možete dodati vanjske alate u NEXUS koristeći _Model Context Protocol_, ili MCP. NEXUS podržava i lokalne i udaljene servere.
...
Možete definirati MCP servere u vašoj [NEXUS Config](https://nexus.ai/docs/config/) pod `mcp`. Dodajte svaki MCP sa jedinstvenim imenom. Možete se pozvati na taj MCP po imenu kada tražite LLM.
...
Organizacije mogu obezbijediti zadane MCP servere preko svoje krajnje tačke `.well-known/nexus`. Ovi serveri mogu biti onemogućeni prema zadanim postavkama, omogućavajući korisnicima da se odluče za one koji su im potrebni.
...
NEXUS automatski rukuje OAuth autentifikacijom za udaljene MCP servere. Kada server zahtijeva autentifikaciju, NEXUS će:
...
Ako server zahtijeva autentifikaciju, NEXUS će vas tražiti prijavu pri prvom korištenju. Ako se to ne desi, možete [ručno pokrenuti tok](#authenticating) naredbom `nexus mcp auth <server-name>`.
...
Komanda `mcp auth` otvara pretraživač za autorizaciju. Nakon odobrenja, NEXUS sigurno čuva tokene u `~/.local/share/nexus/mcp-auth.json`.
...
Vaši MCP serveri su dostupni kao alati u NEXUS, zajedno s ugrađenim alatima. Možete njima upravljati kroz NEXUS konfiguraciju kao i bilo kojim drugim alatom.
...
Ovo će otvoriti prozor pretraživača da završite OAuth tok i povežete NEXUS sa vašim Sentry nalogom.
