---
title: Dodaci
description: Napišite vlastite dodatke za proširenje NEXUS.
---

Dodaci vam omogućavaju da proširite NEXUS spajanjem na različite događaje i prilagođavanjem ponašanja. Možete kreirati dodatke za dodavanje novih funkcija, integraciju sa eksternim uslugama ili izmenu zadanog ponašanja NEXUS.
...
- `.nexus/plugins/` - Dodaci na nivou projekta
- `~/.config/nexus/plugins/` - Globalni dodaci
...
**npm dodaci** se instaliraju automatski pomoću Bun pri pokretanju. Paketi i njihove zavisnosti su keširani u `~/.cache/nexus/node_modules/`.
...
1. Globalna konfiguracija (`~/.config/nexus/nexus.json`)
2. Konfiguracija projekta (`nexus.json`)
3. Globalni direktorij dodataka (`~/.config/nexus/plugins/`)
4. Direktorij dodataka projekta (`.nexus/plugins/`)
...
NEXUS pokreće `bun install` pri pokretanju da ih instalira. Vaši dodaci i alati ih zatim mogu uvesti.
...
- `client`: Nexus SDK klijent za interakciju sa AI.
...
Evo nekoliko primjera dodataka koje možete koristiti za proširenje NEXUS.
...
Spriječite nexus da čita `.env` fajlove:
...
Dodaci također mogu dodati prilagođene alate u nexus:
...
Pomoćnik `tool` kreira prilagođeni alat koji nexus može pozvati. Uzima funkciju Zod sheme i vraća definiciju alata sa:

- `description`: Šta alat radi
- `args`: Zod šema za argumente alata
- `execute`: Funkcija koja se pokreće kada se pozove alat
  Vaši prilagođeni alati će biti dostupni za nexus zajedno sa ugrađenim alatima.
...
Nivoi su: `debug`, `info`, `warn`, `error`. Pogledajte [SDK dokumentaciju](https://nexus.ai/docs/sdk) za detalje.
