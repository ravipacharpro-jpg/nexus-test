---
title: Provajderi
description: Korištenje bilo kojeg LLM provajdera u NEXUS.
---

import config from "../../../../config.mjs"
export const console = config.console

NEXUS koristi [AI SDK](https://ai-sdk.dev/) i [Models.dev](https://models.dev) za podršku **75+ LLM provajdera** i podržava pokretanje lokalnih modela.

Za dodavanje provajdera potrebno je:

1. Dodajte API ključeve za provajdera koristeći naredbu `/connect`.
2. Konfigurirajte dobavljača u vašoj NEXUS konfiguraciji.
...
u `~/.local/share/nexus/auth.json`.

---

### Konfiguracija

Možete prilagoditi dobavljače putem odjeljka `provider` u vašem NEXUS
config.
...
NEXUS Zen je lista modela koje je obezbedio NEXUS tim koji su bili
testirano i potvrđeno da dobro radi sa NEXUS. [Saznajte više](/docs/zen).

:::tip
Ako ste novi, preporučujemo da počnete sa NEXUS Zen.
:::

1. Pokrenite naredbu `/connect` u TUI-u, odaberite nexus i idite na [nexus.ai/auth](https://nexus.ai/auth).
...
Radi kao i svaki drugi provajder u NEXUS i potpuno je opcionalan za korištenje.

---

## NEXUS Go

NEXUS Go je jeftin plan pretplate koji pruža pouzdan pristup popularnim modelima otvorenog kodiranja koje pruža NEXUS tim i koji su testirani i verificirani da dobro rade s NEXUS-om.

1. Pokrenite naredbu `/connect` u TUI-u, odaberite `NEXUS Go` i idite na [nexus.ai/auth](https://nexus.ai/zen).
...
Da biste koristili Amazon Bedrock s NEXUS:
...
2. **Konfigurirajte autentifikaciju** koristeći jedan od sljedećih metoda:
   ...
   Postavite jednu od ovih varijabli okruženja dok pokrećete nexus:
...
Za konfiguraciju specifičnu za projekat ili trajnu konfiguraciju, koristite `nexus.json`:
...
:::info
[Anthropic] (https://anthropic.com) službeno ne podržava korištenje vaše Claude Pro/Max pretplate u NEXUS.
:::
...
Možete konfigurirati nexus za korištenje lokalnih modela preko [Atomic Chata](https://atomic.chat) — desktop aplikacije koja pokreće lokalne LLM-ove iza OpenAI-kompatibilnog API servera (zadana krajnja tačka `http://127.0.0.1:1337/v1`).
...
2. Idite na [Azure AI Foundry](https://ai.azure.com/) i implementirajte model.
   ...
   :::note
   Ime implementacije mora odgovarati imenu modela da bi nexus ispravno radio.
   :::
...
Također možete dodati modele kroz svoju nexus konfiguraciju.
...
da pohrani token u nexus auth memoriju.
:::

##### Samostalni GitLab

:::note[Napomena o usklađenosti]
NEXUS koristi mali model za neke AI zadatke kao što je generiranje naslova sesije.
Podrazumevano je konfigurisan da koristi gpt-5-nano, a hostuje ga Zen. Da zaključate NEXUS
da biste koristili samo svoju vlastitu instancu koju hostuje GitLab, dodajte sljedeće u svoju
`nexus.json` fajl. Također se preporučuje da onemogućite dijeljenje sesije.
...
Više dokumentacije na početnoj stranici [nexus-gitlab-auth](https://www.npmjs.com/package/nexus-gitlab-auth).

##### Konfiguracija

Prilagodite putem `nexus.json`:
...
Da biste koristili svoju GitHub Copilot pretplatu s nexus:
...
Za korištenje Google Vertex AI s NEXUS:
...
2. Postavite potrebne varijable okruženja:
   ...
   Postavite ih dok se pokreće nexus.
...
U slučaju da vidite funkciju ili model iz Helicone-a koji nije automatski konfiguriran putem nexusa, uvijek ga možete sami konfigurirati.
...
Heliconeova funkcija [Sessions](https://docs.helicone.ai/features/sessions) vam omogućava da grupišete povezane LLM zahtjeve zajedno. Koristite dodatak [nexus-helicone-session](https://github.com/H2Shami/nexus-helicone-session) da automatski prijavite svaki NEXUS razgovor kao sesiju u Helicone-u.
...
Dodatak ubacuje zaglavlja `Helicone-Session-Id` i `Helicone-Session-Name` u vaše zahtjeve. Na stranici Helicone Sessions, vidjet ćete svaki NEXUS razgovor naveden kao zasebna sesija.
...
Možete konfigurirati nexus za korištenje lokalnih modela putem [llama.cpp's](https://github.com/ggml-org/llama.cpp) uslužnog programa llama-server
...
Možete konfigurirati nexus za korištenje lokalnih modela preko LM Studio.
...
Možete konfigurirati nexus za korištenje lokalnih modela putem Ollame.

:::tip
Ollama se može automatski konfigurirati za NEXUS. Pogledajte [Ollama integracijske dokumente](https://docs.ollama.com/integrations/nexus) za detalje.
:::
...
Da biste koristili Ollama Cloud s NEXUS:
...
3. Kopirajte API ključ za korištenje u NEXUS.
...
6. **Važno**: Prije upotrebe modela oblaka u NEXUS, morate lokalno povući informacije o modelu:
...
NEXUS Zen je lista testiranih i verifikovanih modela koju je obezbedio NEXUS tim. [Saznajte više](/docs/zen).

1. Prijavite se na **<a href={console}>NEXUS Zen</a>** i kliknite na **Kreiraj API ključ**.

2. Pokrenite naredbu `/connect` i potražite **NEXUS Zen**.

```txt
   /connect
```

3. Unesite svoj NEXUS API ključ.
...
Također možete dodati dodatne modele putem vaše nexus konfiguracije.
...
5. Također ih možete prilagoditi putem vaše nexus konfiguracije. Evo primjera navođenja provajdera
...
Da biste koristili [Scaleway Generative APIs](https://www.scaleway.com/en/docs/generative-apis/) sa Nexusom:
...
Također možete prilagoditi modele kroz svoju nexus konfiguraciju. Evo primjera specificiranja redoslijeda usmjeravanja dobavljača.
...
:::tip
Možete koristiti bilo kojeg OpenAI kompatibilnog provajdera s nexus-om. Većina modernih AI provajdera nudi API-je kompatibilne sa OpenAI.
:::
...
4. Kreirajte ili ažurirajte svoju `nexus.json` datoteku u direktoriju projekta:
...
Polja `limit` omogućavaju NEXUS da shvati koliko vam je konteksta ostalo. Standardni dobavljači ih automatski preuzimaju sa models.dev.
...
1. **Provjerite postavke autentifikacije**: Pokrenite `nexus auth list` da vidite da li su vjerodajnice
...
2. Za prilagođene provajdere, provjerite NEXUS konfiguraciju i:
   - Uvjerite se da ID provajdera korišten u naredbi `/connect` odgovara ID-u u vašoj nexus konfiguraciji.
