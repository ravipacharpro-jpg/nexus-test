---
title: Server
description: Komunicirajte s nexus serverom preko HTTP-a.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

Komanda `nexus serve` pokrece headless HTTP server koji izlaže OpenAPI endpoint koji nexus klijent moze koristiti.
...
| `--mdns-domain` | Prilagodeni domen za mDNS servis    | `nexus.local` |
...
Postavite `NEXUS_SERVER_PASSWORD` da zastitite server HTTP basic auth mehanizmom. Korisnicko ime je po defaultu `nexus`, ili postavite `NEXUS_SERVER_USERNAME` za nadjacavanje. Ovo vazi i za `nexus serve` i za `nexus web`.
...
Kada pokrenete `nexus`, pokrecu se TUI i server. TUI je klijent koji komunicira sa serverom. Server izlaže OpenAPI 3.1 spec endpoint koji se koristi i za generisanje [SDK-a](/docs/sdk).

:::tip
Koristite nexus server za programsku interakciju sa nexus-om.
:::

Ova arhitektura omogucava nexus podrsku za vise klijenata i programsku interakciju.

Mozete pokrenuti `nexus serve` da startate standalone server. Ako je nexus TUI vec pokrenut, `nexus serve` ce pokrenuti novi server.
...
Endpoint [`/tui`](#tui) mozete koristiti za upravljanje TUI-jem kroz server. Na primjer, mozete unaprijed popuniti ili pokrenuti prompt. Ovaj setup koriste NEXUS [IDE](/docs/ide) pluginovi.
...
nexus server izlaže sljedece API-je.
