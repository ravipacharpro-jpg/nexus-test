...
Agent skills omogucavaju NEXUS da pronade uputstva koja se mogu ponovo koristiti iz repozitorija ili home direktorija.
...
NEXUS pretrazuje ove lokacije:

- Konfiguracija projekta: `.nexus/skills/<name>/SKILL.md`
- Globalna konfiguracija: `~/.config/nexus/skills/<name>/SKILL.md`
...
Za projektne lokalne putanje, NEXUS ide prema gore od trenutnog radnog direktorija dok ne dode do git worktree-ja.
Usput ucitava sve odgovarajuce `skills/*/SKILL.md` u `.nexus/` i odgovarajuce `.claude/skills/*/SKILL.md` ili `.agents/skills/*/SKILL.md`.

Globalne definicije se takoder ucitavaju iz `~/.config/nexus/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` i `~/.agents/skills/*/SKILL.md`.
...
Kreirajte `.nexus/skills/git-release/SKILL.md` ovako:
...
NEXUS navodi dostupne skills u opisu `skill` alata.
...
Kontrolisite kojim skills agenti mogu pristupiti pomocu dozvola baziranih na obrascima u `nexus.json`:
...
**Za ugradene agente** (u `nexus.json`):
