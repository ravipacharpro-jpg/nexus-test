...
U NEXUS mozete birati izmedu vise ugradenih tema, koristiti temu koja se prilagodava terminalu ili definisati vlastitu temu.

Po defaultu, NEXUS koristi nasu `nexus` temu.
...
NEXUS dolazi sa vise ugradenih tema.
...
- Zele da NEXUS odgovara izgledu njihovog terminala
...
NEXUS podrzava fleksibilan sistem tema baziran na JSON-u koji olaksava kreiranje i prilagodavanje tema.
...
2. **Korisnicki config direktorij** - `~/.config/nexus/themes/*.json` ili `$XDG_CONFIG_HOME/nexus/themes/*.json`
3. **Korijenski direktorij projekta** - `<project-root>/.nexus/themes/*.json`
4. **Trenutni radni direktorij** - `./.nexus/themes/*.json`
