...
Alati omogucavaju LLM-u da izvrsava akcije u vasem kodu. NEXUS dolazi sa skupom ugradenih alata, a mozete ga prosiriti kroz [custom tools](/docs/custom-tools) ili [MCP servers](/docs/mcp-servers).
...
Ovo su svi ugradeni alati dostupni u NEXUS.
...
:::note
Ovaj alat je dostupan samo kada je `NEXUS_EXPERIMENTAL_LSP_TOOL=true` (ili `NEXUS_EXPERIMENTAL=true`).
:::
...
:::note
Ovaj alat je dostupan samo uz NEXUS provajdera ili kada je varijabla `NEXUS_ENABLE_EXA` postavljena na truthy vrijednost (npr. `true` ili `1`).

Da ukljucite pri pokretanju NEXUS:
