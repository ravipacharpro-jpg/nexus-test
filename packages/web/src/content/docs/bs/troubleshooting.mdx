...
Da biste otklonili probleme s NEXUS, počnite provjeravanjem dnevnika i lokalnih podataka koje pohranjuje na disku.
...
Možete postaviti nivo dnevnika pomoću opcije komandne linije `--log-level` da biste dobili detaljnije informacije o otklanjanju grešaka. Na primjer, `nexus --log-level DEBUG`.

---

## Pohrana

NEXUS pohranjuje podatke o sesiji i druge podatke aplikacije na disku na:
...
NEXUS Desktop pokreće lokalni NEXUS server (`nexus-cli` sidecar) u pozadini. Većina problema je uzrokovana nedostatkom dodatka, oštećenom keš memorijom ili lošim postavkama servera.
...
- samo za macOS: `NEXUS` meni -> **Ponovo učitaj Webview** (pomaže ako je korisnički interfejs prazan/zamrznut).
...
- **macOS/Linux**: `~/.config/nexus/nexus.jsonc` (ili `~/.config/nexus/nexus.json`)
- **macOS/Linux** (starije instalacije): `~/.local/share/nexus/nexus.jsonc`
...
NEXUS također može učitati lokalne dodatke s diska. Privremeno ih maknite s puta (ili preimenujte folder) i ponovo pokrenite desktop aplikaciju:
...
Ako onemogućavanje dodataka ne pomogne (ili se instalacija dodatka zaglavila), obrišite keš memoriju kako bi ga NEXUS mogao ponovo izgraditi.

1. Potpuno zatvorite NEXUS Desktop.
2. Izbrišite keš direktorij:

- **macOS**: Finder -> `Cmd+Shift+G` -> zalijepi `~/.cache/nexus`
- **Linux**: obrišite `~/.cache/nexus` (ili pokrenite `rm -rf ~/.cache/nexus`)
- **Windows**: Pritisnite `WIN+R` i zalijepite `%USERPROFILE%\.cache\nexus`

3. Ponovo pokrenite NEXUS Desktop.

---

### Rješavanje problema sa vezom na serveru

NEXUS Desktop može ili pokrenuti svoj lokalni server (podrazumevano) ili se povezati na URL servera koji ste konfigurisali.
...
Ako vaš `nexus.json(c)` sadrži odjeljak `server`, privremeno ga uklonite i ponovo pokrenite desktop aplikaciju.

#### Provjerite varijable okruženja

Ako ste postavili `NEXUS_PORT` u svom okruženju, desktop aplikacija će pokušati da koristi taj port za lokalni server.

- Poništite `NEXUS_PORT` (ili odaberite slobodan port) i ponovo pokrenite.
...
Na Windows-u, NEXUS Desktop zahtijeva Microsoft Edge **WebView2 Runtime**. Ako se aplikacija otvori u praznom prozoru ili se ne pokrene, instalirajte/ažurirajte WebView2 i pokušajte ponovo.

---

### Windows: Opšti problemi sa performansama

Ako imate spore performanse, probleme s pristupom datotekama ili probleme s terminalom na Windows-u, pokušajte koristiti [WSL (Windows podsistem za Linux)](/docs/windows-wsl). WSL pruža Linux okruženje koje radi neprimetnije sa NEXUS karakteristikama.

---

### Obavještenja se ne prikazuju

NEXUS Desktop prikazuje sistemska obavještenja samo kada:

- obavještenja su omogućena za NEXUS u postavkama vašeg OS-a, i
...
1. Zatvorite NEXUS Desktop.
2. Pronađite i izbrišite ove datoteke (oni žive u direktoriju podataka NEXUS Desktop aplikacije):

- `nexus.settings.dat` (URL zadanog servera za desktop)
- `nexus.global.dat` i `nexus.workspace.*.dat` (stanje korisničkog interfejsa poput nedavnih servera/projekata)
...
3. Uvjerite se da imate najnoviju verziju sa `nexus upgrade`
...
Da saznate kojim modelima imate pristup, pokrenite `nexus models`
...
Ako naiđete na greške API poziva, to može biti zbog zastarjelih paketa dobavljača. NEXUS dinamički instalira pakete dobavljača (OpenAI, Anthropic, Google, itd.) po potrebi i kešira ih lokalno.
...
2. Ponovo pokrenite NEXUS da ponovo instalirate najnovije pakete dobavljača

Ovo će prisiliti NEXUS da preuzme najnovije verzije paketa dobavljača, što često rješava probleme kompatibilnosti s parametrima modela i promjenama API-ja.
...
NEXUS će otkriti da li koristite Wayland i preferirate `wl-clipboard`, u suprotnom će pokušati pronaći alate međuspremnika po redoslijedu: `xclip` i `xsel`.
