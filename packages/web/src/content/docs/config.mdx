---
title: Config
description: Using the NEXUS JSON config.
---

You can configure NEXUS using a JSON config file.

---

## Format

NEXUS supports both **JSON** and **JSONC** (JSON with Comments) formats.

```jsonc title="nexus.jsonc"
{
...
1. **Remote config** (from `.well-known/nexus`) - organizational defaults
2. **Global config** (`~/.config/nexus/nexus.json`) - user preferences
3. **Custom config** (`NEXUS_CONFIG` env var) - custom overrides
4. **Project config** (`nexus.json` in project) - project-specific settings
5. **`.nexus` directories** - agents, commands, plugins
6. **Inline config** (`NEXUS_CONFIG_CONTENT` env var) - runtime overrides
7. **Managed config files** (`/Library/Application Support/nexus/` on macOS) - admin-controlled
...
:::note
The `.nexus` and `~/.config/nexus` directories use **plural names** for subdirectories: `agents/`, `commands/`, `modes/`, `plugins/`, `skills/`, `tools/`, and `themes/`. Singular names (e.g., `agent/`) are also supported for backwards compatibility.
:::
...
Organizations can provide default configuration via the `.well-known/nexus` endpoint. This is fetched automatically when you authenticate with a provider that supports it.
...
```json title="Remote config from .well-known/nexus"
{
  "mcp": {
    "jira": {
      "type": "remote",
...
Place your global NEXUS config in `~/.config/nexus/nexus.json`. Use global config for user-wide server/runtime preferences like providers, models, and permissions.

For TUI-specific settings, use `~/.config/nexus/tui.json`.
...
Add `nexus.json` in your project root. Project config has the highest precedence among standard config files - it overrides both global and remote configs.
...
When NEXUS starts up, it first looks for a config file in the current directory, then traverses up to the nearest Git directory.
...
Specify a custom config file path using the `NEXUS_CONFIG` environment variable.

```bash
export NEXUS_CONFIG=/path/to/my/custom-config.json
nexus run "Hello world"
```
...
Specify a custom config directory using the `NEXUS_CONFIG_DIR`
environment variable. This directory will be searched for agents, commands,
modes, and plugins just like the standard `.nexus` directory, and should
follow the same structure.

```bash
export NEXUS_CONFIG_DIR=/path/to/my/config-directory
nexus run "Hello world"
```

The custom directory is loaded after the global config and `.nexus` directories, so it **can override** their settings.
...
Drop an `nexus.json` or `nexus.jsonc` file in the system managed config directory:
...
On macOS, NEXUS reads managed preferences from the `ai.nexus.managed` preference domain. Deploy a `.mobileconfig` via MDM (Jamf, Kandji, FleetDM) and the settings are enforced automatically.

NEXUS checks these paths:

1. `/Library/Managed Preferences/<user>/ai.nexus.managed.plist`
2. `/Library/Managed Preferences/ai.nexus.managed.plist`

The plist keys map directly to `nexus.json` fields. MDM metadata keys (`PayloadUUID`, `PayloadType`, etc.) are stripped automatically.

**Creating a `.mobileconfig`**

Use the `ai.nexus.managed` PayloadType. The NEXUS config keys go directly in the payload dict:
...
<dict>
  ...
  <array>
    <dict>
      <key>PayloadType</key>
      <string>ai.nexus.managed</string>
      <key>PayloadIdentifier</key>
      <string>com.example.nexus.config</string>
      <key>PayloadUUID</key>
  ...
  <key>PayloadIdentifier</key>
  <string>com.example.nexus</string>
  <key>PayloadUUID</key>
  <string>GENERATE-YOUR-OWN-UUID</string>
  <key>PayloadVersion</key>
  <integer>1</integer>
...
The server/runtime config schema is defined in [**`nexus.ai/config.json`**](https://nexus.ai/config.json).

TUI config uses [**`nexus.ai/tui.json`**](https://nexus.ai/tui.json).
...
Use `NEXUS_TUI_CONFIG` to point to a custom TUI config file.
...
Legacy `theme`, `keybinds`, and `tui` keys in `nexus.json` are deprecated and automatically migrated when possible.

---

### Server

You can configure server settings for the `nexus serve` and `nexus web` commands through the `server` option.
...
- `mdns` - Enable mDNS service discovery. This allows other devices on the network to discover your NEXUS server.
- `mdnsDomain` - Custom domain name for mDNS service. Defaults to `nexus.local`. Useful for running multiple instances on the same network.
...
If not specified, NEXUS will automatically discover and use a sensible default based on your operating system (e.g. `pwsh` or `cmd.exe` on Windows, `/bin/zsh` or `/bin/bash` on macOS/Linux). You can provide an absolute path or a short name.
...
You can configure the providers and models you want to use in your NEXUS config through the `provider`, `model` and `small_model` options.
...
The `small_model` option configures a separate model for lightweight tasks like title generation. By default, NEXUS tries to use a cheaper model if one is available from your provider, otherwise it falls back to your main model.
...
Use the `experimental.policies` option to allow or deny NEXUS actions on configured resources. Currently, policies can control which providers NEXUS may use.
...
NEXUS normalizes image attachments before sending them to the model. By default, images are resized when they exceed `2000x2000` pixels or `5242880` base64 bytes.
...
If an image still cannot fit after resizing, NEXUS omits oversized tool-result images or fails oversized user-provided images with an image size error.
...
You can also define agents using markdown files in `~/.config/nexus/agents/` or `.nexus/agents/`. [Learn more here](/docs/agents).
...
The default agent must be a primary agent (not a subagent). This can be a built-in agent like `"build"` or `"plan"`, or a [custom agent](/docs/agents) you've defined. If the specified agent doesn't exist or is a subagent, NEXUS will fall back to `"build"` with a warning.

This setting applies across all interfaces: TUI, CLI (`nexus run`), desktop app, and GitHub Action.
...
You can also define commands using markdown files in `~/.config/nexus/commands/` or `.nexus/commands/`. [Learn more here](/docs/commands).
...
NEXUS uses snapshots to track file changes during agent operations, enabling you to undo and revert changes within a session. Snapshots are enabled by default.
...
NEXUS will automatically download any new updates when it starts up. You can disable this with the `autoupdate` option.
...
By default, nexus **allows all operations** without requiring explicit approval. You can change this using the `permission` option.
...
[Plugins](/docs/plugins) extend NEXUS with custom tools, hooks, and integrations.

Place plugin files in `.nexus/plugins/` or `~/.config/nexus/plugins/`. You can also load plugins from npm through the `plugin` option.

```json title="nexus.json"
{
  "$schema": "https://nexus.ai/config.json",
  "plugin": ["nexus-helicone-session", "@my-org/custom-plugin"]
}
```
...
This is useful when you want to restrict NEXUS to only use specific providers rather than disabling them one by one.
