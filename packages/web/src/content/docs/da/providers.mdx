---
title: Udbydere
description: Brug en hvilken som helst LLM-udbyder i NEXUS.
---

import config from "../../../../config.mjs"
export const console = config.console

NEXUS bruger [AI SDK](https://ai-sdk.dev/) og [Models.dev](https://models.dev) til at understøtte **75+ LLM-udbydere** og understøtter kørsel af lokale modeller.

For at tilføje en udbyder skal du:

1. Tilføje API-nøglerne for udbyderen ved at bruge kommandoen `/connect`.
2. Konfigurere udbyderen i NEXUS-konfigurationen.

---

### Legitimationsoplysninger

Når du tilføjer en udbyders API-nøgler med `/connect`-kommandoen, gemmes de i `~/.local/share/nexus/auth.json`.

---

### Konfiguration

Du kan tilpasse udbyderne gennem `provider`-delen i NEXUS-konfigurationen.
...
NEXUS Zen er en liste over modeller leveret af NEXUS-teamet, som er blevet testet og verificeret til at fungere godt med NEXUS. [Læs mere](/docs/zen).

:::tip
Hvis du er ny, anbefaler vi at starte med NEXUS Zen.
:::

1. Kør kommandoen `/connect` i TUI, vælg nexus og gå til [nexus.ai/auth](https://nexus.ai/auth).
...
Det fungerer som alle andre udbydere i NEXUS og er helt valgfrit at bruge.

---

## NEXUS Go

NEXUS Go er en billig abonnementsplan, der giver pålidelig adgang til populære åbne kodningsmodeller leveret af NEXUS-teamet, som er testet og verificeret til at fungere godt med NEXUS.

1. Kør kommandoen `/connect` i TUI, vælg `NEXUS Go`, og gå til [nexus.ai/auth](https://nexus.ai/zen).
...
Sådan bruger du Amazon Bedrock med NEXUS:
...
2. **Konfigurer godkendelse** ved at bruge en af følgende metoder:
   ...
   Angiv en af disse miljøvariabler, mens du kører nexus:
   ...
   For projektspecifik eller vedvarende konfiguration, brug `nexus.json`:
...
:::info
At bruge dit Claude Pro/Max-abonnement i NEXUS understøttes ikke officielt af [Anthropic](https://anthropic.com).
:::
...
Du kan konfigurere nexus til at bruge lokale modeller via [Atomic Chat](https://atomic.chat) — en desktopapplikation, der kører lokale LLM'er bag en OpenAI-kompatibel API-server (standard-endpoint `http://127.0.0.1:1337/v1`).
...
2. Gå til [Azure AI Foundry](https://ai.azure.com/) og distribuer en model.
   ...
   :::note
   Distributionsnavnet skal matche modelnavnet for at nexus skal fungere korrekt.
   :::
...
5. Kør kommandoen `/models` for at vælge en model.
   ...
   Du kan også tilføje modeller gennem nexus-konfigurationen.
...
gemme tokenet i nexus auth-lagring.
:::

##### Selvhostet GitLab

:::note[overholdelsesbemærkning]
NEXUS bruger en lille model til nogle AI-opgaver som at generere sessionstitlen.
Den er konfigureret til at bruge gpt-5-nano som standard, hostet af Zen. For at låse NEXUS
til kun at bruge din egen GitLab-hostede instans, tilføj følgende i din
`nexus.json` fil. Det anbefales også at deaktivere sessionsdeling.
...
Mere dokumentation på [nexus-gitlab-auth](https://www.npmjs.com/package/nexus-gitlab-auth) hjemmesiden.

##### Konfiguration

Tilpas gennem `nexus.json`:
...
Sådan bruger du GitHub Copilot-abonnementet med NEXUS:
...
Sådan bruger du Google Vertex AI med NEXUS:
...
I tilfælde af at du ser en funktion eller model fra Helicone som ikke konfigureres automatisk gennem nexus, kan du altid konfigurere den selv.
...
Helicones [Sessions](https://docs.helicone.ai/features/sessions) funktion lader dig gruppe relaterede LLM anmodninger sammen. Brug [nexus-helicone-session](https://github.com/H2Shami/nexus-helicone-session) plugin for automatisk at logge hver NEXUS-samtale som en session i Helicone.
...
Programmatisk injicerer `Helicone-Session-Id` og `Helicone-Session-Name` headers i dine anmodninger. På Helicones Sessions-side vil du se hver NEXUS-samtale opført som en separat session.
...
Du kan konfigurere nexus til at bruge lokale modeller gennem [llama.cpps](https://github.com/ggml-org/llama.cpp) llama-server-værktøj
...
Du kan konfigurere nexus til at bruge lokale modeller gennem LM Studio.
...
Du kan konfigurere nexus til at bruge lokale modeller gennem Ollama.

:::tip
Ollama kan automatisk konfigurere sig selv for NEXUS. Se [Ollama-integrationsdokumenterne](https://docs.ollama.com/integrations/nexus) for detaljer.
:::
...
Sådan bruger du Ollama Cloud med NEXUS:
...
3. Kopier API-nøglen til brug i NEXUS.
...
6. **Vigtigt**: Før du bruger skymodeller i NEXUS, skal du hente modelinformationen lokalt:
...
NEXUS Zen er en liste over testede og verificerede modeller leveret af NEXUS-teamet. [Læs mere](/docs/zen).

1. Log på **<a href={console}>NEXUS Zen</a>** og klik på **Create API Key**.

2. Kør kommandoen `/connect` og søg efter **NEXUS Zen**.

   ```txt
   /connect
   ```

3. Indtast NEXUS API-nøglen.
...
4. Mange OpenRouter-modeller er forudindlæst som standard, kør kommandoen `/models` for at vælge den du ønsker.
   ...
   Du kan også tilføje flere modeller gennem nexus-konfigurationen.
...
5. Du kan også tilpasse dem gennem nexus-konfigurationen. Her er et eksempel på at specificere en udbyder
...
Sådan bruger du [Scaleway Generative APIs](https://www.scaleway.com/en/docs/generative-apis/) med NEXUS:
...
Du kan også tilpasse modeller gennem nexus-konfigurationen. Her er et eksempel på specificering af udbyder-rutingsrækkefølge.
...
:::tip
Du kan bruge hvilken som helst OpenAI-kompatibel udbyder med nexus. De fleste moderne AI-udbydere tilbyder OpenAI-kompatible API'er.
:::
...
4. Opret eller opdater `nexus.json`-filen i projektkataloget:
...
`limit`-felterne lader NEXUS forstå hvor meget kontekst du har tilbage. Standardudbydere henter disse automatisk fra models.dev.
...
1. **Tjek godkendelsesopsætningen**: Kør `nexus auth list` for at se om legitimationsoplysningerne
...
2. For tilpassede udbydere, tjek nexus-konfigurationen og:
   - Sørg for at udbyderens ID som bruges i `/connect`-kommandoen matcher ID i nexus-konfigurationen.
