---
title: Server
description: Interager med nexus-serveren over HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

Kommandoen `nexus serve` kører en hovedløs HTTP-server, som afslører et OpenAPI-endpoint, som en nexus-klient kan bruge.
...
| `--mdns-domain` | Brugerdefineret domænenavn for mDNS-tjeneste | `nexus.local` |
...
Indstil `NEXUS_SERVER_PASSWORD` for at beskytte serveren med HTTP grundlæggende godkendelse. Brugernavnet er som standard `nexus`, eller indstil `NEXUS_SERVER_USERNAME` for at tilsidesætte det. Dette gælder både `nexus serve` og `nexus web`.
...
Når du kører `nexus` starter den en TUI og en server. Hvor TUI er
...
:::tip
Brug nexus-serveren til at interagere med nexus programmatisk.
:::

Denne arkitektur lader nexus understøtte flere klienter og lader dig interagere med nexus programmatisk.

Du kan køre `nexus serve` for at starte en selvstændig server. Hvis du har
nexus TUI kørende, vil `nexus serve` starte en ny server.
...
[`/tui`](#tui) endpointet kan bruges til at køre TUI gennem serveren. Du kan for eksempel forududfylde eller køre en forespørgsel. Denne opsætning bruges af NEXUS [IDE](/docs/ide) plugins.
...
NEXUS-serveren viser følgende API'er.
