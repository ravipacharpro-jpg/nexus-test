...
Med NEXUS kan du velge fra ett av flere innebygde temaer, bruge et tema som tilpasser seg terminaltemaet ditt, eller definere ditt eget tilpassede tema.

Som standard bruger NEXUS vårt eget `nexus`-tema.
...
NEXUS støtter et fleksibelt JSON-basert temasystem som lar brugere enkelt lage og tilpasse temaer.
...
2. **Brugerkonfigurasjonskatalog** - Definert i `~/.config/nexus/themes/*.json` eller `$XDG_CONFIG_HOME/nexus/themes/*.json`
3. **Prosjektrotkatalog** - Definert i `<project-root>/.nexus/themes/*.json`
4. **Nuværende arbejdskatalog** - Definert i `./.nexus/themes/*.json`
