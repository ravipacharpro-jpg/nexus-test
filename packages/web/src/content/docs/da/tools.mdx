...
Her er alle de innebygde verktøyene som er tilgængelige i NEXUS.
...
:::note
Dette verktøyet er bare tilgængelig når `NEXUS_EXPERIMENTAL_LSP_TOOL=true` (eller `NEXUS_EXPERIMENTAL=true`).
:::
...
:::note
Dette verktøyet er bare tilgængelig når du bruger NEXUS-leverandøren eller når miljøvariabelen `NEXUS_ENABLE_EXA` er satt til en sannverdi (f.eks. `true` eller `1`).
