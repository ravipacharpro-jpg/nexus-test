...
For at fejlfinde problemer med NEXUS, start med at tjekke logfilerne og de lokale data, den gemmer på disken.
...
Du kan angive logniveauet med kommandolinjeflaget `--log-level` for at få mere detaljeret fejlfindingsinformation. For eksempel `nexus --log-level DEBUG`.

---

## Lagring

nexus gemmer sessionsdata og andre applikationsdata på disken på:
...
NEXUS Desktop kører en lokal NEXUS-server (`nexus-cli` sidevognen) i baggrunden. De fleste problemer er forårsaget af et plugin, der fungerer dårligt, en ødelagt cache eller en dårlig serverindstilling.

### Hurtige tjek

- Afslut og start appen på ny.
- Hvis appen viser en fejlskærm, klikker du på **Start på ny** og kopierer fejldetaljerne.
- Kun macOS: `NEXUS`-menu -> **Genindlæs webvisning** (hjælper hvis UI er tom/frosset).
...
- **macOS/Linux**: `~/.config/nexus/nexus.jsonc` (eller `~/.config/nexus/nexus.json`)
- **macOS/Linux** (ældre installationer): `~/.local/share/nexus/nexus.jsonc`
...
NEXUS kan også indlæse lokale plugins fra disken. Flyt disse midlertidigt væk (eller giv mappen nyt navn) og start skrivebordsappen på ny:
...
Hvis deaktivering af plugins ikke hjælper (eller en plugin-installation sidder fast), tøm cachen så NEXUS kan genopbygge den.

1. Afslut NEXUS Desktop helt.
2. Slet cache-mappen:

- **macOS**: Finder -> `Cmd+Shift+G` -> indsæt `~/.cache/nexus`
- **Linux**: slet `~/.cache/nexus` (eller kør `rm -rf ~/.cache/nexus`)
- **Windows**: Tryk `WIN+R` og indsæt `%USERPROFILE%\.cache\nexus`

3. Start NEXUS Desktop på ny.

---

### Løs problemer med serverforbindelse

NEXUS Desktop kan enten starte sin egen lokale server (standard) eller forbinde til en server URL, du har konfigureret.
...
Hvis `nexus.json(c)` indeholder en `server`-del, fjern den midlertidigt og start skrivebordsappen på ny.

#### Tjek miljøvariabler

Hvis du har `NEXUS_PORT` sat i dit miljø, vil skrivebordsappen prøve at bruge den port for den lokale server.

- Deaktiver `NEXUS_PORT` (eller vælg en ledig port) og start på ny.
...
På Windows kræver NEXUS Desktop Microsoft Edge **WebView2 Runtime**. Hvis appen åbnes i et tomt vindue eller ikke starter, installer/opdater WebView2 og prøv igen.

---

### Windows: Generelle ydeevneproblemer

Hvis du oplever langsom ydeevne, problemer med filadgang eller terminalproblemer på Windows, kan du prøve at bruge [WSL (Windows Subsystem for Linux)](/docs/windows-wsl). WSL giver et Linux-miljø som fungerer mere sømløst med funktionerne i NEXUS.

---

### Meddelelser vises ikke

NEXUS Desktop viser kun systemvarsler når:

- varsler er aktiveret for NEXUS i dine OS-indstillinger, og
...
1. Afslut NEXUS Desktop.
2. Find og slet disse filer (de findes i NEXUS Desktop-appens datamappe):

- `nexus.settings.dat` (skrivebordsstandardserver URL)
- `nexus.global.dat` og `nexus.workspace.*.dat` (UI tilstand som nylige servere/projekter)
...
Hvis du oplever problemer med NEXUS:
...
For at finde ud af hvilke modeller du har adgang til, kør `nexus models`
...
Hvis du støder på API-kaldsfejl, kan dette skyldes forældede udbyderpakker. nexus installerer dynamisk udbyderpakker (OpenAI, Anthropic, Google, etc.) efter behov og cacher dem lokalt.
...
2. Start nexus på ny for at installere de nyeste udbyderpakker på ny

Dette vil tvinge nexus til at downloade de nyeste versioner af udbyderpakkerne, som ofte løser kompatibilitetsproblemer med modelparametre og API-ændringer.
...
nexus vil opdage om du bruger Wayland og foretrække `wl-clipboard`, ellers vil den prøve at finde udklipsholderværktøjer i rækkefølgen: `xclip` og `xsel`.
