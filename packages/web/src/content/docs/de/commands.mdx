...
Erstellen Sie `.nexus/commands/test.md`:

```md title=".nexus/commands/test.md"
---
...
Sie können benutzerdefinierte Befehle über die NEXUS-Konfiguration hinzufügen oder indem Sie Markdown-Dateien im Verzeichnis `commands/` erstellen.

---

### JSON

Verwenden Sie die Option `command` in Ihrem NEXUS [config](/docs/config):

```json title="nexus.jsonc" {4-12}
{
...
- Global: `~/.config/nexus/commands/`
- Pro Projekt: `.nexus/commands/`

```markdown title="~/.config/nexus/commands/test.md"
---
description: Run tests with coverage
agent: build
model: anthropic/claude-3-5-sonnet-20241022
...
```md title=".nexus/commands/component.md"
---
...
```md title=".nexus/commands/create-file.md"
---
...
```md title=".nexus/commands/analyze-coverage.md"
---
...
```md title=".nexus/commands/review-changes.md"
---
...
```md title=".nexus/commands/review-component.md"
---
...
NEXUS enthält mehrere integrierte Befehle wie `/init`, `/undo`, `/redo`, `/share`, `/help`; [mehr erfahren](/docs/tui#commands).
