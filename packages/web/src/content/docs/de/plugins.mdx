---
title: Plugins
description: Schreiben Sie Ihre eigenen Plugins, um NEXUS zu erweitern.
---

Mit Plugins können Sie NEXUS erweitern, indem Sie verschiedene Ereignisse einbinden und das Verhalten anpassen. Sie können Plugins erstellen, um neue Funktionen hinzuzufügen, externe Dienste zu integrieren oder das Standardverhalten von NEXUS zu ändern.
...
- `.nexus/plugins/` – Plugins auf Projektebene
- `~/.config/nexus/plugins/` – Globale Plugins
...
**NPM-Plugins** werden beim Start automatisch mit Bun installiert. Pakete und ihre Abhängigkeiten werden in `~/.cache/nexus/node_modules/` zwischengespeichert.
...
1. Globale Konfiguration (`~/.config/nexus/nexus.json`)
2. Projektkonfiguration (`nexus.json`)
3. Globales Plugin-Verzeichnis (`~/.config/nexus/plugins/`)
4. Projekt-Plugin-Verzeichnis (`.nexus/plugins/`)
...
NEXUS führt beim Start `bun install` aus, um diese zu installieren. Ihre Plugins und Tools können sie dann importieren.
...
- `client`: Ein NEXUS-SDK-Client für die Interaktion mit AI.
...
Hier sind einige Beispiele für Plugins, mit denen Sie NEXUS erweitern können.
...
:::note
Wenn Sie die NEXUS-Desktop-App verwenden, kann diese automatisch Systembenachrichtigungen senden, wenn eine Antwort bereit ist oder wenn in einer Sitzung ein Fehler auftritt.
:::
...
Verhindern Sie, dass NEXUS `.env`-Dateien liest:
...
Plugins können NEXUS auch benutzerdefinierte Tools hinzufügen:
...
Der `tool`-Helfer erstellt ein benutzerdefiniertes Tool, das NEXUS aufrufen kann. Es verwendet eine Zod-Schemafunktion und gibt eine Tooldefinition zurück mit:
...
Ihre benutzerdefinierten Tools stehen neben den integrierten Tools für NEXUS zur Verfügung.
...
Ebenen: `debug`, `info`, `warn`, `error`. Weitere Informationen finden Sie unter [SDK documentation](https://nexus.ai/docs/sdk).
