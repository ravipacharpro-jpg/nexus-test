---
title: Regeln
description: Lege eigene Anweisungen fuer nexus fest.
---

Du kannst nexus ueber eine `AGENTS.md`-Datei mit eigenen Anweisungen steuern.
...
Um eine neue `AGENTS.md` zu erstellen, fuehre in nexus den Befehl `/init` aus.
...
So kann nexus sich in deinem Code besser orientieren.

Wenn bereits eine `AGENTS.md` existiert, versucht nexus sie zu erweitern.
...
nexus kann `AGENTS.md` aus mehreren Orten lesen.
...
Du kannst auch globale Regeln in `~/.config/nexus/AGENTS.md` speichern.
Diese gelten in allen nexus-Sitzungen.
...
Wenn du von Claude Code kommst, unterstuetzt NEXUS dessen Dateikonventionen als Fallback:

- **Projektregeln**: `CLAUDE.md` im Projekt (wenn keine `AGENTS.md` existiert)
- **Globale Regeln**: `~/.claude/CLAUDE.md` (wenn keine `~/.config/nexus/AGENTS.md` existiert)
...
Beim Start sucht nexus in dieser Reihenfolge nach Regeldateien:

1. **Lokale Dateien** beim Hochlaufen ab dem aktuellen Verzeichnis (`AGENTS.md`, `CLAUDE.md`)
2. **Globale Datei** unter `~/.config/nexus/AGENTS.md`
...
Du kannst zusaetzliche Anweisungsdateien in deiner `nexus.json` oder in `~/.config/nexus/nexus.json` angeben.
...
nexus parst Datei-Referenzen in `AGENTS.md` nicht automatisch.
Du erreichst aehnliches Verhalten auf zwei Wegen.

### Mit nexus.json

Empfohlen ist das `instructions`-Feld in der `nexus.json`:
...
Du kannst nexus explizit anweisen, externe Dateien zu lesen.
...
:::tip
Fuer Monorepos oder gemeinsame Standards ist `nexus.json` mit Glob-Mustern (z. B. `packages/*/AGENTS.md`) oft wartbarer als manuelle Anweisungen.
:::
