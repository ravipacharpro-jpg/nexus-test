---
title: Server
description: Nutze den nexus-Server ueber HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

Der Befehl `nexus serve` startet einen headless HTTP-Server.
Er stellt einen OpenAPI-Endpunkt bereit, den ein nexus-Client nutzen kann.
...
Setze `NEXUS_SERVER_PASSWORD`, um den Server mit HTTP Basic Auth zu schuetzen.
Der Benutzername ist standardmaessig `nexus`, kann aber mit `NEXUS_SERVER_USERNAME` ueberschrieben werden. Das gilt fuer `nexus serve` und `nexus web`.
...
Wenn du `nexus` startest, werden TUI und Server gestartet.
...
:::tip
Nutze den nexus-Server, um nexus programmatisch anzusteuern.
:::

Diese Architektur erlaubt mehrere Clients und programmatische Nutzung.

Mit `nexus serve` startest du einen eigenstaendigen Server.
Laeuft bereits eine TUI, startet `nexus serve` trotzdem eine neue Serverinstanz.
...
Dieses Muster nutzen auch die NEXUS-[IDE](/docs/ide)-Plugins.
...
Der nexus-Server stellt folgende APIs bereit.
