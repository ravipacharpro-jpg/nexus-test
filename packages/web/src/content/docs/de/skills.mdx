...
Agent Skills erlauben NEXUS, wiederverwendbare Anweisungen aus deinem Repo oder Home-Verzeichnis zu finden.
...
NEXUS sucht in folgenden Pfaden:
...
Bei projektlokalen Pfaden laeuft NEXUS vom aktuellen Verzeichnis nach oben bis zum Git-Worktree.
Dabei werden passende `skills/*/SKILL.md` in `.nexus/` sowie passende Dateien unter `.claude/skills/*/SKILL.md` und `.agents/skills/*/SKILL.md` geladen.

Globale Definitionen kommen zusaetzlich aus `~/.config/nexus/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` und `~/.agents/skills/*/SKILL.md`.
...
Erstelle `.nexus/skills/git-release/SKILL.md` zum Beispiel so:
...
NEXUS listet verfuegbare Skills in der Beschreibung des `skill`-Tools.
...
Steuere in `nexus.json` per Muster, auf welche Skills Agenten zugreifen duerfen:
...
**Fuer eingebaute Agenten** (in `nexus.json`):
