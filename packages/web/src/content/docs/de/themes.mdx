...
Mit NEXUS kannst du ein eingebautes Theme waehlen, ein an dein Terminal angepasstes Theme nutzen oder ein eigenes Theme definieren.

Standardmaessig verwendet NEXUS das `nexus`-Theme.
...
NEXUS liefert mehrere eingebaute Themes mit.
...
- NEXUS optisch ans Terminal anpassen wollen
...
NEXUS unterstuetzt ein flexibles JSON-basiertes Theme-System.
...
2. **Benutzer-Config-Verzeichnis** - Definiert in `~/.config/nexus/themes/*.json` oder `$XDG_CONFIG_HOME/nexus/themes/*.json`
3. **Projekt-Root-Verzeichnis** - Definiert in `<project-root>/.nexus/themes/*.json`
4. **Aktuelles Arbeitsverzeichnis** - Definiert in `./.nexus/themes/*.json`
