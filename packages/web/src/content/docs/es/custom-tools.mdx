...
description: Cree herramientas que LLM pueda llamar en nexus.
---

Las herramientas personalizadas son funciones que usted crea y que el LLM puede llamar durante las conversaciones. Trabajan junto con las [herramientas integradas](/docs/tools) de nexus como `read`, `write` y `bash`.
...
- Localmente colocándolos en el directorio `.nexus/tools/` de tu proyecto.
- O de forma global, colocándolos en `~/.config/nexus/tools/`.
...
```ts title=".nexus/tools/database.ts" {1}
...
```ts title=".nexus/tools/project.ts" {8}
import { tool } from "@nexus-ai/plugin"
...
```python title=".nexus/tools/add.py"
import sys
...
```ts title=".nexus/tools/python-add.ts" {10}
import { tool } from "@nexus-ai/plugin"
...
export default tool({
  ...
  async execute(args, context) {
    const script = path.join(context.worktree, ".nexus/tools/add.py")
