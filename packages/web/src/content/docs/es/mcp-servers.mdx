...
Puede agregar herramientas externas a NEXUS utilizando el _Model Context Protocol_ o MCP. NEXUS admite servidores locales y remotos.
...
Puede definir servidores MCP en su [NEXUS Config](https://nexus.ai/docs/config/) en `mcp`. Agregue cada MCP con un nombre único. Puede referirse a ese MCP por su nombre cuando solicite el LLM.
...
Las organizaciones pueden proporcionar servidores MCP predeterminados a través de su punto final `.well-known/nexus`. Estos servidores pueden estar deshabilitados de forma predeterminada, lo que permite a los usuarios optar por los que necesitan.
...
NEXUS maneja automáticamente la autenticación OAuth para servidores MCP remotos. Cuando un servidor requiere autenticación, NEXUS:
...
Si el servidor requiere autenticación, NEXUS le pedirá que se autentique cuando intente usarlo por primera vez. De lo contrario, puede [activar manualmente el flujo](#authenticating) con `nexus mcp auth <server-name>`.
...
El comando `mcp auth` abrirá su navegador para obtener autorización. Después de su autorización, NEXUS almacenará los tokens de forma segura en `~/.local/share/nexus/mcp-auth.json`.
...
Sus MCP están disponibles como herramientas en NEXUS, junto con herramientas integradas. Para que puedas administrarlos a través de la configuración NEXUS como cualquier otra herramienta.
...
Esto abrirá una ventana del navegador para completar el flujo OAuth y conectar NEXUS a su cuenta Sentry.
