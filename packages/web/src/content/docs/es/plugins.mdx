---
title: Complementos
description: Escriba sus propios complementos para extender NEXUS.
---

Los complementos le permiten extender NEXUS al conectarse a varios eventos y personalizar el comportamiento. Puede crear complementos para agregar nuevas funciones, integrarlos con servicios externos o modificar el comportamiento predeterminado de NEXUS.
...
- `.nexus/plugins/` - Complementos a nivel de proyecto
- `~/.config/nexus/plugins/` - Complementos globales
...
```json title="nexus.json"
{
  ...
  "plugin": ["nexus-helicone-session", "nexus-wakatime", "@my-org/custom-plugin"]
}
```
...
Los **complementos npm** se instalan automáticamente usando Bun al inicio. Los paquetes y sus dependencias se almacenan en caché en `~/.cache/nexus/node_modules/`.
...
1. Configuración global (`~/.config/nexus/nexus.json`)
2. Configuración del proyecto (`nexus.json`)
3. Directorio global de complementos (`~/.config/nexus/plugins/`)
4. Directorio de complementos del proyecto (`.nexus/plugins/`)
...
NEXUS ejecuta `bun install` al inicio para instalarlos. Luego, sus complementos y herramientas pueden importarlos.

```ts title=".nexus/plugins/my-plugin.ts"
...
```js title=".nexus/plugins/example.js"
...
- `client`: Un cliente SDK nexus para interactuar con la IA.
...
A continuación se muestran algunos ejemplos de complementos que puede utilizar para ampliar nexus.
...
```js title=".nexus/plugins/notification.js"
export const NotificationPlugin = async ({ project, client, $, directory, worktree }) => {
  return {
    event: async ({ event }) => {
      // Send notification on session completion
      if (event.type === "session.idle") {
        await $`osascript -e 'display notification "Session completed!" with title "nexus"'`
      }
    },
  }
}
...
:::note
Si está utilizando la aplicación de escritorio NEXUS, puede enviar notificaciones del sistema automáticamente cuando una respuesta esté lista o cuando se produzca un error en una sesión.
:::
...
Evite que nexus lea archivos `.env`:

```javascript title=".nexus/plugins/env-protection.js"
...
```javascript title=".nexus/plugins/inject-env.js"
...
Los complementos también pueden agregar herramientas personalizadas a nexus:

```ts title=".nexus/plugins/custom-tools.ts"
import { type Plugin, tool } from "@nexus-ai/plugin"
...
El ayudante `tool` crea una herramienta personalizada a la que nexus puede llamar. Toma una función de esquema Zod y devuelve una definición de herramienta con:
...
Sus herramientas personalizadas estarán disponibles para nexus junto con las herramientas integradas.
...
Niveles: `debug`, `info`, `warn`, `error`. Consulte la [documentación del SDK](https://nexus.ai/docs/sdk) para obtener más detalles.
...
```ts title=".nexus/plugins/compaction.ts"
import type { Plugin } from "@nexus-ai/plugin"
...
```ts title=".nexus/plugins/custom-compaction.ts"
import type { Plugin } from "@nexus-ai/plugin"
