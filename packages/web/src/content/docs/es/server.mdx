---
title: Servidor
description: Interactuar con el servidor nexus a través de HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

El comando `nexus serve` ejecuta un servidor HTTP sin cabeza que expone un punto final OpenAPI que un cliente nexus puede usar.
...
| `--mdns-domain` | Nombre de dominio personalizado para el servicio mDNS | `nexus.local` |
...
Configure `NEXUS_SERVER_PASSWORD` para proteger el servidor con autenticación básica HTTP. El nombre de usuario predeterminado es `nexus`, o configure `NEXUS_SERVER_USERNAME` para anularlo. Esto se aplica tanto a `nexus serve` como a `nexus web`.
...
Cuando ejecuta `nexus`, inicia un TUI y un servidor. Donde el TUI es el
...
:::tip
Utilice el servidor nexus para interactuar con nexus mediante programación.
:::

Esta arquitectura permite que nexus admita múltiples clientes y le permite interactuar con nexus mediante programación.

Puede ejecutar `nexus serve` para iniciar un servidor independiente. Si tienes el
nexus TUI ejecutándose, `nexus serve` iniciará un nuevo servidor.
...
El punto final [`/tui`](#tui) se puede utilizar para conducir el TUI a través del servidor. Por ejemplo, puede completar previamente o ejecutar un mensaje. Esta configuración es utilizada por los complementos NEXUS [IDE](/docs/ide).
...
El servidor nexus expone las siguientes API.
