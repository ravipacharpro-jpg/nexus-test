...
Las habilidades del agente permiten a NEXUS descubrir instrucciones reutilizables de su repositorio o directorio de inicio.
...
NEXUS busca estas ubicaciones:

- Configuración del proyecto: `.nexus/skills/<name>/SKILL.md`
- Configuración global: `~/.config/nexus/skills/<name>/SKILL.md`
...
Para las rutas locales del proyecto, NEXUS sube desde su directorio de trabajo actual hasta llegar al árbol de trabajo de git.
Carga cualquier `skills/*/SKILL.md` coincidente en `.nexus/` y cualquier `.claude/skills/*/SKILL.md` o `.agents/skills/*/SKILL.md` coincidente a lo largo del camino.

Las definiciones globales también se cargan desde `~/.config/nexus/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` y `~/.agents/skills/*/SKILL.md`.
...
Crea `.nexus/skills/git-release/SKILL.md` así:
...
NEXUS enumera las habilidades disponibles en la descripción de la herramienta `skill`.
...
Controle a qué agentes de habilidades pueden acceder utilizando permisos basados ​​en patrones en `nexus.json`:
...
**Para agentes integrados** (en `nexus.json`):
