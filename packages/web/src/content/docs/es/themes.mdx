...
Con NEXUS puede seleccionar uno de varios temas integrados, usar un tema que se adapte al tema de su terminal o definir su propio tema personalizado.

De forma predeterminada, NEXUS usa nuestro propio tema `nexus`.
...
NEXUS viene con varios temas integrados.
...
- Quiere que NEXUS coincida con la apariencia de su terminal
...
NEXUS admite un sistema de temas flexible basado en JSON que permite a los usuarios crear y personalizar temas fácilmente.
...
2. **Directorio de configuración de usuario**: definido en `~/.config/nexus/themes/*.json` o `$XDG_CONFIG_HOME/nexus/themes/*.json`
3. **Directorio raíz del proyecto**: definido en `<project-root>/.nexus/themes/*.json`
4. **Directorio de trabajo actual** - Definido en `./.nexus/themes/*.json`
