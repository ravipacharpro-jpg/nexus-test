...
Las herramientas permiten que LLM realice acciones en su código base. NEXUS viene con un conjunto de herramientas integradas, pero puede ampliarlo con [herramientas personalizadas](/docs/custom-tools) o [servidores MCP](/docs/mcp-servers).
...
Aquí están todas las herramientas integradas disponibles en NEXUS.
...
:::note
Esta herramienta solo está disponible cuando `NEXUS_EXPERIMENTAL_LSP_TOOL=true` (o `NEXUS_EXPERIMENTAL=true`).
:::
...
:::note
Esta herramienta solo está disponible cuando se utiliza el proveedor NEXUS o cuando la variable de entorno `NEXUS_ENABLE_EXA` está configurada en cualquier valor verdadero (por ejemplo, `true` o `1`).

Para habilitar al iniciar NEXUS:

```bash
NEXUS_ENABLE_EXA=1 nexus
```
