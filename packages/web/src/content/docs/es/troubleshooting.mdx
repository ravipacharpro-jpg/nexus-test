...
Para depurar problemas con NEXUS, comience verificando los registros y los datos locales que almacena en el disco.
...
Puede configurar el nivel de registro con la opción de línea de comandos `--log-level` para obtener información de depuración más detallada. Por ejemplo, `nexus --log-level DEBUG`.

---

## Almacenamiento

nexus almacena datos de sesión y otros datos de aplicaciones en el disco en:
...
NEXUS Desktop ejecuta un servidor NEXUS local (el sidecar `nexus-cli`) en segundo plano. La mayoría de los problemas se deben a un complemento que no funciona correctamente, una memoria caché dañada o una mala configuración del servidor.
...
- Solo macOS: menú `NEXUS` -> **Recargar vista web** (ayuda si la interfaz de usuario está en blanco/congelada).
...
- **macOS/Linux**: `~/.config/nexus/nexus.jsonc` (o `~/.config/nexus/nexus.json`)
- **macOS/Linux** (instalaciones anteriores): `~/.local/share/nexus/nexus.jsonc`
...
NEXUS también puede cargar complementos locales desde el disco. Quítelos temporalmente del camino (o cambie el nombre de la carpeta) y reinicie la aplicación de escritorio:
...
Si deshabilitar los complementos no ayuda (o la instalación de un complemento está bloqueada), borre el caché para que NEXUS pueda reconstruirlo.

1. Salga de NEXUS Desktop por completo.
2. Elimine el directorio de caché:

- **macOS**: Buscador -> `Cmd+Shift+G` -> pegar `~/.cache/nexus`
- **Linux**: eliminar `~/.cache/nexus` (o ejecutar `rm -rf ~/.cache/nexus`)
- **Windows**: Presione `WIN+R` y pegue `%USERPROFILE%\.cache\nexus`

3. Reinicie el escritorio NEXUS.

---

### Solucionar problemas de conexión del servidor

NEXUS Desktop puede iniciar su propio servidor local (predeterminado) o conectarse a la URL de un servidor que haya configurado.
...
Si su `nexus.json(c)` contiene una sección `server`, elimínela temporalmente y reinicie la aplicación de escritorio.

#### Verificar variables de entorno

Si tiene `NEXUS_PORT` configurado en su entorno, la aplicación de escritorio intentará usar ese puerto para el servidor local.

- Desarme `NEXUS_PORT` (o elija un puerto libre) y reinicie.
...
En Windows, el escritorio NEXUS requiere Microsoft Edge **WebView2 Runtime**. Si la aplicación se abre en una ventana en blanco o no se inicia, instale/actualice WebView2 e inténtelo nuevamente.

---

### Windows: Problemas generales de rendimiento

Si tiene un rendimiento lento, problemas de acceso a archivos o problemas de terminal en Windows, intente usar [WSL (Windows Subsistema para Linux)](/docs/windows-wsl). WSL proporciona un entorno Linux que funciona de manera más fluida con las funciones de NEXUS.

---

### Notificaciones que no se muestran

NEXUS Desktop solo muestra notificaciones del sistema cuando:

- las notificaciones están habilitadas para NEXUS en la configuración de su sistema operativo, y
...
1. Salga del escritorio NEXUS.
2. Busque y elimine estos archivos (se encuentran en el directorio de datos de la aplicación de escritorio NEXUS):

- `nexus.settings.dat` (URL del servidor predeterminado de escritorio)
- `nexus.global.dat` y `nexus.workspace.*.dat` (estado de la interfaz de usuario como servidores/proyectos recientes)
...
3. Asegúrese de tener la última versión con `nexus upgrade`
...
Para saber a qué modelos tiene acceso, ejecute `nexus models`
...
Si encuentra errores de llamada API, esto puede deberse a paquetes de proveedores obsoletos. nexus instala dinámicamente paquetes de proveedores (OpenAI, Anthropic, Google, etc.) según sea necesario y los almacena en caché localmente.
...
2. Reinicie nexus para reinstalar los paquetes del proveedor más recientes.

Esto obligará a nexus a descargar las versiones más recientes de los paquetes del proveedor, lo que a menudo resuelve problemas de compatibilidad con los parámetros del modelo y los cambios de API.
...
nexus detectará si estás usando Wayland y prefieres `wl-clipboard`; de lo contrario, intentará encontrar herramientas del portapapeles en el orden de: `xclip` y `xsel`.
