...
Vous pouvez ajouter des commandes personnalisées via la configuration NEXUS ou en créant des fichiers markdown dans le répertoire `commands/`.

---

### JSON

Utilisez l'option `command` dans votre NEXUS [config](/docs/config) :
...
nexus comprend plusieurs commandes intégrées comme `/init`, `/undo`, `/redo`, `/share`, `/help` ; [en savoir plus](/docs/tui#commands).
