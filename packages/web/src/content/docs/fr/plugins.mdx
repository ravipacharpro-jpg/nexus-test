---
title: Plugins
description: Écrivez vos propres plugins pour étendre NEXUS.
---

Les plugins vous permettent d'étendre NEXUS en vous connectant à divers événements et en personnalisant le comportement. Vous pouvez créer des plugins pour ajouter de nouvelles fonctionnalités, intégrer des services externes ou modifier le comportement par défaut de NEXUS.
...
- `.nexus/plugins/` - Plugins au niveau du projet
- `~/.config/nexus/plugins/` - Plugins globaux
...
Les **plugins npm** sont installés automatiquement à l'aide de Bun au démarrage. Les packages et leurs dépendances sont mis en cache dans `~/.cache/nexus/node_modules/`.
...
1. Configuration globale (`~/.config/nexus/nexus.json`)
2. Configuration du projet (`nexus.json`)
3. Répertoire global des plugins (`~/.config/nexus/plugins/`)
4. Répertoire des plugins du projet (`.nexus/plugins/`)
...
NEXUS exécute `bun install` au démarrage pour les installer. Vos plugins et outils peuvent ensuite les importer.
...
- `client` : un client SDK nexus pour interagir avec l'IA.
...
Voici quelques exemples de plugins que vous pouvez utiliser pour étendre nexus.
...
:::note
Si vous utilisez l'application de bureau NEXUS, elle peut envoyer automatiquement des notifications système lorsqu'une réponse est prête ou en cas d'erreur de session.
:::
...
Empêchez nexus de lire les fichiers `.env` :
...
Les plugins peuvent également ajouter des outils personnalisés à nexus :
...
L'assistant `tool` crée un outil personnalisé que nexus peut appeler. Il prend une fonction de schéma Zod et renvoie une définition d'outil avec :
...
Vos outils personnalisés seront disponibles pour nexus aux côtés des outils intégrés.
...
Niveaux : `debug`, `info`, `warn`, `error`. Voir la [documentation du SDK](https://nexus.ai/docs/sdk) pour plus de détails.
