---
title: SDK
description: Client JS de type sécurisé pour le serveur nexus.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

Le SDK nexus JS/TS fournit un client de type sécurisé pour interagir avec le serveur.
Utilisez-le pour créer des intégrations et contrôler nexus par programme.
...
Créez une instance de nexus :

```javascript
import { createNexus } from "@nexus-ai/sdk"

const { client } = await createNexus()
```
...
Vous pouvez transmettre un objet de configuration pour personnaliser le comportement. L'instance récupère toujours votre `nexus.json`, mais vous pouvez remplacer ou ajouter une configuration en ligne :
...
console.log(`Server running at ${nexus.server.url}`)
...
Si vous disposez déjà d'une instance en cours d'exécution de nexus, vous pouvez créer une instance client pour vous y connecter :
...
```typescript
import type { Session, Message, Part } from "@nexus-ai/sdk"
```
