---
title: Serveur
description: Interagissez avec le serveur nexus via HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

La commande `nexus serve` exécute un serveur HTTP sans tête qui expose un point de terminaison OpenAPI qu'un client nexus peut utiliser.
...
| `--mdns-domain` | Nom de domaine personnalisé pour le service mDNS   | `nexus.local` |
...
Définissez `NEXUS_SERVER_PASSWORD` pour protéger le serveur avec l'authentification de base HTTP. Le nom d'utilisateur est par défaut `nexus`, ou définissez `NEXUS_SERVER_USERNAME` pour le remplacer. Cela s’applique à la fois à `nexus serve` et à `nexus web`.
...
Lorsque vous exécutez `nexus`, il démarre un TUI et un serveur. Où le TUI est le
...
:::tip
Utilisez le serveur nexus pour interagir avec nexus par programmation.
:::

Cette architecture permet à nexus de prendre en charge plusieurs clients et vous permet d'interagir avec nexus par programme.

Vous pouvez exécuter `nexus serve` pour démarrer un serveur autonome. Si vous avez le
nexus TUI en cours d'exécution, `nexus serve` démarrera un nouveau serveur.
...
Le point de terminaison [`/tui`](#tui) peut être utilisé pour piloter le TUI via le serveur. Par exemple, vous pouvez pré-remplir ou exécuter une invite. Cette configuration est utilisée par les plugins NEXUS [IDE](/docs/ide).
...
Le serveur nexus expose les API suivantes.
