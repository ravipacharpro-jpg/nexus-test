...
Avec NEXUS, vous pouvez choisir parmi plusieurs thèmes intégrés, utiliser un thème qui s'adapte au thème de votre terminal ou définir votre propre thème personnalisé.

Par défaut, NEXUS utilise notre propre thème `nexus`.
...
NEXUS est livré avec plusieurs thèmes intégrés.
...
- Vous souhaitez que NEXUS corresponde à l'apparence de leur terminal
...
NEXUS prend en charge un système de thèmes flexible basé sur JSON qui permet aux utilisateurs de créer et de personnaliser facilement des thèmes.
...
2. **Répertoire de configuration utilisateur** - Défini dans `~/.config/nexus/themes/*.json` ou `$XDG_CONFIG_HOME/nexus/themes/*.json`
3. **Répertoire racine du projet** - Défini dans `<project-root>/.nexus/themes/*.json`
4. **Répertoire de travail actuel** - Défini dans `./.nexus/themes/*.json`
...
```bash no-frame
mkdir -p ~/.config/nexus/themes
vim ~/.config/nexus/themes/my-theme.json
```

Et pour des thèmes spécifiques au projet.

```bash no-frame
mkdir -p .nexus/themes
vim .nexus/themes/my-theme.json
```
...
{
  "$schema": "https://nexus.ai/theme.json",
  "defs": {
    "nord0": "#2E3440",
    "nord1": "#3B4252",
    "nord2": "#434C5E",
