...
Pour déboguer les problèmes avec NEXUS, commencez par vérifier les journaux et les données locales qu'il stocke sur le disque.
...
Vous pouvez définir le niveau de journalisation avec l'option de ligne de commande `--log-level` pour obtenir des informations de débogage plus détaillées. Par exemple, `nexus --log-level DEBUG`.

---

## Stockage

nexus stocke les données de session et autres données d'application sur le disque à l'emplacement :
...
NEXUS Desktop exécute un serveur NEXUS local (le side-car `nexus-cli`) en arrière-plan. La plupart des problèmes sont causés par un plugin qui se comporte mal, un cache corrompu ou un mauvais paramètre du serveur.
...
- macOS uniquement : menu `NEXUS` -> **Recharger la vue Web** (aide si l'interface utilisateur est vide/gelée).
...
- **macOS/Linux** (anciennes installations) : `~/.local/share/nexus/nexus.jsonc`
...
NEXUS peut également charger des plugins locaux à partir du disque. Écartez-les temporairement (ou renommez le dossier) et redémarrez l'application de bureau :
...
Si la désactivation des plugins ne résout pas le problème (ou si l'installation d'un plugin est bloquée), videz le cache afin que NEXUS puisse le reconstruire.

1. Quittez complètement NEXUS Desktop.
2. Supprimez le répertoire cache :

- **macOS** : Finder -> `Cmd+Shift+G` -> coller `~/.cache/nexus`
- **Linux** : supprimez `~/.cache/nexus` (ou exécutez `rm -rf ~/.cache/nexus`)
- **Windows** : appuyez sur `WIN+R` et collez `%USERPROFILE%\.cache\nexus`

3. Redémarrez le bureau NEXUS.

---

### Résoudre les problèmes de connexion au serveur

NEXUS Desktop peut soit démarrer son propre serveur local (par défaut), soit se connecter à un serveur URL que vous avez configuré.
...
Si votre `nexus.json(c)` contient une section `server`, supprimez-la temporairement et redémarrez l'application de bureau.

#### Vérifier les variables d'environnement

Si `NEXUS_PORT` est défini dans votre environnement, l'application de bureau tentera d'utiliser ce port pour le serveur local.

- Désactivez `NEXUS_PORT` (ou choisissez un port libre) et redémarrez.
...
Sur Windows, NEXUS Desktop nécessite Microsoft Edge **WebView2 Runtime**. Si l'application s'ouvre sur une fenêtre vide ou ne démarre pas, installez/mettez à jour WebView2 et réessayez.
...
NEXUS Desktop affiche uniquement les notifications système lorsque :

- les notifications sont activées pour NEXUS dans les paramètres de votre système d'exploitation, et
...
1. Quittez le bureau NEXUS.
2. Recherchez et supprimez ces fichiers (ils se trouvent dans le répertoire de données de l'application NEXUS Desktop) :

- `nexus.settings.dat` (serveur de bureau par défaut URL)
- `nexus.global.dat` et `nexus.workspace.*.dat` (état de l'interface utilisateur comme les serveurs/projets récents)
...
Si vous rencontrez des problèmes avec NEXUS :
...
3. Assurez-vous d'avoir la dernière version avec `nexus upgrade`
...
Pour déterminer à quels modèles vous avez accès, exécutez `nexus models`
...
Si vous rencontrez des erreurs d’appel API, cela peut être dû à des packages de fournisseurs obsolètes. nexus installe dynamiquement les packages du fournisseur (OpenAI, Anthropic, Google, etc.) selon les besoins et les met en cache localement.
...
2. Redémarrez nexus pour réinstaller les derniers packages du fournisseur

Cela forcera nexus à télécharger les versions les plus récentes des packages du fournisseur, ce qui résout souvent les problèmes de compatibilité avec les paramètres du modèle et les modifications de API.
...
nexus détectera si vous utilisez Wayland et préférez `wl-clipboard`, sinon il essaiera de trouver les outils du presse-papiers dans l'ordre : `xclip` et `xsel`.
