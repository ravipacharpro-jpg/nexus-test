...
Puoi aggiungere comandi personalizzati tramite la config di NEXUS o creando file markdown nella directory `commands/`.

---

### JSON

Usa l'opzione `command` nella [config](/docs/config) di NEXUS:
...
- Globale: `~/.config/nexus/commands/`
- Per progetto: `.nexus/commands/`
...
nexus include diversi comandi integrati come `/init`, `/undo`, `/redo`, `/share`, `/help`; [scopri di piu](/docs/tui#commands).
