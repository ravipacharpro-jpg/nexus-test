---
title: Server LSP
description: NEXUS si integra con i tuoi server LSP.
---

NEXUS può integrarsi con server Language Server Protocol (LSP) per usare la diagnostica come feedback per l'agente.

---

## Integrati

NEXUS include diversi server LSP integrati per linguaggi popolari:
...
:::note
Puoi disabilitare il download automatico dei server LSP impostando la variabile d'ambiente `NEXUS_DISABLE_LSP_DOWNLOAD` a `true`.
:::
...
Quando LSP è abilitato e nexus apre un file:
...
Puoi abilitare e personalizzare i server LSP tramite la sezione `lsp` nella configurazione di nexus.
