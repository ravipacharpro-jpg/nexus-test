...
Puoi aggiungere strumenti esterni a NEXUS usando il _Model Context Protocol_, o MCP. NEXUS supporta sia server locali sia remoti.
...
Puoi definire server MCP nella tua [NEXUS Config](https://nexus.ai/docs/config/) sotto `mcp`. Aggiungi ogni MCP con un nome univoco. Puoi riferirti a quell'MCP per nome quando fai prompt all'LLM.
...
Le organizzazioni possono fornire server MCP predefiniti tramite l'endpoint `.well-known/nexus`. Questi server possono essere disabilitati di default, lasciando agli utenti la possibilita' di fare opt-in su quelli necessari.
...
NEXUS gestisce automaticamente l'autenticazione OAuth per i server MCP remoti. Quando un server richiede autenticazione, NEXUS:
...
Se il server richiede autenticazione, NEXUS ti chiedera' di autenticarti la prima volta che provi a usarlo. In caso contrario, puoi [attivare manualmente il flusso](#authenticating) con `nexus mcp auth <server-name>`.
...
Il comando `mcp auth` aprira' il browser per l'autorizzazione. Dopo l'autorizzazione, NEXUS memorizzera' i token in modo sicuro in `~/.local/share/nexus/mcp-auth.json`.
...
I tuoi MCP sono disponibili come strumenti in NEXUS insieme agli strumenti integrati. Quindi puoi gestirli tramite la configurazione NEXUS come qualunque altro strumento.
...
Questo aprira' una finestra del browser per completare il flusso OAuth e collegare NEXUS al tuo account Sentry.
