---
title: Plugin
description: Scrivi plugin per estendere NEXUS.
---

I plugin ti permettono di estendere NEXUS agganciandoti a vari eventi e personalizzando il comportamento. Puoi creare plugin per aggiungere nuove funzionalità, integrare servizi esterni o modificare il comportamento predefinito di NEXUS.
...
- `.nexus/plugins/` - plugin a livello progetto
- `~/.config/nexus/plugins/` - plugin globali
...
I **plugin npm** vengono installati automaticamente usando Bun all'avvio. I pacchetti e le loro dipendenze vengono cache-ati in `~/.cache/nexus/node_modules/`.
...
1. Config globale (`~/.config/nexus/nexus.json`)
2. Config di progetto (`nexus.json`)
3. Directory plugin globale (`~/.config/nexus/plugins/`)
4. Directory plugin di progetto (`.nexus/plugins/`)
...
NEXUS esegue `bun install` all'avvio per installarle. I tuoi plugin e strumenti potranno poi importarle.
...
- `client`: un client SDK di nexus per interagire con l'AI.
...
Ecco alcuni esempi di plugin che puoi usare per estendere nexus.
...
:::note
Se usi l'app desktop di NEXUS, può inviare automaticamente notifiche di sistema quando una risposta è pronta o quando una sessione va in errore.
:::
...
Impedisci a nexus di leggere i file `.env`:
...
I plugin possono anche aggiungere strumenti personalizzati a nexus:
...
L'helper `tool` crea uno strumento personalizzato che nexus può chiamare. Accetta una funzione di schema Zod e restituisce una definizione di tool con:
...
I tuoi strumenti personalizzati saranno disponibili in nexus insieme agli strumenti integrati.
...
Livelli: `debug`, `info`, `warn`, `error`. Vedi la [documentazione SDK](https://nexus.ai/docs/sdk) per i dettagli.
