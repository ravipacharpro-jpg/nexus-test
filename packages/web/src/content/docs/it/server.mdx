---
title: Server
description: Interagisci con il server nexus via HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

Il comando `nexus serve` avvia un server HTTP headless che espone un endpoint OpenAPI utilizzabile da un client nexus.
...
| `--mdns-domain` | Nome di dominio personalizzato mDNS     | `nexus.local` |
...
Imposta `NEXUS_SERVER_PASSWORD` per proteggere il server con HTTP basic auth. Lo username predefinito e `nexus`, oppure imposta `NEXUS_SERVER_USERNAME` per sovrascriverlo. Questo vale sia per `nexus serve` sia per `nexus web`.
...
Quando esegui `nexus` avvia una TUI e un server. La TUI e il client che parla col server. Il server espone un endpoint con specifica OpenAPI 3.1. Questo endpoint viene anche usato per generare un [SDK](/docs/sdk).

:::tip
Usa il server nexus per interagire con nexus in modo programmatico.
:::

Questa architettura permette a nexus di supportare piu client e di essere usato in modo programmatico.

Puoi eseguire `nexus serve` per avviare un server standalone. Se la TUI di nexus e gia in esecuzione, `nexus serve` avviera un nuovo server.
...
L'endpoint [`/tui`](#tui) puo essere usato per pilotare la TUI tramite il server. Per esempio, puoi precompilare o eseguire un prompt. Questa configurazione e usata dai plugin [IDE](/docs/ide) di NEXUS.
...
Il server nexus espone le seguenti API.
