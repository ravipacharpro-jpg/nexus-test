...
Le skill degli agenti permettono a NEXUS di individuare istruzioni riutilizzabili dal tuo repo o dalla home directory.
...
NEXUS cerca in queste posizioni:

- Config di progetto: `.nexus/skills/<name>/SKILL.md`
- Config globale: `~/.config/nexus/skills/<name>/SKILL.md`
...
Per i percorsi locali al progetto, NEXUS risale dalla directory di lavoro corrente finche' non raggiunge il worktree git.
Carica qualsiasi `skills/*/SKILL.md` corrispondente in `.nexus/` e qualsiasi `.claude/skills/*/SKILL.md` o `.agents/skills/*/SKILL.md` corrispondente lungo il percorso.

Le definizioni globali vengono caricate anche da `~/.config/nexus/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` e `~/.agents/skills/*/SKILL.md`.
...
Crea `.nexus/skills/git-release/SKILL.md` cosi':
...
NEXUS elenca le skill disponibili nella descrizione dello strumento `skill`.
...
Controlla a quali skill gli agenti possono accedere usando permessi basati su pattern in `nexus.json`:
...
**Per agenti integrati** (in `nexus.json`):
