...
Per diagnosticare problemi con NEXUS, inizia controllando i log e i dati locali che salva su disco.
...
Puoi impostare il livello di log con l'opzione a riga di comando `--log-level` per ottenere informazioni di debug piu dettagliate. Per esempio: `nexus --log-level DEBUG`.

---

## Archiviazione

nexus salva i dati delle sessioni e altri dati dell'applicazione su disco in:
...
NEXUS Desktop esegue in background un server locale di NEXUS (il sidecar `nexus-cli`). La maggior parte dei problemi e causata da un plugin che si comporta male, da una cache corrotta o da un'impostazione del server errata.
...
- Solo macOS: menu `NEXUS` -> **Reload Webview** (utile se l'interfaccia e vuota o bloccata).
...
- **macOS/Linux** (installazioni vecchie): `~/.local/share/nexus/nexus.jsonc`
...
NEXUS puo anche caricare plugin locali dal disco. Spostali temporaneamente altrove (o rinomina la cartella) e riavvia l'app desktop:
...
Se disabilitare i plugin non aiuta (o l'installazione di un plugin e bloccata), svuota la cache in modo che NEXUS possa ricostruirla.

1. Chiudi completamente NEXUS Desktop.
2. Elimina la directory della cache:

- **macOS**: Finder -> `Cmd+Shift+G` -> paste `~/.cache/nexus`
- **Linux**: elimina `~/.cache/nexus` (oppure esegui `rm -rf ~/.cache/nexus`)
- **Windows**: premi `WIN+R` e incolla `%USERPROFILE%\.cache\nexus`

3. Riavvia NEXUS Desktop.

---

### Risolvi problemi di connessione al server

NEXUS Desktop puo avviare il proprio server locale (predefinito) oppure connettersi a un URL server che hai configurato.
...
Se il tuo `nexus.json(c)` contiene una sezione `server`, rimuovila temporaneamente e riavvia l'app desktop.

#### Controlla le variabili d'ambiente

Se hai `NEXUS_PORT` impostato nell'ambiente, l'app desktop provera a usare quella porta per il server locale.

- Rimuovi `NEXUS_PORT` (o scegli una porta libera) e riavvia.
...
Su Windows, NEXUS Desktop richiede **WebView2 Runtime** di Microsoft Edge. Se l'app si apre su una finestra vuota o non parte, installa/aggiorna WebView2 e riprova.

---

### Windows: problemi generali di prestazioni

Se riscontri prestazioni lente, problemi di accesso ai file o problemi del terminale su Windows, prova a usare [WSL (Windows Subsystem for Linux)](/docs/windows-wsl). WSL fornisce un ambiente Linux che funziona in modo piu fluido con le funzionalita di NEXUS.

---

### Notifiche non visualizzate

NEXUS Desktop mostra le notifiche di sistema solo quando:

- le notifiche sono abilitate per NEXUS nelle impostazioni del sistema operativo, e
...
1. Chiudi NEXUS Desktop.
2. Trova ed elimina questi file (si trovano nella directory dati dell'app NEXUS Desktop):

- `nexus.settings.dat` (desktop default server URL)
- `nexus.global.dat` e `nexus.workspace.*.dat` (stato dell'interfaccia come server/progetti recenti)
...
Se riscontri problemi con NEXUS:
...
3. Assicurati di avere l'ultima versione con `nexus upgrade`
...
Per capire a quali modelli hai accesso, esegui `nexus models`
...
Se incontri errori nelle chiamate API, potrebbe dipendere da pacchetti provider non aggiornati. nexus installa dinamicamente i pacchetti provider (OpenAI, Anthropic, Google, ecc.) quando servono e li mette in cache localmente.
...
2. Riavvia nexus per reinstallare i pacchetti provider piu recenti

Questo forzera nexus a scaricare le versioni piu recenti dei pacchetti provider, cosa che spesso risolve problemi di compatibilita con parametri dei modelli e cambiamenti delle API.
...
nexus rilevera se stai usando Wayland e preferira `wl-clipboard`; altrimenti provera a trovare gli strumenti per gli appunti nell'ordine: `xclip` e `xsel`.
