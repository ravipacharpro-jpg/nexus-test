---
title: サーバー
description: HTTP 経由で NEXUS サーバーと通信します。
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

`nexus serve` コマンドは、NEXUS クライアントが使用できる OpenAPI エンドポイントを公開するヘッドレス HTTP サーバーを実行します。
...
| `--mdns-domain` | mDNS サービスのカスタムドメイン名 | `nexus.local` |
...
HTTP 基本認証でサーバーを保護するには、`NEXUS_SERVER_PASSWORD` を設定します。ユーザー名はデフォルトで `nexus` になるか、`NEXUS_SERVER_USERNAME` を設定してオーバーライドします。これは、`nexus serve` と `nexus web` の両方に当てはまります。
...
`nexus` を実行すると、TUI とサーバーが起動します。 TUI はサーバーと通信するクライアントです。サーバーは OpenAPI 3.1 仕様のエンドポイントを公開します。このエンドポイントは、[SDK](/docs/sdk) を使用してアクセスできます。

:::tip
NEXUS サーバーを使用して、プログラムで NEXUS と対話します。
:::

`nexus serve` を実行してスタンドアロンサーバーを起動できます。すでに NEXUS TUI を実行している場合でも、`nexus serve` は新しいサーバーを起動します。
...
[`/tui`](#tui) エンドポイントは、サーバー経由で TUI を駆動するために使用できます。たとえば、プロンプトを事前入力したり、実行したりできます。この設定は、NEXUS [IDE](/docs/ide) プラグインによって使用されます。
