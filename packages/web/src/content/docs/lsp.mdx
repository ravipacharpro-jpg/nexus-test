---
title: LSP Servers
description: NEXUS integrates with your LSP servers.
---

NEXUS can integrate with Language Server Protocol (LSP) servers to use diagnostics as feedback for the agent.

---

## Built-in

NEXUS comes with several built-in LSP servers for popular languages:
...
:::note
You can disable automatic LSP server downloads by setting the `NEXUS_DISABLE_LSP_DOWNLOAD` environment variable to `true`.
:::
...
When LSP is enabled and nexus opens a file, it:
...
You can enable and customize LSP servers through the `lsp` section in your nexus config.
