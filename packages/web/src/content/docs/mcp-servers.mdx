...
You can add external tools to NEXUS using the _Model Context Protocol_, or MCP. NEXUS supports both local and remote servers.
...
You can define MCP servers in your [NEXUS Config](https://nexus.ai/docs/config/) under `mcp`. Add each MCP with a unique name. You can refer to that MCP by name when prompting the LLM.
...
Organizations can provide default MCP servers via their `.well-known/nexus` endpoint. These servers may be disabled by default, allowing users to opt-in to the ones they need.
...
NEXUS automatically handles OAuth authentication for remote MCP servers. When a server requires authentication, NEXUS will:
...
If the server requires authentication, NEXUS will prompt you to authenticate when you first try to use it. If not, you can [manually trigger the flow](#authenticating) with `nexus mcp auth <server-name>`.
...
The `mcp auth` command will open your browser for authorization. After you authorize, NEXUS will store the tokens securely in `~/.local/share/nexus/mcp-auth.json`.
...
Your MCPs are available as tools in NEXUS, alongside built-in tools. So you can manage them through the NEXUS config like any other tool.
...
This will open a browser window to complete the OAuth flow and connect NEXUS to your Sentry account.
