...
Opprett `.nexus/commands/test.md`:
...
Du kan legge til egendefinerte kommandoer gjennom NEXUS-konfigurasjonen eller ved å lage markdown-filer i katalogen `commands/`.

---

### JSON

Bruk alternativet `command` i NEXUS [config](/docs/config):
...
NEXUS inkluderer flere innebygde kommandoer som `/init`, `/undo`, `/redo`, `/share`, `/help`; [finn ut mer](/docs/tui#commands).
