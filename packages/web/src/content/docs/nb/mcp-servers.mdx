...
Du kan legge til eksterne verktøy i NEXUS ved å bruke _Model Context Protocol_ eller MCP. NEXUS støtter både lokale og eksterne servere.
...
Du kan definere MCP-servere i [NEXUS Config](https://nexus.ai/docs/config/) under `mcp`. Legg til hver MCP med et unikt navn. Du kan referere til den MCP ved navn når du spør LLM-en.

```jsonc title="nexus.jsonc" {6}
{
...
Organisasjoner kan tilby standard MCP-servere via deres `.well-known/nexus`-endepunkt. Disse serverne kan være deaktivert som standard, slik at brukere kan melde seg på de de trenger.
...
NEXUS håndterer automatisk OAuth-autentisering for eksterne MCP-servere. Når en server krever autentisering, vil NEXUS:
...
Hvis serveren krever autentisering, vil NEXUS be deg om å autentisere når du prøver å bruke den for første gang. Hvis ikke, kan du [utløse flyten manuelt](#authenticating) med `nexus mcp auth <server-name>`.
...
```bash
nexus mcp auth my-oauth-server
```
...
```bash
nexus mcp logout my-oauth-server
```

Kommandoen `mcp auth` åpner nettleseren din for autorisasjon. Etter at du har godkjent, vil NEXUS lagre tokens sikkert i `~/.local/share/nexus/mcp-auth.json`.
...
nexus mcp debug my-oauth-server
```
...
MCP-ene dine er tilgjengelige som verktøy i NEXUS, sammen med innebygde verktøy. Så du kan administrere dem gjennom NEXUS-konfigurasjonen som alle andre verktøy.
...
```json title="nexus.json" {11, 14-18}
{
  "$schema": "https://nexus.ai/config.json",
  "mcp": {
    "my-mcp": {
...
```bash
nexus mcp auth sentry
```

Dette åpner et nettleservindu for å fullføre OAuth-flyten og koble NEXUS til Sentry-kontoen din.
...
Programtillegget injiserer `Helicone-Session-Id` og `Helicone-Session-Name` overskrifter i forespørslene dine. På Helicones Sessions-side vil du se hver NEXUS-samtale oppført som en separat økt.
