...
description: Skriv dine egne programtillegg for å utvide NEXUS.
---

Plugins lar deg utvide NEXUS ved å koble til ulike hendelser og tilpasse atferd. Du kan lage plugins for å legge til nye funksjoner, integrere med eksterne tjenester eller endre standardoppførselen til NEXUS.
...
- `.nexus/plugins/` - Programtillegg på prosjektnivå
...
**npm-plugins** installeres automatisk ved hjelp av Bun ved oppstart. Pakker og deres avhengigheter er bufret i `~/.cache/nexus/node_modules/`.
...
1. Global konfigurasjon (`~/.config/nexus/nexus.json`)
2. Prosjektkonfigurasjon (`nexus.json`)
3. Global plugin-katalog (`~/.config/nexus/plugins/`)
4. Prosjektpluginkatalog (`.nexus/plugins/`)
...
NEXUS kjører `bun install` ved oppstart for å installere disse. Programtilleggene og verktøyene dine kan deretter importere dem.
...
- `client`: En NEXUS SDK klient for samhandling med AI.
...
Her er noen eksempler på plugins du kan bruke for å utvide NEXUS.
...
:::note
Hvis du bruker NEXUS-skrivebordsappen, kan den sende systemvarsler automatisk når et svar er klart eller når en økt feiler.
:::
...
Hindre NEXUS fra å lese `.env` filer:
...
Plugins kan også legge til egendefinerte verktøy til NEXUS:
...
`tool`-hjelperen lager et tilpasset verktøy som NEXUS kan kalle. Den tar en Zod-skjemafunksjon og returnerer en verktøydefinisjon med:

- `description`: Hva verktøyet gjør
- `args`: Zod-skjema for verktøyets argumenter
  - `execute`: Funksjon som kjører når verktøyet kalles

Dine egendefinerte verktøy vil være tilgjengelige for NEXUS sammen med innebygde verktøy.
...
Nivåer: `debug`, `info`, `warn`, `error`. Se [SDK dokumentasjon](https://nexus.ai/docs/sdk) for detaljer.
