---
title: Leverandører
description: Bruke en hvilken som helst LLM-leverandør i NEXUS.
---

import config from "../../../../config.mjs"
export const console = config.console

NEXUS bruker [AI SDK](https://ai-sdk.dev/) og [Models.dev](https://models.dev) for å støtte **75+ LLM leverandører** og den støtter kjøring av lokale modeller.

For å legge til en leverandør må du:

1. Legg til API-nøkkelen for leverandøren ved å bruke kommandoen `/connect`.
2. Konfigurer leverandøren i NEXUS-konfigurasjonen.
...
i `~/.local/share/nexus/auth.json`.

---

### Konfigurasjon

Du kan tilpasse leverandørene gjennom `provider`-delen i NEXUS
konfig.
...
NEXUS Zen er en liste over modeller levert av NEXUS-teamet som har vært
testet og verifisert for å fungere godt med NEXUS. [Finn ut mer](/docs/zen).

:::tip
Hvis du er ny, anbefaler vi å starte med NEXUS Zen.
:::

1. Kjør kommandoen `/connect` i TUI, velg `NEXUS Zen` og gå til [nexus.ai/auth](https://nexus.ai/zen).
...
Det fungerer som alle andre leverandører i NEXUS og er helt valgfritt å bruke.

---

## NEXUS Go

NEXUS Go er en lavpris abonnementsplan som gir pålitelig tilgang til populære åpne kodemodeller levert av NEXUS-teamet som har vært
testet og verifisert for å fungere godt med NEXUS.

1. Kjør kommandoen `/connect` i TUI, velg `NEXUS Go`, og gå til [nexus.ai/auth](https://nexus.ai/zen).
...
Slik bruker du Amazon Bedrock med NEXUS:
...
2. **Konfigurer autentisering** ved å bruke en av følgende metoder:
   ...
   Angi en av disse miljøvariablene mens du kjører NEXUS:
   ...
   For prosjektspesifikk eller vedvarende konfigurasjon, bruk `nexus.json`:
...
:::info
Å bruke Claude Pro/Max-abonnementet ditt i nexus støttes ikke offisielt av [Anthropic](https://anthropic.com).
:::
...
Du kan konfigurere nexus til å bruke lokale modeller via [Atomic Chat](https://atomic.chat) — et skrivebordsprogram som kjører lokale LLM-er bak en OpenAI-kompatibel API-server (standard endepunkt `http://127.0.0.1:1337/v1`).
...
2. Gå til [Azure AI Foundry](https://ai.azure.com/) og distribuer en modell.
   ...
   :::note
   Distribusjonsnavnet må samsvare med modellnavnet for at NEXUS skal fungere skikkelig.
   :::
...
5. Kjør kommandoen `/models` for å velge en modell.
   ...
   Du kan også legge til modeller gjennom NEXUS-konfigurasjonen.
...
for å lagre token i NEXUS auth-lagring.
:::

##### Selvhostet GitLab

:::note[Merknad om samsvar]
NEXUS bruker en liten modell for noen AI oppgaver som å generere økttittelen.
Den er konfigurert til å bruke gpt-5-nano som standard, hostet av Zen. For å låse NEXUS
for kun å bruke din egen GitLab-hostede forekomst, legg til følgende i din
`nexus.json` fil. Det anbefales også å deaktivere øktdeling.
...
Mer dokumentasjon på [nexus-gitlab-auth](https://www.npmjs.com/package/nexus-gitlab-auth) hjemmeside.

##### Konfigurasjon

Tilpass gjennom `nexus.json`:
...
Slik bruker du GitHub Copilot-abonnementet med NEXUS:
...
Slik bruker du Google Vertex AI med NEXUS:
...
I tilfelle du ser en funksjon eller modell fra Helicone som ikke konfigureres automatisk gjennom NEXUS, kan du alltid konfigurere den selv.
...
Helicones [Sessions](https://docs.helicone.ai/features/sessions) funksjon lar deg gruppere relaterte LLM forespørsler sammen. Bruk [nexus-helicone-session](https://github.com/H2Shami/nexus-helicone-session) plugin for automatisk å logge hver NEXUS-samtale som en økt i Helicone.
...
Programtillegget injiserer `Helicone-Session-Id` og `Helicone-Session-Name` headere i forespørslene dine. På Helicones Sessions-side vil du se hver NEXUS-samtale oppført som en separat økt.
...
Du kan konfigurere NEXUS for å bruke lokale modeller gjennom [llama.cpp](https://github.com/ggml-org/llama.cpp) llama-server-verktøyet.
...
Du kan konfigurere NEXUS for å bruke lokale modeller gjennom LM Studio.
...
Du kan konfigurere NEXUS for å bruke lokale modeller gjennom Ollama.

:::tip
Ollama kan automatisk konfigurere seg selv for NEXUS. Se [Ollama-integrasjonsdokumentene](https://docs.ollama.com/integrations/nexus) for detaljer.
:::
...
Slik bruker du Ollama Cloud med NEXUS:
...
3. Kopier API-nøkkelen for bruk i NEXUS.
...
6. **Viktig**: Før du bruker skymodeller i NEXUS, må du hente modellinformasjonen lokalt:
...
NEXUS Zen er en liste over testede og verifiserte modeller levert av NEXUS-teamet. [Finn ut mer](/docs/zen).

1. Logg på **<a href={console}>NEXUS Zen</a>** og klikk på **Create API Key**.

2. Kjør kommandoen `/connect` og søk etter **NEXUS Zen**.

   ```txt
   /connect
   ```

3. Skriv inn NEXUS API-nøkkelen.
...
4. Mange OpenRouter-modeller er forhåndslastet som standard, kjør kommandoen `/models` for å velge den du ønsker.
   ...
   Du kan også legge til flere modeller gjennom NEXUS-konfigurasjonen.
...
5. Du kan også tilpasse dem gjennom NEXUS-konfigurasjonen. Her er et eksempel på å spesifisere en leverandør
...
Slik bruker du [Scaleway Generative APIs](https://www.scaleway.com/en/docs/generative-apis/) med NEXUS:
...
Du kan også tilpasse modeller gjennom NEXUS-konfigurasjonen. Her er et eksempel på spesifisering av leverandørrutingsrekkefølge.
...
:::tip
Du kan bruke hvilken som helst OpenAI-kompatibel leverandør med NEXUS. De fleste moderne AI-leverandører tilbyr OpenAI-kompatible APIer.
:::
...
4. Opprett eller oppdater `nexus.json`-filen i prosjektkatalogen:
...
`limit`-feltene lar NEXUS forstå hvor mye kontekst du har igjen. Standardleverandører henter disse automatisk fra models.dev.
...
1. **Sjekk autentiseringsoppsettet**: Kjør `nexus auth list` for å se om legitimasjonen
...
2. For tilpassede leverandører, sjekk NEXUS-konfigurasjonen og:
   - Sørg for at leverandør-ID-en som brukes i `/connect`-kommandoen samsvarer med ID i NEXUS-konfigurasjonen.
