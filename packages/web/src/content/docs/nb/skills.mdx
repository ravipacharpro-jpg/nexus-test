...
Agentferdigheter lar NEXUS oppdage gjenbrukbare instruksjoner fra repo- eller hjemmekatalogen din.
...
nexus søker etter disse stedene:

- Prosjektkonfigurasjon: `.nexus/skills/<name>/SKILL.md`
- Global konfigurasjon: `~/.config/nexus/skills/<name>/SKILL.md`
...
For prosjektlokale stier går nexus opp fra din nåværende arbeidskatalog til den når git-arbeidstreet.
Den laster alle matchende `skills/*/SKILL.md` i `.nexus/` og alle matchende `.claude/skills/*/SKILL.md` eller `.agents/skills/*/SKILL.md` underveis.

Globale definisjoner lastes også inn fra `~/.config/nexus/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` og `~/.agents/skills/*/SKILL.md`.
...
Lag `.nexus/skills/git-release/SKILL.md` slik:
...
nexus viser tilgjengelige ferdigheter i `skill`-verktøybeskrivelsen.
...
Kontroller hvilke ferdigheter agenter har tilgang til ved å bruke mønsterbaserte tillatelser i `nexus.json`:
...
**For innebygde agenter** (i `nexus.json`):
