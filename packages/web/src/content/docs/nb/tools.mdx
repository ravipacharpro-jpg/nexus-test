...
Verktøy lar LLM utføre handlinger i kodebasen din. NEXUS kommer med et sett med innebygde verktøy, men du kan utvide det med [egendefinerte verktøy](/docs/custom-tools) eller [MCP servere](/docs/mcp-servers).
...
Her er alle de innebygde verktøyene som er tilgjengelige i NEXUS.
...
:::note
Dette verktøyet er bare tilgjengelig når `NEXUS_EXPERIMENTAL_LSP_TOOL=true` (eller `NEXUS_EXPERIMENTAL=true`).
:::
...
:::note
Dette verktøyet er bare tilgjengelig når du bruker NEXUS-leverandøren eller når miljøvariabelen `NEXUS_ENABLE_EXA` er satt til en sann verdi (f.eks. `true` eller `1`).

Slik aktiverer du når du starter NEXUS:
