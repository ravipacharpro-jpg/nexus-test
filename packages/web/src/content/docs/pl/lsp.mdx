---
title: Serwery LSP
description: NEXUS integruje się z serwerami LSP.
---

NEXUS może integrować się z serwerami Language Server Protocol (LSP), aby używać diagnostyki jako informacji zwrotnej dla agenta.

---

## Wbudowane

NEXUS posiada kilka wbudowanych serwerów LSP dla następujących języków:
...
:::note
Możesz wyłączyć automatyczne pobieranie serwerów LSP, ustawiając zmienną środowiskową `NEXUS_DISABLE_LSP_DOWNLOAD` na `true`.
:::
...
Gdy LSP jest włączony i NEXUS otwiera plik:
...
Możesz włączać i dostosowywać serwery LSP poprzez sekcję `lsp` w konfiguracji NEXUS.
