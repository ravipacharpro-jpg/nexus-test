...
Możesz dodać dodatkowe informacje do nexus za pomocą _Model Context Protocol_, w skrócie MCP. nexus obsługuje zarówno serwery lokalne, jak i zdalne.
...
Dostępny serwery MCP w [nexus Config](https://nexus.ai/docs/config/) w `mcp`. Dodaj każdy MCP z unikalną nazwą. Możesz zgłosić się do tego MCP po nazwie, pytając LLM.
...
Organizacje mogą udostępniać serwery MCP za pośrednictwem punktu końcowego `.well-known/nexus`. Serwer może być podłączony, dzięki czemu użytkownicy mogą wybrać te, których.
...
nexus automatyczne uwierzytelnianie OAuth dla zdalnych serwerów MCP. Gdy serwer wymaga uwierzytelnienia, nexus:
...
Jeśli serwer wymaga uwierzytelnienia, nexus poprosi Cię o uwierzytelnienie przy próbie jego użycia. Jeśli nie, możesz [ręcznie podłączyć przepływ] (#authenticating) za pomocą `nexus mcp auth <server-name>`.
...
Komenda `mcp auth` została udostępniona w celu autoryzacji. Po autoryzacji nexus bezpieczne przechowa tokeny w `~/.local/share/nexus/mcp-auth.json`.
...
Twoje MCP są dostępne jako narzędzie w nexus, obok narzędzi dodatkowych. Można to zrobić poprzez podłączenie nexus, jak również w innym przypadku.
...
Spowoduje otwarcie okna konfiguracji, w którym można zakończyć proces OAuth i połączyć nexus z kontem Sentry.
