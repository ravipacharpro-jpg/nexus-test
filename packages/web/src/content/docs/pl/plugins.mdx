---
title: Wtyczki
description: Napisz własną wtyczkę, aby rozszerzyć nexus.
---

Wtyczki umożliwiające wyłączenie nexus poprzez podjęcie różnych zdarzeń i zachowanie. Można utworzyć wtyczkę, aby sprawdzić nowe funkcje, integrować się z usługami lub sprawdzaniem zachowania nexus.
...
- `.nexus/plugins/` - Wtyczki na poziomie projektu
- `~/.config/nexus/plugins/` - Wtyczki globalne
...
**wtyczki npm** są instalowane automatycznie przy użyciu Bun podczas uruchamiania. Pakiety i ich zależności są buforowane w `~/.cache/nexus/node_modules/`.
...
1. Konfiguracja globalna (`~/.config/nexus/nexus.json`)
2. Konfiguracja projektu (`nexus.json`)
3. Globalny katalog wtyczek (`~/.config/nexus/plugins/`)
4. Katalog wtyczek projektu (`.nexus/plugins/`)
...
nexus uruchomienie `bun install` przy uruchomieniu, aby je uruchomić. Twoje dodatkowe funkcje będą dostępne po zaimportowaniu.
...
- `client`: Klient SDK z nexus do sztucznej inteligencji.
...
Oto kilka dostępnych wtyczek, które można udostępnić do nexus.
...
:::note
Jeśli korzystasz z aplikacji komputerowej nexus, może ona automatycznie wysłać wiadomość systemową, gdy odpowiedź jest gotowa lub gdy wystąpią błędy sesji.
:::
...
Zablokuj nexus czytanie plików `.env`:
...
Wtyczki mogą również dodawać niestandardowe narzędzia do nexus:
...
Pomocnik `tool` tworzy narzędzie użytkowe, które może wywołać kod nexus. Pobierane schematu Zoda i narzędzie wyszukiwania za pomocą:
...
Poziomy: `debug`, `info`, `warn`, `error`. Aby uzyskać szczegółowe informacje, zobacz [dokumentację pakietu SDK](https://nexus.ai/docs/sdk).
