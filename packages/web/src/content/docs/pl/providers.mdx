---
title: Dostawcy
description: Korzystanie z dowolnego dostawcy LLM w nexus.
---

import config from "../../../../config.mjs"
export const console = config.console

nexus używa [AI SDK](https://ai-sdk.dev/) i [Models.dev](https://models.dev) do obsługi **ponad 75 dostawców LLM** i obsługuje uruchamianie modeli lokalnych.

Aby dodać dostawcę należy:

1. Dodaj klucze API dla dostawcy za pomocą komendy `/connect`.
2. Skonfiguruj dostawcę w konfiguracji nexus.
...
w `~/.local/share/nexus/auth.json`.

---

### Konfiguracja

Możesz dostosować dostawców za pomocą sekcji `provider` w swoim nexus
config.
...
NEXUS Zen to lista modeli dostarczonych przez zespół nexus, które zostały
przetestowane i zweryfikowane, aby dobrze współpracować z nexus. [Dowiedz się więcej](/docs/zen).

:::tip
Jeśli jesteś nowy, zalecamy rozpoczęcie od NEXUS Zen.
:::

1. Uruchom polecenie `/connect` w TUI, wybierz `NEXUS Zen` i przejdź do [nexus.ai/auth](https://nexus.ai/zen).
...
Działa jak każdy inny dostawca w nexus i jest całkowicie opcjonalny w użyciu.

---

## NEXUS Go

NEXUS Go to tani plan subskrypcji, który zapewnia niezawodny dostęp do popularnych modeli open coding dostarczanych przez zespół nexus, które zostały przetestowane i zweryfikowane pod kątem dobrej współpracy z nexus.

1. Uruchom polecenie `/connect` w TUI, wybierz `NEXUS Go` i przejdź do [nexus.ai/auth](https://nexus.ai/zen).
...
Aby używać Amazon Bedrock z nexus:
...
2. **Skonfiguruj uwierzytelnianie** przy użyciu jednej z następujących metod:
   ...
   Ustaw jedną z tych zmiennych środowiskowych podczas uruchamiania nexus:
   ...
   W przypadku konfiguracji specyficznej dla projektu lub trwałej użyj `nexus.json`:
...
:::info
Korzystanie z subskrypcji Claude Pro/Max w nexus nie jest oficjalnie obsługiwane przez [Anthropic](https://anthropic.com).
:::
...
Możesz skonfigurować nexus do korzystania z modeli lokalnych przez [Atomic Chat](https://atomic.chat) — aplikację desktopową uruchamiającą lokalne LLM-y za serwerem API zgodnym z OpenAI (domyślny endpoint `http://127.0.0.1:1337/v1`).
...
2. Przejdź do [Azure AI Foundry](https://ai.azure.com/) i wdróż model.
   ...
   :::note
   Aby kod nexus działał poprawnie, nazwa wdrożenia musi być zgodna z nazwą modelu.
   :::
...
5. Uruchom komendę `/models`, aby wybrać model.
   ...
   Możesz także dodawać modele za pomocą konfiguracji nexus.
...
nexus używa małego modelu do niektórych zadań AI, takich jak generowanie tytułu sesji.
Domyślnie jest skonfigurowany do używania gpt-5-nano, hostowanego przez Zen. Aby zablokować nexus
aby używać tylko własnej instancji hostowanej przez GitLab, dodaj następujące elementy do pliku
`nexus.json` plik. Zalecane jest również wyłączenie udostępniania sesji.
...
Więcej informacji znajdziesz na stronie [nexus-gitlab-auth](https://www.npmjs.com/package/nexus-gitlab-auth).
...
Aby korzystać z subskrypcji GitHub Copilot z kodem nexus:
...
Aby używać Google Vertex AI z nexus:
...
2. Ustaw wymagane zmienne środowiskowe:
   ...
   Ustaw je podczas uruchamiania nexus.
...
Jeśli zobaczysz funkcję lub model firmy Helicone, która nie jest konfigurowana automatycznie za pomocą nexus, zawsze możesz ją skonfigurować samodzielnie.
...
Funkcja [Sesje](https://docs.helicone.ai/features/sessions) firmy Helicone umożliwia grupowanie powiązanych żądań LLM. Użyj wtyczki [nexus-helicone-session](https://github.com/H2Shami/nexus-helicone-session), aby automatycznie rejestrować każdą konwersację nexus jako sesję w Helicone.
...
Wtyczka wstawia nagłówki `Helicone-Session-Id` i `Helicone-Session-Name` do Twoich żądań. Na stronie Sesje Helicone każda konwersacja nexus będzie wymieniona jako osobna sesja.
...
Możesz skonfigurować nexus tak, aby korzystał z modeli lokalnych, za pomocą narzędzia llama-server [llama.cpp](https://github.com/ggml-org/llama.cpp)
...
Możesz skonfigurować nexus tak, aby korzystał z modeli lokalnych poprzez LM Studio.
...
Możesz skonfigurować nexus tak, aby korzystał z modeli lokalnych poprzez Ollamę.

:::tip
Ollama może automatycznie skonfigurować się pod kątem nexus. Aby uzyskać szczegółowe informacje, zobacz [dokumentację integracji Ollama](https://docs.ollama.com/integrations/nexus).
:::
...
Aby korzystać z Ollama Cloud z nexus:
...
3. Skopiuj klucz API do użycia w nexus.
...
6. **Ważne**: Przed użyciem modeli chmurowych w nexus musisz pobrać informacje o modelu lokalnie:
...
NEXUS Zen to lista przetestowanych i zweryfikowanych modeli udostępniona przez zespół nexus. [Dowiedz się więcej](/docs/zen).

1. Zaloguj się do **<a href={console}>NEXUS Zen</a>** i kliknij **Utwórz klucz API**.

2. Uruchom polecenie `/connect` i wyszukaj **NEXUS Zen**.

   ```txt
   /connect
   ```

3. Wprowadź klucz API nexus.
...
4. Wiele modeli OpenRouter jest domyślnie załadowanych fabrycznie. Uruchom komendę `/models`, aby wybrać ten, który chcesz.
   ...
   Możesz także dodać dodatkowe modele za pomocą konfiguracji nexus.
...
5. Możesz je także dostosować za pomocą konfiguracji nexus. Oto przykład określenia dostawcy
...
Aby używać [generatywnych interfejsów API Scaleway](https://www.scaleway.com/en/docs/generative-apis/) z nexus:
...
Możesz także dostosować modele za pomocą konfiguracji nexus. Oto przykład określenia kolejności routingu dostawcy.
...
:::tip
Możesz użyć dowolnego dostawcy kompatybilnego z OpenAI z nexus. Większość nowoczesnych dostawców sztucznej inteligencji oferuje interfejsy API kompatybilne z OpenAI.
:::
...
4. Utwórz lub zaktualizuj plik `nexus.json` w katalogu projektu:
...
Pola `limit` pozwalają nexus zrozumieć, ile kontekstu pozostało. Standardowi dostawcy pobierają je automatycznie z models.dev.
...
1. **Sprawdź konfigurację uwierzytelniania**: Uruchom `nexus auth list`, aby sprawdzić, czy poświadczenia
...
2. W przypadku dostawców niestandardowych sprawdź konfigurację nexus i:
   - Upewnij się, że identyfikator dostawcy użyty w poleceniu `/connect` jest zgodny z identyfikatorem w konfiguracji nexus.
