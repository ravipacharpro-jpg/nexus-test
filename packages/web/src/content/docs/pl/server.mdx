---
title: Serwer
description: Interakcja z serwerem nexus za pośrednictwem protokołu HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

Komenda `nexus serve` uruchamia bezgłowy serwer HTTP, który udostępnia punkt końcowy OpenAPI, z którego może korzystać klient nexus.
...
| `--mdns-domain` | Niestandardowa nazwa domeny dla usługi mDNS  | `nexus.local` |
...
Ustaw `NEXUS_SERVER_PASSWORD`, aby chronić serwer za pomocą podstawowego uwierzytelniania HTTP. Domyślną nazwą użytkownika jest `nexus` lub ustaw `NEXUS_SERVER_USERNAME`, aby ją zastąpić. Dotyczy to zarówno `nexus serve`, jak i `nexus web`.
...
Kiedy uruchomisz `nexus`, uruchomi się TUI i serwer. Gdzie jest TUI
...
:::tip
Użyj serwera nexus do programowej interakcji z nexus.
:::

Ta architektura umożliwia obsługę wielu klientów przez nexus i programową interakcję z kodem otwartym.

Możesz uruchomić `nexus serve`, aby uruchomić samodzielny serwer. Jeśli masz
nexus TUI działa, `nexus serve` uruchomi nowy serwer.
...
Punktu końcowego [`/tui`](#TUI) można użyć do sterowania TUI przez serwer. Można na przykład wstępnie wypełnić lub uruchomić monit. Ta konfiguracja jest używana przez wtyczki nexus [IDE](/docs/ide).
...
Serwer nexus udostępnia następujące interfejsy API.
