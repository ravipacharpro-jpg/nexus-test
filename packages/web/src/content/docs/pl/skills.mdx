...
Umiejętności agenta pozwalają nexus odkryć instrukcje wielokrotnego użytku z repozytorium lub katalogu domowego.
...
nexus przeszukuje te lokalizacje:

- Project config: `.nexus/skills/<name>/SKILL.md`
- Global config: `~/.config/nexus/skills/<name>/SKILL.md`
...
W przypadku ścieżek lokalnych projektu nexus przechodzi od bieżącego katalogu roboczego, aż dotrze do drzewa roboczego git.
Ładuje po drodze dowolne pasujące `skills/*/SKILL.md` w `.nexus/` i dowolne pasujące `.claude/skills/*/SKILL.md` lub `.agents/skills/*/SKILL.md`.

Ładowane są także definicje globalne z `~/.config/nexus/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` i `~/.agents/skills/*/SKILL.md`.
...
Utwórz `.nexus/skills/git-release/SKILL.md` w ten sposób:
...
nexus wymienia dostępne umiejętności w opisie narzędzia `skill`.
...
Kontroluj, do których umiejętności agenci mogą uzyskać dostęp, używając uprawnień opartych na wzorcach w `nexus.json`:
...
**Dla agentów wbudowanych** (w `nexus.json`):
