...
Dzięki nexus możesz wybrać jeden z kilku wbudowanych motywów, użyć motywu, który dostosowuje się do motywu terminala lub zdefiniować własny, niestandardowy motyw.

Domyślnie nexus używa naszego własnego motywu `nexus`.
...
nexus ma kilka wbudowanych motywów.
...
- Chcą, aby NEXUS pasował do wyglądu ich terminala
...
nexus obsługuje elastyczny system motywów oparty na JSON, który pozwala użytkownikom łatwo tworzyć i dostosowywać motywy.
...
2. **Katalog konfiguracji użytkownika** - Zdefiniowany w `~/.config/nexus/themes/*.json` lub `$XDG_CONFIG_HOME/nexus/themes/*.json`
3. **Katalog główny projektu** - Zdefiniowany w `<project-root>/.nexus/themes/*.json`
4. **Bieżący katalog roboczy** - Zdefiniowany w `./.nexus/themes/*.json`
