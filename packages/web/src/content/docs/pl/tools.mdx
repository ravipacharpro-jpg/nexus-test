...
Narzędzia umożliwiają LLM wykonywanie działań w bazie kodu. NEXUS posiada zestaw wbudowanych narzędzi, ale możesz go rozszerzyć o [narzędzia niestandardowe](/docs/custom-tools) lub [serwery MCP](/docs/mcp-servers).
...
Oto wszystkie wbudowane narzędzia dostępne w NEXUS.
...
:::note
To narzędzie jest dostępne tylko wtedy, gdy `NEXUS_EXPERIMENTAL_LSP_TOOL=true` (lub `NEXUS_EXPERIMENTAL=true`).
:::
...
:::note
To narzędzie jest dostępne tylko w przypadku korzystania z dostawcy NEXUS lub gdy zmienna środowiskowa `NEXUS_ENABLE_EXA` jest ustawiona na dowolną wartość prawdziwą (np. `true` lub `1`).

Aby włączyć przy uruchamianiu NEXUS:
