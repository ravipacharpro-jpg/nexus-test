...
Aby debugować problemy z nexus, zacznij od sprawdzenia dzienników i danych lokalnych przechowywanych na dysku.
...
Możesz ustawić poziom dziennika za pomocą opcji wiersza poleceń `--log-level`, aby uzyskać bardziej szczegółowe informacje debugowania. Na przykład `nexus --log-level DEBUG`.

---

## Przechowywanie danych

nexus przechowuje dane sesji i inne dane aplikacji na dysku pod adresem:
...
nexus Desktop uruchamia w tle lokalny serwer nexus (sidecar `nexus-cli`). Większość problemów jest spowodowana nieprawidłowo działającą wtyczką, uszkodzoną pamięcią podręczną lub złymi ustawieniami serwera.
...
- Tylko macOS: menu `NEXUS` -> **Załaduj ponownie przeglądarkę internetową** (pomaga, jeśli interfejs użytkownika jest pusty/zawieszony).
...
- **macOS/Linux**: `~/.config/nexus/nexus.jsonc` (lub `~/.config/nexus/nexus.json`)
- **macOS/Linux** (starsze instalacje): `~/.local/share/nexus/nexus.jsonc`
...
nexus może także ładować lokalne wtyczki z dysku. Tymczasowo usuń je (lub zmień nazwę folderu) i uruchom ponownie aplikację komputerową:
...
Jeśli wyłączenie wtyczek nie pomoże (lub instalacja wtyczki utknęła), wyczyść pamięć podręczną, aby nexus mógł ją odbudować.

1. Całkowicie zamknij nexus Desktop.
2. Usuń katalog pamięci podręcznej:

- **macOS**: Finder -> `Cmd+Shift+G` -> paste `~/.cache/nexus`
- **Linux**: usuń `~/.cache/nexus` (lub uruchom `rm -rf ~/.cache/nexus`)
- **Windows**: Naciśnij `WIN+R` i wklej `%USERPROFILE%\.cache\nexus`

3. Uruchom ponownie nexus Desktop.

---

### Napraw problemy z połączeniem z serwerem

nexus Desktop może uruchomić własny serwer lokalny (domyślnie) lub połączyć się ze skonfigurowanym adresem URL serwera.
...
Jeśli Twój `nexus.json(c)` zawiera sekcję `server`, tymczasowo usuń ją i uruchom ponownie aplikację komputerową.

#### Sprawdź zmienne środowiskowe

Jeśli w swoim środowisku masz ustawiony `NEXUS_PORT`, aplikacja komputerowa spróbuje użyć tego portu dla serwera lokalnego.

- usuń `NEXUS_PORT` (lub wybierz wolny port) i uruchom ponownie.
...
W systemie Windows nexus Desktop wymaga Microsoft Edge **WebView2 Runtime**. Jeśli aplikacja otwiera się w pustym oknie lub nie uruchamia się, zainstaluj/zaktualizuj WebView2 i spróbuj ponownie.

---

### Windows: Ogólne problemy z wydajnością

Jeśli doświadczasz niskiej wydajności, problemów z dostępem do plików lub problemów z terminalem w systemie Windows, spróbuj użyć [WSL (podsystem Windows dla systemu Linux) (./windows-wsl). WSL zapewnia środowisko Linux, które płynniej współpracuje z funkcjami nexus.

---

### Brak powiadomień

nexus Desktop pokazuje powiadomienia systemowe tylko wtedy, gdy:

- powiadomienia są włączone dla nexus w ustawieniach systemu operacyjnego, oraz
...
1. Zamknij całkowicie nexus Desktop.
2. Znajdź i usuń te pliki (znajdują się w katalogu danych aplikacji nexus Desktop):

- `nexus.settings.dat` (domyślny adres URL serwera na komputerze stacjonarnym)
- `nexus.global.dat` i `nexus.workspace.*.dat` (stan interfejsu użytkownika, taki jak najnowsze serwery/projekty)
...
3. Upewnij się, że masz najnowszą wersję z `nexus upgrade`
...
Aby dowiedzieć się, do jakich modeli masz dostęp, uruchom `nexus models`
...
Jeśli napotkasz błędy wywołań API, może to wynikać z nieaktualnych pakietów dostawców. nexus dynamicznie instaluje pakiety dostawców (OpenAI, Anthropic, Google itp.) w razie potrzeby i przechowuje je lokalnie w pamięci podręcznej.
...
2. Uruchom ponownie kod nexus, aby ponownie zainstalować najnowsze pakiety dostawców

Zmusi to nexus do pobrania najnowszych wersji pakietów dostawców, co często rozwiązuje problemy ze zgodnością z parametrami modelu i zmianami API.
...
nexus wykryje, czy używasz Waylanda i wolisz `wl-clipboard`, w przeciwnym razie spróbuje znaleźć narzędzia schowka w kolejności: `xclip` i `xsel`.
