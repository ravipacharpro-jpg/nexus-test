---
title: Policies
description: Control which configured resources NEXUS may use.
---

Policies control whether NEXUS may perform an action on a named resource. This feature is experimental and is configured with the `experimental.policies` array in `nexus.json`.

Policies are separate from [permissions](/docs/permissions). Permissions control what tools can do during a session, while policies control whether NEXUS may use a resource such as an LLM provider.
...
NEXUS currently supports one policy action:
