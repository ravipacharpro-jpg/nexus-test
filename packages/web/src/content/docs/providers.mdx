---
title: Providers
description: Using any LLM provider in NEXUS.
---

import config from "../../../config.mjs"
export const console = config.console

NEXUS uses the [AI SDK](https://ai-sdk.dev/) and [Models.dev](https://models.dev) to support **75+ LLM providers** and it supports running local models.

To add a provider you need to:

1. Add the API keys for the provider using the `/connect` command.
2. Configure the provider in your NEXUS config.
...
in `~/.local/share/nexus/auth.json`.

---

### Config

You can customize the providers through the `provider` section in your NEXUS
config.
...
NEXUS Zen is a list of models provided by the NEXUS team that have been
tested and verified to work well with NEXUS. [Learn more](/docs/zen).

:::tip
If you are new, we recommend starting with NEXUS Zen.
:::

1. Run the `/connect` command in the TUI, select `NEXUS Zen`, and head to [nexus.ai/auth](https://nexus.ai/zen).
...
It works like any other provider in NEXUS and is completely optional to use.

---

## NEXUS Go

NEXUS Go is a low cost subscription plan that provides reliable access to popular open coding models provided by the NEXUS team that have been
tested and verified to work well with NEXUS.

1. Run the `/connect` command in the TUI, select `NEXUS Go`, and head to [nexus.ai/auth](https://nexus.ai/zen).
...
To use Amazon Bedrock with NEXUS:
...
2. **Configure authentication** using one of the following methods:
   ...
   ***
   ...
   Set one of these environment variables while running nexus:

   ```bash
   # Option 1: Using AWS access keys
   AWS_ACCESS_KEY_ID=XXX AWS_SECRET_ACCESS_KEY=YYY nexus

   # Option 2: Using named AWS profile
   AWS_PROFILE=my-profile nexus

   # Option 3: Using Bedrock bearer token
   AWS_BEARER_TOKEN_BEDROCK=XXX nexus
   ```
   ...
   ***
   ...
   For project-specific or persistent configuration, use `nexus.json`:
...
NEXUS. Anthropic explicitly prohibits this.

Previous versions of NEXUS came bundled with these plugins but that is no
longer the case as of 1.3.0

Other companies support freedom of choice with developer tooling - you can use
the following subscriptions in NEXUS with zero setup:
...
You can configure nexus to use local models through [Atomic Chat](https://atomic.chat), a desktop application that runs local LLMs behind an OpenAI-compatible API server (default endpoint `http://127.0.0.1:1337/v1`).

```json title="nexus.json" "atomic-chat" {5, 6, 8, 10-14}
{
  "$schema": "https://nexus.ai/config.json",
  "provider": {
    "atomic-chat": {
...
2. Go to [Azure AI Foundry](https://ai.azure.com/) and deploy a model.
   ...
   :::note
   The deployment name must match the model name for nexus to work properly.
   :::
...
5. Set your resource name as an environment variable:
   ...
   ```bash
   AZURE_COGNITIVE_SERVICES_RESOURCE_NAME=XXX nexus
   ```
...
6. Run the `/models` command to select a model.
   ...
   You can also add models through your nexus config.
...
- **OAuth (Recommended)** — Sign in to your DigitalOcean account; NEXUS uses your DigitalOcean API token directly for inference and discovers your Inference Routers.
...
3. Your browser opens to authorize NEXUS. Sign in and approve.

   :::note
   NEXUS requests `genai:read` and `inference:query` OAuth scopes. Your DigitalOcean API token is used directly for inference — no separate Model Access Key is created.
   :::
...
Inference Routers let you define a routing policy across multiple models — picking the cheapest, fastest, or most appropriate model per request based on the task. After OAuth, NEXUS surfaces each router as `router:<router-name>` in the model picker.

Selecting a router model is a drop-in replacement for any other model — NEXUS forwards your request and DigitalOcean picks the underlying model based on your router's policy. Learn more about [Inference Routers](https://docs.digitalocean.com/products/inference/how-to/use-inference-router/)
...
GitLab Duo support in NEXUS is experimental. Features, configuration, and
behavior may change in future releases.
:::

NEXUS integrates with the [GitLab Duo Agent Platform](https://docs.gitlab.com/user/duo_agent_platform/),
...
to store token in nexus auth storage.
:::

##### Self-Hosted GitLab

:::note[compliance note]
NEXUS uses a small model for some AI tasks like generating the session title.
It is configured to use gpt-5-nano by default, hosted by Zen. To lock NEXUS
to only use your own GitLab-hosted instance, add the following to your
`nexus.json` file. It is also recommended to disable session sharing.
...
More documentation on [nexus-gitlab-auth](https://www.npmjs.com/package/nexus-gitlab-auth) homepage.

##### Configuration

Customize through `nexus.json`:
...
When a `duo-workflow-*` model is selected, NEXUS will:
...
4. Route tool execution requests through NEXUS's permission-gated tool system
...
To use your GitHub Copilot subscription with nexus:
...
To use Google Vertex AI with NEXUS:
...
2. Set the required environment variables:
   ...
   Set them while running nexus.

   ```bash
   GOOGLE_APPLICATION_CREDENTIALS=/path/to/service-account.json GOOGLE_CLOUD_PROJECT=your-project-id nexus
   ```
...
In the event you see a feature or model from Helicone that isn't configured automatically through nexus, you can always configure it yourself.
...
```jsonc title="~/.config/nexus/nexus.jsonc"
{
  "$schema": "https://nexus.ai/config.json",
  "provider": {
    "helicone": {
...
Helicone's [Sessions](https://docs.helicone.ai/features/sessions) feature lets you group related LLM requests together. Use the [nexus-helicone-session](https://github.com/H2Shami/nexus-helicone-session) plugin to automatically log each NEXUS conversation as a session in Helicone.

```bash
npm install -g nexus-helicone-session
```
...
```json title="nexus.json"
{
  "plugin": ["nexus-helicone-session"]
}
```

The plugin injects `Helicone-Session-Id` and `Helicone-Session-Name` headers into your requests. In Helicone's Sessions page, you'll see each NEXUS conversation listed as a separate session.
...
You can configure nexus to use local models through [llama.cpp's](https://github.com/ggml-org/llama.cpp) llama-server utility

```json title="nexus.json" "llama.cpp" {5, 6, 8, 10-15}
{
  "$schema": "https://nexus.ai/config.json",
  "provider": {
    "llama.cpp": {
...
You can configure nexus to use local models through LM Studio.

```json title="nexus.json" "lmstudio" {5, 6, 8, 10-14}
{
  "$schema": "https://nexus.ai/config.json",
  "provider": {
    "lmstudio": {
...
You can configure nexus to use local models through Ollama.

:::tip
Ollama can automatically configure itself for NEXUS. See the [Ollama integration docs](https://docs.ollama.com/integrations/nexus) for details.
:::

```json title="nexus.json" "ollama" {5, 6, 8, 10-14}
{
  "$schema": "https://nexus.ai/config.json",
  "provider": {
    "ollama": {
...
To use Ollama Cloud with NEXUS:
...
3. Copy the API key for use in NEXUS.
...
6. **Important**: Before using cloud models in NEXUS, you must pull the model information locally:
...
NEXUS Zen is a list of tested and verified models provided by the NEXUS team. [Learn more](/docs/zen).

1. Sign in to **<a href={console}>NEXUS Zen</a>** and click **Create API Key**.

2. Run the `/connect` command and search for **NEXUS Zen**.

   ```txt
   /connect
   ```

3. Enter your NEXUS API key.
...
4. Many OpenRouter models are preloaded by default, run the `/models` command to select the one you want.
   ...
   You can also add additional models through your nexus config.
...
5. You can also customize them through your nexus config. Here's an example of specifying a provider
...
3. Enter your service key JSON.
   ...
   ```bash
   AICORE_SERVICE_KEY='{"clientid":"...","clientsecret":"...","url":"...","serviceurls":{"AI_API_URL":"..."}}' nexus
   ```
...
4. Optionally set deployment ID and resource group:
   ...
   ```bash
   AICORE_DEPLOYMENT_ID=your-deployment-id AICORE_RESOURCE_GROUP=your-resource-group nexus
   ```
...
To use [Scaleway Generative APIs](https://www.scaleway.com/en/docs/generative-apis/) with Nexus:
...
Don't have a Snowflake account? [Sign up for a free trial](https://signup.snowflake.com/?utm_source=nexus&utm_medium=docs&utm_campaign=cortex-provider).

nexus's core workflow for coding, editing files, and running commands relies on tool calling. Only the Claude and OpenAI families within Snowflake Cortex support this. The provider is limited to those families to support the core workflow.
...
5. Complete sign-in in the browser that opens. NEXUS captures the OAuth callback automatically and stores the token — no copy/paste needed.
...
For CI or headless environments, set a PAT or JWT before starting nexus:
...
The model catalog is provided automatically. A minimal `nexus.json` is all that's needed:
...
You can also customize models through your nexus config. Here's an example of specifying provider routing order.
...
2. Select **SuperGrok Subscription**. NEXUS opens xAI's verification link with the user code pre-populated when supported.

3. Approve the consent screen. If xAI asks for a code, enter the user code displayed by NEXUS. NEXUS polls xAI's token endpoint and stores the resulting OAuth tokens once you approve.
...
NEXUS refreshes the OAuth access token automatically. Any Grok or X Premium plan that includes Grok API access works; you do not need a separate `XAI_API_KEY`.
...
:::tip
You can use any OpenAI-compatible provider with nexus. Most modern AI providers offer OpenAI-compatible APIs.
:::
...
3. Enter your API key for the provider.
   ...
   ┌  Add credential
   │
   ▲  This only stores a credential for myprovider - you will need to configure it in nexus.json, check the docs for examples.
   │
   ◇  Enter your API key
   │  sk-...
   └
...
4. Create or update your `nexus.json` file in your project directory:

   ```json title="nexus.json" ""myprovider"" {5-15}
   {
     "$schema": "https://nexus.ai/config.json",
     "provider": {
       "myprovider": {
...
```json title="nexus.json" {9,11,17-20}
{
  "$schema": "https://nexus.ai/config.json",
  "provider": {
    "myprovider": {
...
The `limit` fields allow NEXUS to understand how much context you have left. Standard providers pull these from models.dev automatically.
...
1. **Check the auth setup**: Run `nexus auth list` to see if the credentials
...
2. For custom providers, check the nexus config and:
   - Make sure the provider ID used in the `/connect` command matches the ID in your nexus config.
