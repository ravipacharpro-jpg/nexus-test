...
O NEXUS Go é uma assinatura de baixo custo — **US$ 5 no seu primeiro mês**, depois **US$ 10/mês** — que oferece acesso confiável a modelos abertos de programação populares.

O Go funciona como qualquer outro provedor no NEXUS. Você assina o NEXUS Go e
...
O NEXUS Go lhe dá acesso a esses modelos por **US$ 5 no seu primeiro mês**, depois **US$ 10/mês**.

---

## Como funciona

O NEXUS Go funciona como qualquer outro provedor no NEXUS.

1. Você entra no **<a href={console}>NEXUS Zen</a>**, assina o Go e
   copia a sua chave de API.
2. Você executa o comando `/connect` na TUI, seleciona `NEXUS Go` e cola
   a sua chave de API.
3. Execute `/models` na TUI para ver a lista de modelos disponíveis através do Go.

:::note
Apenas um membro por workspace pode assinar o NEXUS Go.
:::
...
O NEXUS Go inclui os seguintes limites:
...
O [ID do modelo](/docs/config/#models) na sua configuração do NEXUS
usa o formato `nexus-go/<model-id>`. Por exemplo, para o Kimi K3, você usaria
`nexus-go/kimi-k3` na sua configuração.
...
4. Não ter **nenhuma restrição de fornecedor (lock-in)**, permitindo que você use qualquer outro provedor com o NEXUS também.
