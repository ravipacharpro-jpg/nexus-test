...
Você pode adicionar ferramentas externas ao nexus usando o _Model Context Protocol_, ou MCP. O nexus suporta servidores locais e remotos.
...
Você pode definir servidores MCP em sua [Configuração do nexus](https://nexus.ai/docs/config/) sob `mcp`. Adicione cada MCP com um nome único. Você pode se referir a esse MCP pelo nome ao solicitar ao LLM.
...
As organizações podem fornecer servidores MCP padrão através de seu endpoint `.well-known/nexus`. Esses servidores podem estar desabilitados por padrão, permitindo que os usuários optem pelos que precisam.
...
O nexus lida automaticamente com a autenticação OAuth para servidores MCP remotos. Quando um servidor requer autenticação, o nexus irá:
...
Se o servidor requer autenticação, o nexus solicitará que você se autentique quando tentar usá-lo pela primeira vez. Se não, você pode [iniciar manualmente o fluxo](#authenticating) com `nexus mcp auth <server-name>`.
...
O comando `mcp auth` abrirá seu navegador para autorização. Após você autorizar, o nexus armazenará os tokens de forma segura em `~/.local/share/nexus/mcp-auth.json`.
...
Seus MCPs estão disponíveis como ferramentas no nexus, juntamente com ferramentas integradas. Portanto, você pode gerenciá-los através da configuração do nexus como qualquer outra ferramenta.
...
Isso abrirá uma janela do navegador para completar o fluxo OAuth e conectar o nexus à sua conta do Sentry.
