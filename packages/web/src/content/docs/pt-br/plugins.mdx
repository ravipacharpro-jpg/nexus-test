---
title: Plugins
description: Escreva seus próprios plugins para estender o nexus.
---

Plugins permitem que você estenda o nexus conectando-se a vários eventos e personalizando o comportamento. Você pode criar plugins para adicionar novos recursos, integrar-se a serviços externos ou modificar o comportamento padrão do nexus.
...
- `.nexus/plugins/` - Plugins em nível de projeto
- `~/.config/nexus/plugins/` - Plugins globais
...
**Plugins npm** são instalados automaticamente usando Bun na inicialização. Pacotes e suas dependências são armazenados em cache em `~/.cache/nexus/node_modules/`.
...
1. Configuração global (`~/.config/nexus/nexus.json`)
2. Configuração do projeto (`nexus.json`)
3. Diretório de plugins global (`~/.config/nexus/plugins/`)
4. Diretório de plugins do projeto (`.nexus/plugins/`)
...
O nexus executa `bun install` na inicialização para instalar esses pacotes. Seus plugins e ferramentas podem então importá-los.
...
- `client`: Um cliente SDK do nexus para interagir com a AI.
...
Aqui estão alguns exemplos de plugins que você pode usar para estender o nexus.
...
:::note
Se você estiver usando o aplicativo desktop nexus, ele pode enviar notificações do sistema automaticamente quando uma resposta estiver pronta ou quando ocorrer um erro na sessão.
:::
...
Impeça o nexus de ler arquivos `.env`:
...
Plugins também podem adicionar ferramentas personalizadas ao nexus:
...
O helper `tool` cria uma ferramenta personalizada que o nexus pode chamar. Ele aceita uma função de esquema Zod e retorna uma definição de ferramenta com:
...
Suas ferramentas personalizadas estarão disponíveis para o nexus junto com as ferramentas integradas.
...
Níveis: `debug`, `info`, `warn`, `error`. Veja a [documentação do SDK](https://nexus.ai/docs/sdk) para detalhes.
