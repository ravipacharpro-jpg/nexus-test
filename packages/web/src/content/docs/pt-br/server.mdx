---
title: Servidor
description: Interaja com o servidor nexus via HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

O comando `nexus serve` executa um servidor HTTP sem cabeça que expõe um endpoint OpenAPI que um cliente nexus pode usar.
...
```bash
nexus serve [--port <number>] [--hostname <string>] [--cors <origin>]
```
...
| `--mdns-domain` | Nome de domínio personalizado para o serviço mDNS | `nexus.local` |
| `--cors`        | Origens adicionais de navegador a permitir        | `[]`             |

`--cors` pode ser passado várias vezes:

```bash
nexus serve --cors http://localhost:5173 --cors https://app.example.com
```
...
Defina `NEXUS_SERVER_PASSWORD` para proteger o servidor com autenticação básica HTTP. O nome de usuário padrão é `nexus`, ou defina `NEXUS_SERVER_USERNAME` para substituí-lo. Isso se aplica tanto ao `nexus serve` quanto ao `nexus web`.

```bash
NEXUS_SERVER_PASSWORD=your-password nexus serve
```
...
Quando você executa `nexus`, ele inicia um TUI e um servidor. Onde o TUI é o cliente que se comunica com o servidor. O servidor expõe um endpoint de especificação OpenAPI 3.1. Este endpoint também é usado para gerar um [SDK](/docs/sdk).

:::tip
Use o servidor nexus para interagir com o nexus programaticamente.
:::

Essa arquitetura permite que o nexus suporte múltiplos clientes e permite que você interaja com o nexus programaticamente.

Você pode executar `nexus serve` para iniciar um servidor autônomo. Se você tiver o TUI do nexus em execução, `nexus serve` iniciará um novo servidor.
...
O endpoint [`/tui`](#tui) pode ser usado para controlar o TUI através do servidor. Por exemplo, você pode preencher ou executar um prompt. Essa configuração é usada pelos plugins do nexus [IDE](/docs/ide).
...
O servidor nexus expõe as seguintes APIs.
