...
As habilidades do agente permitem que o nexus descubra instruções reutilizáveis do seu repositório ou diretório pessoal.
...
O nexus pesquisa nesses locais:

- Configuração do projeto: `.nexus/skills/<name>/SKILL.md`
- Configuração global: `~/.config/nexus/skills/<name>/SKILL.md`
...
Para caminhos locais do projeto, o nexus sobe a partir do seu diretório de trabalho atual até alcançar a árvore de trabalho do git.
Ele carrega qualquer `skills/*/SKILL.md` correspondente em `.nexus/` e qualquer `.claude/skills/*/SKILL.md` ou `.agents/skills/*/SKILL.md` ao longo do caminho.

As definições globais também são carregadas de `~/.config/nexus/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` e `~/.agents/skills/*/SKILL.md`.
...
Crie `.nexus/skills/git-release/SKILL.md` assim:
...
O nexus lista as habilidades disponíveis na descrição da ferramenta `skill`.
...
Controle quais habilidades os agentes podem acessar usando permissões baseadas em padrões em `nexus.json`:
...
**Para agentes embutidos** (em `nexus.json`):
