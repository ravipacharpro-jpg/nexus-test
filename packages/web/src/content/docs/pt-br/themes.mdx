...
Com o nexus, você pode selecionar um dos vários temas embutidos, usar um tema que se adapta ao tema do seu terminal ou definir seu próprio tema personalizado.

Por padrão, o nexus usa nosso próprio tema `nexus`.
...
O nexus vem com vários temas embutidos.
...
- Querem que o nexus corresponda à aparência do seu terminal
...
O nexus suporta um sistema de temas flexível baseado em JSON que permite aos usuários criar e personalizar temas facilmente.
...
2. **Diretório de configuração do usuário** - Definido em `~/.config/nexus/themes/*.json` ou `$XDG_CONFIG_HOME/nexus/themes/*.json`
3. **Diretório raiz do projeto** - Definido em `<project-root>/.nexus/themes/*.json`
4. **Diretório de trabalho atual** - Definido em `./.nexus/themes/*.json`
