...
Para depurar problemas com o nexus, comece verificando os logs e os dados locais que ele armazena no disco.
...
Você pode definir o nível de log com a opção de linha de comando `--log-level` para obter informações de depuração mais detalhadas. Por exemplo, `nexus --log-level DEBUG`.

---

## Armazenamento

O nexus armazena dados de sessão e outros dados do aplicativo no disco em:
...
O nexus Desktop executa um servidor nexus local (o sidecar `nexus-cli`) em segundo plano. A maioria dos problemas é causada por um plugin com mau funcionamento, um cache corrompido ou uma configuração de servidor incorreta.
...
- Apenas macOS: menu `NEXUS` -> **Recarregar Webview** (ajuda se a interface estiver em branco/congelada).
...
- **macOS/Linux**: `~/.config/nexus/nexus.jsonc` (ou `~/.config/nexus/nexus.json`)
- **macOS/Linux** (instalações mais antigas): `~/.local/share/nexus/nexus.jsonc`
...
O nexus também pode carregar plugins locais do disco. Mova-os temporariamente para fora do caminho (ou renomeie a pasta) e reinicie o aplicativo de desktop:
...
Se desativar plugins não ajudar (ou se a instalação de um plugin estiver travada), limpe o cache para que o nexus possa reconstruí-lo.

1. Saia completamente do nexus Desktop.
2. Exclua o diretório de cache:

- **macOS**: Finder -> `Cmd+Shift+G` -> cole `~/.cache/nexus`
- **Linux**: exclua `~/.cache/nexus` (ou execute `rm -rf ~/.cache/nexus`)
- **Windows**: Pressione `WIN+R` e cole `%USERPROFILE%\.cache\nexus`

3. Reinicie o nexus Desktop.

---

### Corrigindo problemas de conexão com o servidor

O nexus Desktop pode iniciar seu próprio servidor local (padrão) ou conectar-se a uma URL de servidor que você configurou.
...
Se seu `nexus.json(c)` contiver uma seção `server`, remova-a temporariamente e reinicie o aplicativo de desktop.

#### Verificando as variáveis de ambiente

Se você tiver `NEXUS_PORT` definido em seu ambiente, o aplicativo de desktop tentará usar essa porta para o servidor local.

- Desfaça `NEXUS_PORT` (ou escolha uma porta livre) e reinicie.
...
No Windows, o nexus Desktop requer o **WebView2 Runtime** do Microsoft Edge. Se o aplicativo abrir em uma janela em branco ou não iniciar, instale/atualize o WebView2 e tente novamente.

---

### Windows: Problemas gerais de desempenho

Se você estiver enfrentando desempenho lento, problemas de acesso a arquivos ou problemas no terminal no Windows, tente usar [WSL (Windows Subsystem for Linux)](/docs/windows-wsl). O WSL fornece um ambiente Linux que funciona de forma mais integrada com os recursos do nexus.

---

### Notificações não aparecendo

O nexus Desktop só mostra notificações do sistema quando:

- as notificações estão habilitadas para o nexus nas configurações do seu sistema operacional, e
...
1. Saia do nexus Desktop.
2. Encontre e exclua estes arquivos (eles estão no diretório de dados do aplicativo nexus Desktop):

- `nexus.settings.dat` (URL do servidor padrão do desktop)
- `nexus.global.dat` e `nexus.workspace.*.dat` (estado da interface como servidores/projetos recentes)
...
Se você estiver enfrentando problemas com o nexus:
...
3. Certifique-se de que você tem a versão mais recente com `nexus upgrade`
...
Para descobrir quais modelos você tem acesso, execute `nexus models`
...
Se você encontrar erros de chamada de API, isso pode ser devido a pacotes de provedores desatualizados. O nexus instala dinamicamente pacotes de provedores (OpenAI, Anthropic, Google, etc.) conforme necessário e os armazena em cache localmente.
...
2. Reinicie o nexus para reinstalar os pacotes de provedores mais recentes

Isso forçará o nexus a baixar as versões mais recentes dos pacotes de provedores, o que muitas vezes resolve problemas de compatibilidade com parâmetros de modelo e alterações na API.
...
O nexus detectará se você está usando Wayland e preferirá `wl-clipboard`, caso contrário, tentará encontrar ferramentas de área de transferência na ordem: `xclip` e `xsel`.
