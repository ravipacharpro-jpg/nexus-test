...
Создайте `.nexus/commands/test.md`:
...
Вы можете добавлять собственные команды через конфигурацию nexus или создав файлы Markdown в каталоге `commands/`.

---

### JSON

Используйте опцию `command` в вашем nexus [config](/docs/config):
...
- Глобальный: `~/.config/nexus/commands/`
- Для каждого проекта: `.nexus/commands/`
...
nexus включает несколько встроенных команд, таких как `/init`, `/undo`, `/redo`, `/share`, `/help`; [подробнее](/docs/tui#commands).
