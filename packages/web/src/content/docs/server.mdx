---
title: Server
description: Interact with nexus server over HTTP.
---

import config from "../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

The `nexus serve` command runs a headless HTTP server that exposes an OpenAPI endpoint that an nexus client can use.
...
| `--mdns-domain` | Custom domain name for mDNS service | `nexus.local` |
...
Set `NEXUS_SERVER_PASSWORD` to protect the server with HTTP basic auth. The username defaults to `nexus`, or set `NEXUS_SERVER_USERNAME` to override it. This applies to both `nexus serve` and `nexus web`.
...
When you run `nexus` it starts a TUI and a server. Where the TUI is the
...
:::tip
Use the nexus server to interact with nexus programmatically.
:::

This architecture lets nexus support multiple clients and allows you to interact with nexus programmatically.

You can run `nexus serve` to start a standalone server. If you have the
nexus TUI running, `nexus serve` will start a new server.
...
The [`/tui`](#tui) endpoint can be used to drive the TUI through the server. For example, you can prefill or run a prompt. This setup is used by the NEXUS [IDE](/docs/ide) plugins.
...
The nexus server exposes the following APIs.
