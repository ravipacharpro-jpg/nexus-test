...
With NEXUS you can select from one of several built-in themes, use a theme that adapts to your terminal theme, or define your own custom theme.

By default, NEXUS uses our own `nexus` theme.
...
NEXUS comes with several built-in themes.
...
- Want NEXUS to match their terminal's appearance
...
NEXUS supports a flexible JSON-based theme system that allows users to create and customize themes easily.
...
2. **User config directory** - Defined in `~/.config/nexus/themes/*.json` or `$XDG_CONFIG_HOME/nexus/themes/*.json`
3. **Project root directory** - Defined in the `<project-root>/.nexus/themes/*.json`
4. **Current working directory** - Defined in `./.nexus/themes/*.json`
