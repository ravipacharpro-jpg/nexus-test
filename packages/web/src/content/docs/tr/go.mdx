...
NEXUS Go, popüler açık kodlama modellerine güvenilir erişim sağlayan düşük maliyetli bir aboneliktir — **ilk ayınız için 5$**, sonrasında **aylık 10$**.

Go, NEXUS'daki diğer sağlayıcılar gibi çalışır. NEXUS Go'ya abone olur ve API anahtarınızı alırsınız. Bu **tamamen isteğe bağlıdır** ve NEXUS'u kullanmak için bunu kullanmanıza gerek yoktur.
...
NEXUS Go, bu modellere **ilk ayınız için 5$**, ardından **aylık 10$** karşılığında erişmenizi sağlar.

---

## Nasıl çalışır?

NEXUS Go, NEXUS'daki diğer sağlayıcılar gibi çalışır.

1. **<a href={console}>NEXUS Zen</a>**'e giriş yapın, Go'ya abone olun ve API anahtarınızı kopyalayın.
2. TUI'de `/connect` komutunu çalıştırın, `NEXUS Go`yu seçin ve API anahtarınızı yapıştırın.
3. Go üzerinden kullanabileceğiniz modellerin listesini görmek için TUI'de `/models` komutunu çalıştırın.

:::note
Her çalışma alanından yalnızca bir üye NEXUS Go'ya abone olabilir.
:::
...
NEXUS Go aşağıdaki limitleri içerir:
...
NEXUS yapılandırmanızdaki [model id](/docs/config/#models) formatı `nexus-go/<model-id>` şeklindedir. Örneğin, Kimi K3 için yapılandırmanızda `nexus-go/kimi-k3` kullanmalısınız.
...
NEXUS Go'yu şu amaçlarla oluşturduk:
...
4. NEXUS ile diğer sağlayıcıları da kullanmanıza olanak tanıyarak **kilitlenme (vendor lock-in) olmamasını** sağlamak.
