...
description: NEXUS, LSP sunucularınızla bütünleşir.
---

NEXUS, tanılamayı agent için geri bildirim olarak kullanmak üzere Language Server Protocol (LSP) sunucularıyla bütünleşebilir.

---

## Yerleşik

NEXUS, popüler diller için çeşitli yerleşik LSP sunucularıyla birlikte gelir:
...
:::note
`NEXUS_DISABLE_LSP_DOWNLOAD` ortam değişkenini `true` olarak ayarlayarak otomatik LSP sunucu indirmelerini devre dışı bırakabilirsiniz.
:::
...
LSP etkinleştirildiğinde ve nexus bir dosyayı açtığında:
...
LSP sunucularını, nexus yapılandırmanızdaki `lsp` bölümü aracılığıyla etkinleştirebilir ve özelleştirebilirsiniz.
