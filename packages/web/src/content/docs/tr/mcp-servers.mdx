...
_Model Bağlam Protokolü_ veya MCP kullanarak nexus'a harici araçlar ekleyebilirsiniz. nexus hem yerel hem de uzak sunucuları destekler.
...
MCP sunucularını [nexus Config](https://nexus.ai/docs/config/)'nizde `mcp` altında tanımlayabilirsiniz. Her MCP'ü benzersiz bir adla ekleyin. LLM isteminde bulunurken bu MCP adına başvurabilirsiniz.
...
Kuruluşlar, `.well-known/nexus` uç noktaları aracılığıyla varsayılan MCP sunucularını sağlayabilir. Bu sunucular varsayılan olarak devre dışı bırakılarak kullanıcıların ihtiyaç duydukları sunucuları seçmelerine olanak tanınabilir.
...
nexus, uzak MCP sunucuları için OAuth kimlik doğrulamasını otomatik olarak yönetir. Bir sunucu kimlik doğrulaması gerektirdiğinde nexus şunları yapar:
...
Sunucu kimlik doğrulama gerektiriyorsa nexus, onu ilk kez kullanmaya çalıştığınızda kimlik doğrulamanızı isteyecektir. Değilse, `nexus mcp auth <server-name>` ile [manually trigger the flow](#authenticating) yapabilirsiniz.
...
`mcp auth` komutu tarayıcınızı yetkilendirme için açacaktır. Yetkilendirmenizin ardından nexus, belirteçleri `~/.local/share/nexus/mcp-auth.json`'de güvenli bir şekilde saklar.
...
MCP'leriniz yerleşik araçların yanı sıra nexus'da araç olarak mevcuttur. Böylece bunları diğer araçlar gibi nexus yapılandırması aracılığıyla yönetebilirsiniz.
...
Bu, OAuth akışını tamamlamak ve nexus'u Sentry hesabınıza bağlamak için bir tarayıcı penceresi açacaktır.
