---
title: Eklentiler
description: NEXUS'u genişletmek için kendi eklentilerinizi yazın.
---

Eklentiler, çeşitli olaylara bağlanarak ve davranışı özelleştirerek NEXUS'u genişletmenize olanak tanır. Yeni özellikler eklemek, harici hizmetlerle entegrasyon sağlamak veya NEXUS'un varsayılan davranışını değiştirmek için eklentiler oluşturabilirsiniz.
...
- `.nexus/plugins/` - Proje düzeyinde eklentiler
- `~/.config/nexus/plugins/` - Genel eklentiler
...
**npm eklentileri** başlangıçta Bun kullanılarak otomatik olarak yüklenir. Paketler ve bağımlılıkları `~/.cache/nexus/node_modules/`'da önbelleğe alınır.
...
1. Global config (`~/.config/nexus/nexus.json`)
2. Project config (`nexus.json`)
3. Global eklenti dizini (`~/.config/nexus/plugins/`)
4. Proje eklenti dizini (`.nexus/plugins/`)
...
NEXUS bunları yüklemek için başlangıçta `bun install` komutunu çalıştırır. Eklentileriniz ve araçlarınız daha sonra bunları içe aktarabilir.
...
- `client`: Yapay zeka ile etkileşim kurmak için bir NEXUS SDK istemcisi.
...
NEXUS'u genişletmek için kullanabileceğiniz bazı eklenti örneklerini burada bulabilirsiniz.
...
:::note
NEXUS masaüstü uygulamasını kullanıyorsanız yanıt hazır olduğunda veya oturum hataları oluştuğunda otomatik olarak sistem bildirimleri gönderebilir.
:::
...
NEXUS'un `.env` dosyalarını okumasını önleyin:
...
Eklentiler ayrıca NEXUS'a özel araçlar da ekleyebilir:
...
`tool` yardımcısı, NEXUS'un çağırabileceği özel bir araç oluşturur. Bir Zod şeması işlevini alır ve aşağıdakileri içeren bir araç tanımı döndürür:

- `description`: Araç ne yapar?
- `args`: Aracın argümanları için Zod şeması
- `execute`: Araç çağrıldığında çalışan fonksiyon

Özel araçlarınız, yerleşik araçların yanı sıra NEXUS'a da hazır olacaktır.
...
Seviyeler: `debug`, `info`, `warn`, `error`. Ayrıntılar için [SDK belgelerine](https://nexus.ai/docs/sdk) bakın.
