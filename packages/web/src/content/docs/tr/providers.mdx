---
title: Sağlayıcılar
description: nexus'da herhangi bir LLM sağlayıcısını kullanma.
---

import config from "../../../../config.mjs"
export const console = config.console

nexus, **75'ten fazla LLM sağlayıcısını** desteklemek için [AI SDK](https://ai-sdk.dev/) ve [Models.dev](https://models.dev) kullanır ve yerel modellerin çalıştırılmasını destekler.
...
2. Sağlayıcıyı nexus yapılandırmanızda yapılandırın.
...
`~/.local/share/nexus/auth.json` içinde.

---

### Yapılandırma

Sağlayıcıları nexus'unuzdaki `provider` bölümü aracılığıyla özelleştirebilirsiniz.
config.
...
NEXUS Zen, nexus ekibi tarafından sağlanan modellerin bir listesidir.
nexus ile iyi çalıştığı test edilip doğrulandı. [Daha fazla bilgi](/docs/zen).

:::tip
Yeniyseniz NEXUS Zen ile başlamanızı öneririz.
:::

1. TUI'de `/connect` komutunu çalıştırın, nexus'u seçin ve [nexus.ai/auth](https://nexus.ai/auth)'ye gidin.
...
nexus'daki diğer sağlayıcılar gibi çalışır ve kullanımı tamamen isteğe bağlıdır.

---

## NEXUS Go

NEXUS Go, nexus ile iyi çalıştığı test edilmiş ve doğrulanmış, nexus ekibi tarafından sağlanan popüler açık kodlama modellerine güvenilir erişim sağlayan düşük maliyetli bir abonelik planıdır.

1. TUI'de `/connect` komutunu çalıştırın, `NEXUS Go`'yu seçin ve [nexus.ai/auth](https://nexus.ai/zen) adresine gidin.
...
Amazon Bedrock'u nexus ile kullanmak için:
...
2. **Kimlik doğrulamayı yapılandırın**: Aşağıdaki yöntemlerden birini kullanın:
   ...
   nexus'u çalıştırırken bu ortam değişkenlerinden birini ayarlayın:
   ...
   Projeye özel veya kalıcı yapılandırma için `nexus.json` kullanın:
...
:::info
Claude Pro/Max aboneliğinizi nexus'da kullanmak [Anthropic](https://anthropic.com) tarafından resmi olarak desteklenmemektedir.
:::
...
nexus'u, yerel LLM'leri OpenAI uyumlu bir API sunucusunun arkasında çalıştıran bir masaüstü uygulaması olan [Atomic Chat](https://atomic.chat) aracılığıyla yerel modelleri kullanacak şekilde yapılandırabilirsiniz (varsayılan uç nokta `http://127.0.0.1:1337/v1`).
...
2. [Azure AI Foundry](https://ai.azure.com/)'a gidin ve bir model dağıtın.
   ...
   :::note
   nexus'un düzgün çalışması için dağıtım adının model adıyla eşleşmesi gerekir.
   :::
...
5. Bir model seçmek için `/models` komutunu çalıştırın.
   ...
   Nexus yapılandırmanız aracılığıyla da modeller ekleyebilirsiniz.
...
:::note[uyumluluk notu]
nexus, oturum başlığını oluşturmak gibi bazı AI görevleri için küçük bir model kullanır.
Varsayılan olarak Zen tarafından barındırılan gpt-5-nano'yu kullanacak şekilde yapılandırılmıştır. nexus'u kilitlemek için
yalnızca kendi GitLab tarafından barındırılan örneğinizi kullanmak için aşağıdakileri ekleyin:
`nexus.json` dosyası. Ayrıca oturum paylaşımını devre dışı bırakmanız da önerilir.
...
Daha fazla belge [nexus-gitlab-auth](https://www.npmjs.com/package/nexus-gitlab-auth) ana sayfasında.

##### Yapılandırma

`nexus.json` üzerinden özelleştirin:
...
GitHub Copilot aboneliğinizi nexus'la kullanmak için:
...
Google Vertex AI'yi nexus ile kullanmak için:
...
2. Gerekli ortam değişkenlerini ayarlayın:
   ...
   Bunları nexus çalıştırılırken ayarlayın.
...
Helicone'un nexus aracılığıyla otomatik olarak yapılandırılmayan bir özelliği veya modelini görmeniz durumunda bunu her zaman kendiniz yapılandırabilirsiniz.
...
Helicone'un [Sessions](https://docs.helicone.ai/features/sessions) özelliği, ilgili LLM isteklerini birlikte gruplandırmanıza olanak tanır. Her nexus görüşmesini Helicone'da bir oturum olarak otomatik olarak kaydetmek için [nexus-helicone-session](https://github.com/H2Shami/nexus-helicone-session) eklentisini kullanın.
...
Eklenti isteklerinize `Helicone-Session-Id` ve `Helicone-Session-Name` başlıklarını enjekte eder. Helicone'un Oturumlar sayfasında her nexus görüşmesinin ayrı bir oturum olarak listelendiğini göreceksiniz.
...
nexus'u yerel modelleri kullanacak şekilde [llama.cpp's](https://github.com/ggml-org/llama.cpp) llama-server yardımcı programını kullanarak yapılandırabilirsiniz.
...
LM Studio aracılığıyla nexus'u yerel modelleri kullanacak şekilde yapılandırabilirsiniz.
...
Ollama aracılığıyla nexus'u yerel modelleri kullanacak şekilde yapılandırabilirsiniz.

:::tip
Ollama kendisini nexus için otomatik olarak yapılandırabilir. Ayrıntılar için [Ollama integration docs](https://docs.ollama.com/integrations/nexus)'a bakın.
:::
...
Ollama Cloud'u nexus ile kullanmak için:
...
3. nexus'da kullanmak için API anahtarını kopyalayın.
...
6. **Önemli**: nexus'da bulut modellerini kullanmadan önce model bilgilerini yerel olarak almanız gerekir:
...
NEXUS Zen, nexus ekibinin test edip doğruladığı modellerin bir listesidir. [Daha fazla bilgi](/docs/zen).

1. **<a href={console}>NEXUS Zen</a>**'de oturum açın ve **API Anahtarı Oluştur**'u tıklayın.

2. `/connect` komutunu çalıştırın ve **NEXUS Zen**'i arayın.

   ```txt
   /connect
   ```

3. nexus API anahtarınızı girin.
...
4. Birçok OpenRouter modeli varsayılan olarak önceden yüklenmiştir; istediğiniz modeli seçmek için `/models` komutunu çalıştırın.
   ...
   nexus yapılandırmanız aracılığıyla ek modeller de ekleyebilirsiniz.
...
5. Bunları ayrıca nexus yapılandırmanız aracılığıyla da özelleştirebilirsiniz. Burada bir sağlayıcı belirtmeye ilişkin bir örnek verilmiştir
...
[Scaleway Generative APIs](https://www.scaleway.com/en/docs/generative-apis/)'yi Nexus ile kullanmak için:
...
Modelleri nexus yapılandırmanız aracılığıyla da özelleştirebilirsiniz. Sağlayıcı yönlendirme sırasını belirtmeye ilişkin bir örneği burada bulabilirsiniz.
...
:::tip
OpenAI uyumlu herhangi bir sağlayıcıyı nexus ile kullanabilirsiniz. Çoğu modern yapay zeka sağlayıcısı OpenAI uyumlu API'ler sunar.
:::
...
4. Proje dizininizde `nexus.json` dosyanızı oluşturun veya güncelleyin:
...
`limit` alanları nexus'un ne kadar bağlamınız kaldığını anlamasını sağlar. Standart sağlayıcılar bunları models.dev'den otomatik olarak alır.
...
1. **Kimlik doğrulama kurulumunu kontrol edin**: Kimlik bilgilerinin geçerli olup olmadığını görmek için `nexus auth list` komutunu çalıştırın.
...
2. Özel sağlayıcılar için nexus yapılandırmasını kontrol edin ve:
   - `/connect` komutunda kullanılan sağlayıcı kimliğinin, nexus yapılandırmanızdaki kimlikle eşleştiğinden emin olun.
