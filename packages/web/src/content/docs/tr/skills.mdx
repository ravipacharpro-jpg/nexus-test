...
Ajan becerileri, nexus'un deponuzdan veya ev dizininizden tekrar kullanilabilir talimatlari kesfetmesini saglar.
...
nexus su konumlari tarar:

- Proje konfigurasyonu: `.nexus/skills/<name>/SKILL.md`
- Genel konfigurasyon: `~/.config/nexus/skills/<name>/SKILL.md`
...
Proje yerel yollarinda nexus, mevcut calisma dizininizden baslayip git worktree kokune kadar yukari cikar.
Bu sirada `skills/*/SKILL.md` altindaki eslesen `.nexus/` dosyalarini ve `.claude/skills/*/SKILL.md` ya da `.agents/skills/*/SKILL.md` dosyalarini yukler.

Genel tanimlar da `~/.config/nexus/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` ve `~/.agents/skills/*/SKILL.md` yollarindan yuklenir.
...
`.nexus/skills/git-release/SKILL.md` dosyasini soyle olusturun:
...
nexus, `skill` araci aciklamasinda mevcut becerileri listeler.
...
`nexus.json` icinde pattern tabanli izinlerle ajanlarin hangi becerilere erisecegini kontrol edin:
...
**Yerlesik ajanlar icin** (`nexus.json` icinde):
