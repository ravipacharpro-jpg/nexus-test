...
nexus ile birden fazla yerleşik tema arasından seçim yapabilir, terminal temanıza uyum sağlayan bir tema kullanabilir veya kendi özel temanızı tanımlayabilirsiniz.

Varsayılan olarak nexus kendi `nexus` temasını kullanır.
...
nexus birden fazla yerleşik temayla gelir.
...
- nexus'un terminal görünümüyle birebir uyumlu olmasını isteyenler
...
nexus, kullanıcıların kolayca tema oluşturup özelleştirebilmesi için esnek bir JSON tabanlı tema sistemi sunar.
...
2. **Kullanıcı yapılandırma dizini** - `~/.config/nexus/themes/*.json` veya `$XDG_CONFIG_HOME/nexus/themes/*.json`
3. **Proje kök dizini** - `<project-root>/.nexus/themes/*.json`
4. **Geçerli çalışma dizini** - `./.nexus/themes/*.json`
