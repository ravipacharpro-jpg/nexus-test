...
Araçlar, LLM'in kod tabanınızda eylem gerçekleştirmesini sağlar. nexus yerleşik bir araç setiyle gelir, ancak bunu [özel araçlarla](/docs/custom-tools) veya [MCP sunucularıyla](/docs/mcp-servers) genişletebilirsiniz.
...
nexus içinde bulunan tüm yerleşik araçlar aşağıdadır.
...
:::note
Bu araç yalnızca `NEXUS_EXPERIMENTAL_LSP_TOOL=true` (veya `NEXUS_EXPERIMENTAL=true`) olduğunda kullanılabilir.
:::
...
:::note
Bu araç yalnızca nexus provider kullanılırken veya `NEXUS_ENABLE_EXA` ortam değişkeni doğru değerlerden birine ayarlandığında (örneğin `true` ya da `1`) kullanılabilir.

nexus başlatırken etkinleştirmek için:
