...
nexus ile ilgili bir sorunu ayiklamak icin once loglari ve diskte tuttugu yerel verileri kontrol edin.
...
Daha ayrintili ayiklama bilgisi icin `--log-level` komut satiri secenegini kullanabilirsiniz. Ornek: `nexus --log-level DEBUG`.

---

## Depolama

nexus, oturum verilerini ve diger uygulama verilerini diskte su konumda saklar:
...
nexus Desktop arka planda yerel bir nexus sunucusu (`nexus-cli` sidecar) calistirir. Sorunlarin cogu bozuk bir plugin, hasarli cache veya hatali sunucu ayarindan kaynaklanir.

### Hızlı kontroller

- Uygulamayi tamamen kapatip yeniden acin
- Uygulama hata ekrani gosteriyorsa **Restart**'a tiklayip hata detaylarini kopyalayin
- Yalnizca macOS: `nexus` menusu -> **Reload Webview** (arayuz bos/donukse yardimci olur)
...
- **macOS/Linux**: `~/.config/nexus/nexus.jsonc` (veya `~/.config/nexus/nexus.json`)
- **macOS/Linux** (eski kurulumlar): `~/.local/share/nexus/nexus.jsonc`
- **Windows**: `WIN+R` tuslayip `%USERPROFILE%\.config\nexus\nexus.jsonc` yapistirin
...
nexus diskten yerel plugin de yukleyebilir. Bu dizinleri gecici olarak tasiyin (veya klasoru yeniden adlandirin) ve uygulamayi yeniden baslatin:

- **Global plugins**
  - **macOS/Linux**: `~/.config/nexus/plugins/`
  - **Windows**: `WIN+R` tuslayip `%USERPROFILE%\.config\nexus\plugins` yapistirin
...
Pluginleri kapatmak ise yaramazsa (veya plugin kurulumu takili kaldiysa), nexus'un cache'i yeniden olusturmasi icin cache'i temizleyin.

1. nexus Desktop'u tamamen kapatin
2. Cache dizinini silin:

- **macOS**: Finder -> `Cmd+Shift+G` -> `~/.cache/nexus`
- **Linux**: `~/.cache/nexus` dizinini silin (veya `rm -rf ~/.cache/nexus` calistirin)
- **Windows**: `WIN+R` tuslayip `%USERPROFILE%\.cache\nexus` yapistirin

3. nexus Desktop'u yeniden baslatin

---

### Sunucu bağlantı sorunlarını düzeltin

nexus Desktop ya kendi yerel sunucusunu baslatir (varsayilan) ya da sizin tanimladiginiz bir sunucu URL'sine baglanir.
...
`nexus.json(c)` dosyanizda `server` bolumu varsa gecici olarak kaldirin ve desktop uygulamasini yeniden baslatin.

#### Ortam degiskenlerini kontrol edin

Ortamda `NEXUS_PORT` ayarliysa desktop uygulamasi yerel sunucu icin o portu kullanmaya calisir.

- `NEXUS_PORT` degerini kaldirin (veya bos bir port secin) ve yeniden baslatin
...
Windows'ta nexus Desktop, Microsoft Edge **WebView2 Runtime** gerektirir. Uygulama bos pencereyle aciliyorsa veya hic baslamiyorsa WebView2'yi kurup/guncelleyip tekrar deneyin.

---

### Windows: Genel performans sorunları

Windows'ta yavaslik, dosya erisim sorunlari veya terminal problemleri yasiyorsaniz [WSL (Windows Subsystem for Linux)](/docs/windows-wsl) kullanmayi deneyin. WSL, nexus ozellikleriyle daha sorunsuz calisan bir Linux ortami saglar.

---

### Bildirimler görünmüyor

nexus Desktop sistem bildirimlerini yalnizca su durumlarda gosterir:

- Isletim sistemi ayarlarinizda nexus icin bildirimler etkinse
...
1. nexus Desktop'u kapatin
2. Su dosyalari bulun ve silin (nexus Desktop uygulama veri dizininde yer alirlar):

- `nexus.settings.dat` (desktop varsayilan sunucu URL'si)
- `nexus.global.dat` ve `nexus.workspace.*.dat` (son sunucular/projeler gibi UI durumu)
...
nexus ile ilgili bir sorun yasiyorsaniz:
...
3. `nexus upgrade` ile en guncel surumu kullandiginizdan emin olun
...
Erisiminiz olan modelleri gormek icin `nexus models` calistirin.
...
API cagrisi hatalari aliyorsaniz bunun nedeni guncel olmayan provider paketleri olabilir. nexus, provider paketlerini (OpenAI, Anthropic, Google vb.) gerektikce dinamik yukler ve yerelde onbellekler.
...
2. En guncel provider paketlerini yeniden kurmak icin nexus'u yeniden baslatin

Bu, nexus'un en yeni provider paket surumlerini indirmesini zorlar ve model parametresi/API degisikliklerinden kaynakli uyumsuzluklari cogu zaman cozer.
...
nexus, Wayland kullandiginizi algilarsa `wl-clipboard` tercihi yapar. Aksi halde sirayla `xclip` ve `xsel` araclarini arar.
