...
To debug issues with NEXUS, start by checking the logs and local data it stores on disk.
...
You can set the log level with the `--log-level` command-line option to get more detailed debug information. For example, `nexus --log-level DEBUG`.

---

## Storage

nexus stores session data and other application data on disk at:
...
To uninstall the NEXUS CLI and remove its related files, run:
...
To remove NEXUS Desktop, uninstall the application through your operating system's app management tools.

---

## Desktop app

NEXUS Desktop runs a local NEXUS server (the `nexus-cli` sidecar) in the background. Most issues are caused by a misbehaving plugin, a corrupted cache, or a bad server setting.

### Quick checks

- Fully quit and relaunch the app.
- If the app shows an error screen, click **Restart** and copy the error details.
- macOS only: `NEXUS` menu -> **Reload Webview** (helps if the UI is blank/frozen).
...
- **macOS/Linux**: `~/.config/nexus/nexus.jsonc` (or `~/.config/nexus/nexus.json`)
- **macOS/Linux** (older installs): `~/.local/share/nexus/nexus.jsonc`
...
NEXUS can also load local plugins from disk. Temporarily move these out of the way (or rename the folder) and restart the desktop app:
...
If disabling plugins doesn't help (or a plugin install is stuck), clear the cache so NEXUS can rebuild it.

1. Quit NEXUS Desktop completely.
2. Delete the cache directory:

- **macOS**: Finder -> `Cmd+Shift+G` -> paste `~/.cache/nexus`
- **Linux**: delete `~/.cache/nexus` (or run `rm -rf ~/.cache/nexus`)
- **Windows**: Press `WIN+R` and paste `%USERPROFILE%\.cache\nexus`

3. Restart NEXUS Desktop.

---

### Fix server connection issues

NEXUS Desktop can either start its own local server (default) or connect to a server URL you configured.
...
If your `nexus.json(c)` contains a `server` section, temporarily remove it and restart the desktop app.

#### Check environment variables

If you have `NEXUS_PORT` set in your environment, the desktop app will try to use that port for the local server.

- Unset `NEXUS_PORT` (or pick a free port) and restart.
...
On Windows, NEXUS Desktop requires the Microsoft Edge **WebView2 Runtime**. If the app opens to a blank window or won't start, install/update WebView2 and try again.

---

### Windows: General performance issues

If you're experiencing slow performance, file access issues, or terminal problems on Windows, try using [WSL (Windows Subsystem for Linux)](/docs/windows-wsl). WSL provides a Linux environment that works more seamlessly with NEXUS's features.

---

### Notifications not showing

NEXUS Desktop only shows system notifications when:

- notifications are enabled for NEXUS in your OS settings, and
...
1. Quit NEXUS Desktop.
2. Find and delete these files (they live in the NEXUS Desktop app data directory):

- `nexus.settings.dat` (desktop default server URL)
- `nexus.global.dat` and `nexus.workspace.*.dat` (UI state like recent servers/projects)
...
If you're experiencing issues with NEXUS:
...
3. Ensure you have the latest version with `nexus upgrade`
...
To figure out what models you have access to, run `nexus models`
...
If you encounter API call errors, this may be due to outdated provider packages. nexus dynamically installs provider packages (OpenAI, Anthropic, Google, etc.) as needed and caches them locally.
...
2. Restart nexus to reinstall the latest provider packages

This will force nexus to download the most recent versions of provider packages, which often resolves compatibility issues with model parameters and API changes.
...
nexus will detect if you're using Wayland and prefer `wl-clipboard`, otherwise it will try to find clipboard tools in order of: `xclip` and `xsel`.
