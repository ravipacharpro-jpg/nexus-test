---
title: TUI
description: Using the NEXUS terminal user interface.
---

import { Tabs, TabItem } from "@astrojs/starlight/components"

NEXUS provides an interactive terminal interface or TUI for working on your projects with an LLM.

Running NEXUS starts the TUI for the current directory.
...
When using the NEXUS TUI, you can type `/` followed by a command name to quickly execute actions. For example:
...
Add a provider to NEXUS. Allows you to select from available providers and add their API keys.
...
This is separate from `nexus.json`, which configures server/runtime behavior.
...
- `leader_timeout` - Controls how long NEXUS waits after the leader key. Defaults to `2000`.
...
Use `NEXUS_TUI_CONFIG` to load a custom TUI config path.
...
- `sound_pack` - Sound pack ID to use. Defaults to `nexus.default`.
