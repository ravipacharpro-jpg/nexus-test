...
import { createNexus } from "@nexus-ai/sdk"
...
`)
  ...
  const nexus = await createNexus({ port: 0 })
  ...
  try {
    ...
    const session = await nexus.client.session.create()
    const result = await nexus.client.session
      .prompt({
