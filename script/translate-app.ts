...
export function parseTranslationArgs(args: string[]) {
  const parsed = parseArgs({
    ...
    options: {
      ...
      model: { type: "string", default: "nexus/gpt-5.5" },
...
export function glossaryFile(locale: Locale) {
  if (locale === "zh") return ".nexus/glossary/zh-cn.md"
  if (locale === "zht") return ".nexus/glossary/zh-tw.md"
  return `.nexus/glossary/${locale}.md`
}
...
export function sessionIDFromEvents(output: string) {
  ...
  if (!match?.[1]) throw new Error("NEXUS did not report a session ID.")
  return match[1]
}

export function sessionModels(value: unknown) {
  if (!isRecord(value) || !Array.isArray(value.messages))
    throw new Error("NEXUS returned an invalid session export.")
  return value.messages.flatMap((message) => {
    ...
    if (typeof message.info.providerID !== "string" || typeof message.info.modelID !== "string") {
      throw new Error("NEXUS session export omitted the assistant model.")
    }
    return [
      {
...
Options:
  -c, --concurrency <count>  Maximum parallel NEXUS runs for 'all' (default: 4)
      --model <provider/id>  NEXUS model (default: nexus/gpt-5.5)
      --variant <name>       Model variant (default: xhigh)
      --dry-run              Report drift without running NEXUS
...
`)
  ...
  if (failed.length) console.error(`\nNEXUS failed for: ${failed.map((result) => result.locale).join(", ")}`)
...
async function translate(
  ...
  env.NEXUS_DISABLE_PROJECT_CONFIG = "1"
  env.NEXUS_CONFIG_CONTENT = JSON.stringify(
    translationConfig(
      agent,
      model,
  ...
  const exported = Bun.spawn(["nexus", "--pure", "export", sessionID, "--sanitize"], {
    cwd: root,
    env,
    stdout: "pipe",
    stderr: "pipe",
...
async function resolveModelVariant(model: string, variant: string) {
  ...
  const env = isolatedEnvironment()
  env.NEXUS_DISABLE_PROJECT_CONFIG = "1"
  const proc = Bun.spawn(["nexus", "--pure", "models", provider, "--verbose"], {
    cwd: root,
    env,
    stdin: "ignore",
    stdout: "pipe",
...
function isolatedEnvironment() {
  ...
  delete env.NEXUS_CONFIG
  delete env.NEXUS_CONFIG_DIR
  delete env.NEXUS_CONFIG_CONTENT
  delete env.NEXUS_PERMISSION
  delete env.NEXUS_AUTO_SHARE
  return env
}
