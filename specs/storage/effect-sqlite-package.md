...
Create a small workspace package that vendors the Drizzle `effect-sqlite` adapter shape for our repo. This is not an nexus storage abstraction. It is a local package that ports the Drizzle Effect SQLite implementation so we can use it before/independently of upstream release timing.

`packages/nexus` will use it internally, but the package itself should be generic: Drizzle + Effect + SQLite. No nexus paths, migrations, tables, transaction hooks, post-commit behavior, or domain language should live in this package.
...
- `@nexus-ai/effect-drizzle-sqlite`
...
- Nexus-specific path/channel selection stays in `packages/nexus`.

## Nexus Adoption Notes

These are not package requirements, but they matter for the later nexus adoption PR.

The current `packages/nexus/src/storage/db.ts` has two non-obvious semantics that the nexus wrapper must preserve when it consumes this adapter:
...
The nexus wrapper can implement that using Effect context instead of `LocalContext`:
...
Do not remove this behavior while moving nexus to Effect SQLite. `SyncEvent.run` depends on transaction composability and `behavior: "immediate"` for sequencing correctness.

## Migration Strategy

1. Add `@nexus-ai/effect-drizzle-sqlite` with a minimal in-memory/file SQLite test schema.
...
4. Add `@nexus-ai/effect-drizzle-sqlite` as a dependency of `packages/nexus`.
5. Port `packages/nexus/src/storage/db.ts` to be a thin compatibility wrapper over the adapter plus nexus-specific transaction/post-commit context.
...
8. Only then build domain stores like session/message/project stores on top of nexus's storage wrapper.
...
An Effect Drizzle SQLite package lets us vendor the adapter once. Then nexus can build its own storage wrapper on top, and `SessionStorage`, `MessageStorage`, event store, and projector writes can all share the same transaction and migration model.
...
- Should `afterCommit` stay nexus-specific until event publishing moves? Default answer: yes.
...
- Do CLI/admin raw SQL and sqlite shell stay in `packages/nexus`, or does the storage package expose backend capabilities for them?
...
- Use a tiny test schema, not nexus domain tables.
- Prove Effect Drizzle SQLite queries, transactions, and migrations.
- Do not migrate `packages/nexus` yet except possibly adding the dependency if needed for typechecking.

That gives us a focused place to validate the Effect SQLite approach before disturbing nexus's current database runtime.
