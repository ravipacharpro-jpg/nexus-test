# Remove `packages/nexus/src/storage/db.ts`

## Goal

Remove all production usages of the legacy `packages/nexus/src/storage/db.ts` module.
...
This does not mean removing SQLite or Drizzle everywhere in one step. The smaller target is deleting the nexus legacy wrapper by moving call sites onto deeper modules or onto the core/effect database adapter directly.

## Current Inventory

Production imports from `packages/nexus/src/storage/db.ts` are concentrated in 22 source files:

- `packages/nexus/src/account/repo.ts`
- `packages/nexus/src/cli/cmd/db.ts`
- `packages/nexus/src/cli/cmd/import.ts`
- `packages/nexus/src/cli/cmd/stats.ts`
- `packages/nexus/src/control-plane/workspace.ts`
- `packages/nexus/src/index.ts`
- `packages/nexus/src/node.ts`
- `packages/nexus/src/permission/index.ts`
- `packages/nexus/src/project/project.ts`
- `packages/nexus/src/server/projectors.ts`
- `packages/nexus/src/server/routes/instance/httpapi/handlers/sync.ts`
- `packages/nexus/src/server/shared/fence.ts`
- `packages/nexus/src/session/message-v2.ts`
- `packages/nexus/src/session/projectors.ts`
- `packages/nexus/src/session/prompt.ts`
- `packages/nexus/src/session/session.ts`
- `packages/nexus/src/session/todo.ts`
- `packages/nexus/src/share/share-next.ts`
- `packages/nexus/src/storage/db.ts`
- `packages/nexus/src/sync/index.ts`
- `packages/nexus/src/worktree/index.ts`
...
Status: Completed. Startup, the public node export, and database CLI tooling no longer import the legacy nexus database wrapper; `packages/nexus/src/storage/db.ts` has been deleted.
...
- Move database path and client startup behind the core/effect database module rather than the nexus wrapper.
...
Status: Completed. `SyncEvent` and the nexus projector boundary were removed; session/message event projection now lives in core EventV2/projector infrastructure.
...
Status: Completed. These services no longer import the legacy nexus database wrapper.
...
Status: Completed. Session/message reads and projector writes have moved off the legacy nexus database wrapper.
...
Status: Completed. Remaining one-off CLI/admin reads and writes now use core database services or domain services instead of the legacy nexus database wrapper.
...
All migration groups are complete or superseded. `packages/nexus/src/storage/db.ts` has been deleted.

## Superseded: Data Migrations

Status: Superseded. No nexus data-migration group remains.

The previous nexus `data-migration.ts` service only backfilled session usage from message rows. That work is now covered by core database migration `packages/core/src/database/migration/20260510033149_session_usage.ts`, so there is no separate nexus data-migration group.
...
- Existing schema ownership remains in `packages/core/src/**/*.sql.ts`; do not move table definitions back into `packages/nexus`.

## Verification Commands

- `rg "@/storage/db|./storage/db|Database\.(use|transaction|effect|Client|getPath)|\bTxOrDb\b|\bTransaction\b" packages/nexus/src`
- `bun typecheck` from `packages/nexus`
- Relevant package tests from `packages/nexus`, not the repo root
