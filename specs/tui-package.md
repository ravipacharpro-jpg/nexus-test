...
Move the canonical NEXUS terminal application from
`packages/nexus/src/cli/cmd/tui` into a self-contained workspace package while
...
generic presentation libraries. It must not depend on `packages/nexus`,
`packages/cli`, or `@nexus-ai/core`.

The SDK is the TUI's NEXUS boundary. Missing backend data or operations must
...
- Do not preserve private imports by creating aliases from `packages/tui` back
  into `packages/nexus`.
- Do not replace private `packages/nexus` imports with `@nexus-ai/core`
...
- Keep legacy CLI command parsing, server startup, worker management,
  authentication, and config discovery outside `@nexus-ai/tui`.
...
- NEXUS domain data displayed by the TUI
...
```text
packages/nexus/src/cli/cmd/tui
```

Its private dependency on `packages/nexus` is primarily expressed through
the `@/*` TypeScript alias, which resolves to `packages/nexus/src/*`.
...
Status: Completed. The private `@nexus-ai/tui` workspace package now has an
...
- Add `packages/tui/package.json` with the name `@nexus-ai/tui`.
...
- The package has no dependency on `nexus`, `@nexus-ai/cli`, or
  `@nexus-ai/core`.
...
primitives now live in `@nexus-ai/tui`. The legacy host consumes explicit
...
and build inputs through `@nexus-ai/tui/runtime`. Movable app, component,
route, and feature-plugin code no longer reads NEXUS globals or process state;
...
Replace process-global NEXUS state with resolved TUI inputs.
...
`@nexus-ai/tui/config` now owns schemas, defaults, keybind resolution, the
...
- Keep config path discovery, project/global precedence, migration, variable
  expansion, and plugin package installation in `packages/nexus` initially.
...
canonical in `@nexus-ai/tui`. Configured references resolve through the new
...
adapters rather than NEXUS domain implementations.

Make the SDK the only NEXUS domain boundary used by the TUI.
...
- Domain-facing TUI code imports NEXUS data and operations only from
  `@nexus-ai/sdk`.
...
`@nexus-ai/tui`. The legacy host injects a narrow plugin host that retains
...
- Move plugin rendering tests to `packages/tui`; retain installation/loading
  integration tests in `packages/nexus`.
...
- The package has no imports from `packages/nexus`, `packages/cli`, or
  `@nexus-ai/core`.
...
public `@nexus-ai/tui` root while retaining worker/server/config/plugin and
...
- Keep the legacy yargs commands corresponding to current `thread.ts` and
  `attach.ts` in `packages/nexus`.
- Keep the legacy embedded worker and server startup in `packages/nexus`.
- Change those adapters to load config, create transport inputs, and call the
  public `@nexus-ai/tui` API.
...
- Remove obsolete `@tui/*` path mappings from `packages/nexus`.
- Remove stale test fixtures and update all imports to package exports.
- Narrow `@nexus-ai/tui` exports to intentional public entrypoints.
...
- No source under `packages/tui` imports `@/...`, `@nexus-ai/core`, or either
  executable package.
...
- TUI package imports do not reach into executable or backend implementation
  packages.
- SDK wire data is treated as the source of truth for NEXUS domain state.
...
cd packages/nexus && bun typecheck
...
rg '@nexus-ai/core|packages/nexus|packages/cli' packages/tui
rg 'src/cli/cmd/tui|@tui/' packages/nexus/src packages/nexus/test
```
...
- Build the current-platform `packages/nexus` binary.
