...
Both `action` and `resource` use nexus's existing wildcard matching behavior.
...
The relative policy precedence of direct project files and `.nexus` files is intentionally deferred until `.nexus` configuration is reviewed.
...
Plugins must not be allowed to add, remove, or override policy statements. Plugins can contribute functionality or configured providers; policy determines whether nexus permits an operation through its managed execution paths.
