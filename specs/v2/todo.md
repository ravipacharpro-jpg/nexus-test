...
The nexus server has moved to the Effect HttpApi backend. Remaining work is
...
- remove the public in-memory `@nexus-ai/llm` tool loop after replacing its
...
- plugins get global "nexus" instance like in that post i showed
- nexus instance has stuff like `nexus.session.prompt()` or
  `nexus.tool.register({...})`
...
- decide whether to preserve deprecated `@nexus-ai/llm` orchestration exports
